...
- inputs/textareas/selects → `border:1px solid var(--c-border-strong); border-radius:var(--r-sm); background:var(--c-surface);` + a shared **focus state** (none exists today — a11y gap): `input:focus-visible, textarea:focus-visible, select:focus-visible { outline:none; border-color:var(--c-accent); box-shadow:0 0 0 3px var(--c-accent-ring); }`. **AR#D-R3-5 — the ring must also reach the other interactive controls** (input-only left buttons/tabs/subtabs/links on the bare UA outline — uneven keyboard focus for a "polished + accessible" bar): add `button:focus-visible, .tab-btn:focus-visible, .subtab-btn:focus-visible, .link-btn:focus-visible, a:focus-visible { outline:none; box-shadow:0 0 0 3px var(--c-accent-ring); border-radius:var(--r-sm); }`. (No *global* `outline:none` — only these `:focus-visible` selectors drop it, so any unstyled control keeps the UA fallback.)
...
- **`.label` is generalized so the coupon code is styled (AR#D-R3-2).** The only `.label` rule today is the descendant-scoped `.order-info .label` (`admin/index.html:51`), so the coupon code's `<span class="label">` in `#coupons-list` (IMPLEMENT `renderCoupons`) renders as plain inherited text. Drop the `.order-info ` prefix → a standalone `.label{color:var(--c-text-muted);font-size:var(--fs-xs);text-transform:uppercase;letter-spacing:0.5px}` (retokened from its `#666`/`12px` values), so the order-card labels **and** the coupon code wear the same muted/uppercase affordance — same intent, now shared. (IMPLEMENT's old "`.label` already exists" line is corrected to match.)
...
```
The zones are role-routers into the same `#p-images`/`#p-media-list` (3a/3b/3c read them) — no separate store, payload shape unchanged. Sean render-tunes the visual (drop targets, grid, hover); the routing is fixed. **AR#D-R3-1 — the `seo_thumbnail` + `checkout_image` zones also set their column-backed fields.** Those two roles are NOT images-derived: `buildProductPayload` reads them from the dedicated `#p-seo-thumbnail` / `#p-checkout-image` fields (`admin.js:429`/`:432`) and the consumers read the **columns** (Stripe checkout image `api/products.ts:658` → `stripeSync.ts:67`; the OG/social `seo_thumbnail` via `main.js:60/74` + `product.js:204`) — no code derives either from a role prefix (only `hero-` is derived, gallery is `gallery-`-filtered). So a zone upload that only landed in `images[]` left the attribute **unset** (silent miss on a money + SEO path, and the GPT can set these as explicit fields — /admin's zone must match for F6 first-class parity). The success branch now mirrors the `thumbnail` special-case for both column-backed single-roles in **both** branches (new-row + existing-row refresh); the file still also lands in `images[]` exactly as `thumbnail` does, so the AR#F7 replace-in-place guard + the visual confirmation both keep working — populating the field is the load-bearing part.
