...
**Revision**: consolidated at v3.6.4; QA pre-flight at v3.6.5; **v3.6.6 — round-1 formal cold-A fold.** The cold-A pass returned NEEDS ANOTHER PASS (NARROW); nine findings were validated + folded (the `/api/config` field-name anchor, the `CRON_SECRET` production go-live handoff, the GPT end-sale identification heuristic + `auto_apply` on the coupon-list output, the `.mitem--errored` red-ring CSS, cron-vs-gpt actor attribution on a scheduled publish, the sold null-fallback operator harmonized to `!p.available` across all storefront sites, the auto-generated sale-code clarification, and a stale WS3 `authHeader` NEEDS-VERIFY removed) and two were dismissed as false alarms (the `/api/config` field names AND the storefront sale-CSS tokens are both VERIFIED present against the tree). **ONE owner decision was SURFACED, not folded:** the expanded publish gate (§2.7) can 400 a legacy catalog piece on edit-publish/republish or Available OFF→ON — grandfather vs backfill vs accept is Sean's call (REVIEW_PROMPTS ledger 66; the must-surface-400 + a legacy-shaped TESTING case are folded under every branch). Folds to date: round-1 A + round-1 breadth + owner-decisions + this round (see REVIEW_PROMPTS ledger 39-66). Rationale stays inline through the pre-execution loop; relocates to sibling `RATIONALE.md` only at the v4.0.0 execution cut (DEV_RULES → Build Guide Final Cuts).

**Required reading first**: `assets/docs/EVERLASTINGS_STORE.md` · `README.md` · THIS doc + its two addenda **in this same `assets/docs/archive/v3_6/` directory** (`v3_6_6_ADDENDUM_DESIGN.md`, `v3_6_6_ADDENDUM_TESTING.md`) · the design handoff (in the sibling `v3_5/` dir per the directory note above) in its own read order — `design-handoff/out/INTEGRATION.md` → `out/PRODUCT_LIFECYCLE.md` → `design-handoff/brief.md` → `design-handoff/data-flow.md` → the `out/` files (`products.html`+`products-app.js`, `orders*`, `sales*`, `account*`, `portal.css`, `portal.js`, `data.js`) → `design-handoff/controls.html`+`tokens.css` → `design-handoff/feedback/FEEDBACK_v1.md` + `design-handoff/reference/` → `out/README.md` · `assets/docs/archive/v3_5/v3_5_0_ROADMAP.md` · `.agent/DEV_RULES.md`.
...
| `commitAvail` ON / `promptStock` (`:228`,`:265`)                        | republish: `[PUT {quantity}]` → `PUT {available:true}` → `POST ?_action=publish {id}` — a legacy piece can **400** here (expanded gate); **SURFACE it** as a field-list toast, not a silent no-op (§2.7 SURFACED DECISION)                                                         |
...
> **⚠️ SURFACED OWNER DECISION (round-1 cold-A #1 — NOT folded; Sean's call; REVIEW_PROMPTS ledger 66).** The expanded `validatePublishRules` re-runs on **edit-publish (`:631`)** and **first-publish (`:654`)** — so it also gates the **republish of an EXISTING catalog piece** and the **Available OFF→ON round-trip** (Phase 2.2 unpublishes to draft on OFF; the `commitAvail` ON seam then re-publishes, hitting the full gate). A piece published under the **OLD** shared `validateProductRules` never had to carry `features / materials / care_instructions / shipping_details / dimensions(W·D·H) / weight`, nor **alt on every image + media entry** (alt is a NEW hard gate — ledger 23). So a live piece that predates this build can 400 (`"Missing required fields: care_instructions"` / `"Every image needs alt text"`) when Em edits it and taps Publish, or toggles it back up for sale — the thesis's "put it back up for sale" journey blocked on real inventory. **Flag-don't-assert:** it is *unverifiable from the docs* whether any live piece is actually under-filled (the live catalog's field-completeness isn't visible here), so this is structural-and-surfaced, not asserted-broken. **The choice is genuinely the owner's** — (a) **grandfather**: apply the strict set only to never-before-published rows (downside: a live piece could republish still missing alt, partly defeating the accessibility/SEO reason alt was made a hard gate); (b) **pre-go-live catalog audit/backfill** (esp. missing `alt`), sequenced exactly like the §6.5 legacy-quantity backfill already is (downside: a one-time fill task; upside: every live piece meets the new bar + republish just works — the path most consistent with this build's existing legacy-handling pattern); or (c) **accept**: let the 400 fire but add a maker-facing "these fields are needed to republish" surface. **Folded under EVERY branch (this part is NOT the decision, so it lands now):** (1) the interaction is documented here; (2) the `commitAvail` ON seam (WS2 seam table, `:457`) **must surface the 400 to the maker as a field-list toast** — never a silent no-op that reads as "I toggled it back on and it just didn't come back up"; (3) a **legacy-shaped TESTING case** (publish a deliberately alt-less / under-filled legacy-shaped seed and observe the 400) is added to the TESTING addendum. Do NOT re-raise this as an *unaddressed silent regression* — it is known, scaffolded, and surfaced to Sean pending his grandfather/backfill/accept call.
...
## Verification (end-to-end, dev preview — full plan in `v3_6_6_ADDENDUM_TESTING.md`)
