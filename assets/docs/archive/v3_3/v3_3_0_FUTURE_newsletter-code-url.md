...
2. **Email — brand BOTH welcome variants + embed the apply link (concrete drop-in).** Today `subscribe.ts:75-92` builds the email **inline** with its own `new Resend(...)` (`:12`) and a raw `resend.emails.send(...)`, and the *plain* (footer / no-code) welcome is a bare `<p>Welcome — your first email will arrive soon.</p>` — **no branded shell at all** (only the coupon variant has a shell, and even that one — `welcomeCouponEmailHtml` — isn't used; see the `:73` TODO). Route everything through `api/_emails/index.ts` instead:
   - **Coupon variant** (contemplation-offer): use the existing **`welcomeCouponEmailHtml`** (`_emails/index.ts:103`). Extend `WelcomeCouponEmailArgs` with an `applyUrl` and render a button — e.g. `https://everlastingsbyemaline.com/shop?promo=${code}` (matching whatever the front-end honors in item 1) — and **keep the code as text too** (some clients strip buttons).
   - **Plain variant** (footer / any no-code source): there is **no branded plain-welcome template yet** — add one, e.g. `welcomeEmailHtml()` in `_emails/index.ts`, reusing the same `shell()` helper the order/tracking emails use, so the footer welcome stops being a bare `<p>`. Give it real Firelight-Council copy (what the Council is, the cadence, the unsubscribe line) — this is the "very empty" email made whole.
   - **Send via the shared sender:** call **`sendEmail()`** (`_emails/index.ts:201`) instead of `subscribe.ts`'s own `resend.emails.send` — it already does the `RESEND_FROM_EMAIL` guard + consistent error logging and matches how the order/shipping emails send. Then delete the local `new Resend(...)`, the inline HTML, and the `:73` TODO from `subscribe.ts`.
   - **Net shape:** `subscribe.ts` picks the template by `source` (coupon vs plain), builds `{ subject, html }`, then `await sendEmail({ to: email, subject, html })` — one branded code path, with the ready-to-click apply link baked into the coupon email.
