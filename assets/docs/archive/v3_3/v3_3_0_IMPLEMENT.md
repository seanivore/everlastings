...
**Required reading first**: `assets/docs/EVERLASTINGS_STORE.md` · `README.md` · THIS doc + its two addenda **in this same `assets/docs/archive/v3_3/` directory** (`v3_3_0_ADDENDUM_DESIGN.md`, `v3_3_0_ADDENDUM_TESTING.md`) · the two `v3_0_0` briefs (source, in `assets/docs/archive/v3_0/`) · `.agent/DEV_RULES.md`. *(`v3_3_0_RATIONALE.md` sits here too but is **not** required reading — see the banner below.)*
...
- **Operational steps the agent OWNS — do NOT pause the build for the human** (per the revised `v3_3_0_HUMAN_STEPS.md`, 2026-06-18): apply the WS6 migration yourself via the linked Supabase CLI (`supabase db push` applies ONLY the safe `record_sale` function — the cutover UPDATE stays commented and is a **no-op on the seeded test preview**; the real-catalog cutover is a separate one-time prod step, not part of this build); seed the test products the refund/inventory tests need; ensure `charge.refunded` is on the **preview** webhook (add via the Stripe API if missing — preview-only config); and produce the WS5 homepage assets (`text-to-lottie` + `hyperframes` skills → R2 per `reference_cdn_r2_drop` → the 3 `index.html` URL edits) as concrete placeholders. Prove everything you can **headlessly** (curl/API against the preview) so the run never stops for Sean. The genuine human touchpoints are **all end-of-build, never mid-build**: the GPT-client check (re-paste schema/instructions + spot-check in Em's ChatGPT account — the one surface you can't drive; you've already proven the Actions via curl), design feedback on the homepage + /admin, and the post-sign-off `dev → main` ship. (A genuine *decision-shaped* gap still stops → surface to Sean, per the bullet above — that is different from these operational steps, which you just do.)
