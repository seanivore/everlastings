...
**Status**: WS4 (admin) + WS5 (Lottie title + HyperFrames hero) both authored as concrete executable design; byte-anchored to source files during execution.
...
- inputs/textareas/selects → `border:1px solid var(--c-border-strong); border-radius:var(--r-sm); background:var(--c-surface);` + a shared **focus state** (none exists today — a11y gap): `input:focus-visible, textarea:focus-visible, select:focus-visible { outline:none; border-color:var(--c-accent); box-shadow:0 0 0 3px var(--c-accent-ring); }`. **The ring also reaches the other interactive controls:** `button:focus-visible, .tab-btn:focus-visible, .subtab-btn:focus-visible, .link-btn:focus-visible, a:focus-visible { outline:none; box-shadow:0 0 0 3px var(--c-accent-ring); border-radius:var(--r-sm); }`. (No *global* `outline:none` — only these `:focus-visible` selectors drop it, so any unstyled control keeps the UA fallback.)
...
- Replace every literal with a token **by the property's role**, not a blind find-replace (text→`--c-text`, surface fills→`--c-surface*`, hairlines→`--c-border`, accents→`--c-accent`) so no raw hex remains. The current literals map cleanly: `#ddd`→`--c-border`, `#666`→`--c-text-muted`, `#f7f7f7`→`--c-surface-2`, and **`#222`→`--c-accent`** (the only two `#222` usages are `button.primary` and `.subtab-btn.active` **backgrounds**, `admin/index.html:25,34` — both accent fills; body text is already `--c-text` via the global base). **The WS3.7 `admin.js` template-string inline styles** (`addImageRow`'s `.img-role`, `updateCoverage`, `addMediaRow`) + the new `.img-thumb` CSS build in WS3 *before* §4.1's `:root` lands, so they use the **`var(--c-token, #hexfallback)` form** (e.g. `var(--c-text-muted,#666)`) — they render correctly either way. After the build the **only** hex left anywhere is inside such fallbacks; *bare* hex is the violation the TESTING item-24 grep catches. **Two literals to name explicitly** (item-24 greps for ZERO bare hex, so each needs a stated target): (a) `a{color:#06c}` / `a:hover{#03f}` (`admin/index.html:66-67`) → **`--c-accent`** / **`--c-accent-hover`**; and (b) the two **retained** order-email pills `.pill.shipped` (`#cfe`/`#060`) → **`--c-success-bg`/`--c-success`** and `.pill.unsent` (`#fee`/`#800`) → **`--c-danger-bg`/`--c-danger`**, retokened **IN PLACE** (the orders list still uses them for email state, `admin.js:720-723` — the sweep retokens them, does **not** delete them).
- **`.label` is generalized so the coupon code is styled.** Today's only `.label` rule is the descendant-scoped `.order-info .label` (`admin/index.html:51`), so the coupon code's `<span class="label">` in `#coupons-list` renders as plain inherited text. Drop the `.order-info ` prefix → a standalone `.label{color:var(--c-text-muted);font-size:var(--fs-xs);text-transform:uppercase;letter-spacing:0.5px}` (retokened from its `#666`/`12px`), so order-card labels **and** the coupon code share the muted/uppercase affordance.
...
**Product-state taxonomy (ONE source for the P0 filter tabs AND the P2 badge).** Derive a single `productState(p)` from `is_published` / `draft` / `available` / `archived_at`, in this precedence (first match wins):
...
Write it once, verbatim (don't make the builder re-derive the canonical predicate):
```js
...
```
**Also delete the now-dead `onUploadImage` fn** (its only caller, the `#upload-btn` wiring, is gone — P3d). `wireProductSubtabs` is already scoped to `#product-subtabs .subtab-btn` and the orders handler is now scoped to `#tab-orders .subtab-btn`, so the two strips never cross-fire. **Phase 2.1e (WS2) wires the coupon form/refresh/search at its OWN anchor** (`admin.js:164-170`, right after the orders-search `keydown` handler) — three individual `addEventListener` calls (`coupon-form` submit, `coupons-refresh-btn` click, `c-product-search` input), **NOT** a `wireCouponEvents()` wrapper (no such fn exists) and **NOT** in this block. Apply 2.1e at its own byte-anchor; this consolidated diff carries WS4's wirings only.

*(iv) **Hide the product-list chrome while editing.** `openEditor` hides only `#products-list` (`admin.js:276`) and `closeEditor` un-hides only it (`:327`) — but the new `#product-subtabs` (P0(i)) and `.state-legend` (P2) are siblings, so the state-filter strip + color key stay above the editor mid-edit and a stray subtab click silently re-renders the hidden list. Toggle them alongside the list: in `openEditor` add `$('product-subtabs')?.classList.add('hidden'); document.querySelector('.state-legend')?.classList.add('hidden');` and the mirror `.remove('hidden')` in `closeEditor`. (`.toolbar` may also be hidden mid-edit if it reads as clutter — it's pre-existing, so optional render-tune.)*
...
- **P2 · Product-state visibility.** State lives in one cramped meta line (`renderProductList`, `admin.js:246-256`); pills are tiny grey lozenges (`index.html:68-73`). Promote the **status pill to a badge overlaid top-left on the card image** (`.product-card{position:relative} .pc-badge{position:absolute;top:var(--s-3);left:var(--s-3)}` — `--s-3` (12px) matches the `.product-card` padding so the badge sits flush on the image corner); give pills semantic color + shape (`.pill{display:inline-block;border-radius:var(--r-pill);padding:3px 9px;font-size:var(--fs-xs);font-weight:var(--fw-semibold)}` — **merge into the existing `.pill` rule; KEEP `display:inline-block`** so the padding renders) — mapped draft→warn, edits→info, live→success, archived→neutral, sold→accent-soft (a sale is a GOOD state, not a red error); and **refunded**→neutral/muted as its OWN `.pill.refunded` class — the order-card "Refunded" pill (IMPLEMENT 1.5b) uses it instead of reusing `.unsent` (an email-not-sent state); **dim archived cards** (`.product-card.is-archived{opacity:.6}`, class added in JS when `p.archived_at`). Status becomes the loud signal, price the quiet meta. Minimal JS: build the pill as a separate `.pc-badge` node + the conditional class. **Concrete per-state rules (ship these literally):**
```css
...
**State legend (concrete default; colors-without-a-key read as decorative on a long list).** When the P0 state-filter subtabs are active, render a one-line key above the product grid, each label wearing its own `.pill` class so the legend doubles as the color key: `<div class="state-legend"><span class="pill live">Live</span><span class="pill edits">Edits</span><span class="pill sold">Sold</span><span class="pill draft">Draft</span><span class="pill archived">Archived</span></div>` (**no `Refunded` swatch** — `productState` returns only live/edits/sold/draft/archived; `.pill.refunded` lives only on order cards) + `.state-legend{display:flex;gap:var(--s-2);flex-wrap:wrap;margin:var(--s-2) 0 var(--s-3)}`. **Insertion anchor:** drop the static `<div class="state-legend">…</div>` in `admin/index.html` right **after `#product-subtabs`** (above the toolbar and the `#products-list` grid at `:115`); labels are static, no JS needed. Render-tune: hide it on the "All" tab if it reads as clutter.

**`renderProductList` — byte-anchored.** The badge/filter/dim/meta rewrite lands on code that *actively contradicts* it, so it's byte-anchored (a builder adding the badge over the surviving two-pill line would double the status / keep `.unsent` for "sold"). The shipped fn builds TWO pills with an inline predicate: `availPill` reuses bare `.pill` / `.pill.unsent` ("sold" as a red *email-not-sent* state) and `statusPill` reuses `.pill.shipped` for "live" — the exact semantics P2 removes. **CURRENT (`admin.js:235-261`):**
```js
...
- **P3 · Media (the biggest win — removes the only raw-JSON field).**
  - **3a Image-list previews.** `addImageRow` (`admin.js:331-345`) renders URL+alt+Remove only. Prepend a 40×40 `<img class="img-thumb">` (`object-fit:cover; var(--r-sm); background:var(--c-surface-2)`) whose `src` tracks the URL input, + a role-prefix tag from the filename covering **all 7 roles** (`hero`/`thumbnail`/`seo_thumbnail`/`checkout_image`/`gallery-NN`/`detail-NN`/`video-NN` — matches IMPLEMENT 3.7a's regex; the earlier 5-role version dropped `seo_thumbnail`/`checkout_image`). Regrid `.img-url-row` → `40px 64px 1fr 1fr auto` (thumb, role, url, alt, remove) — matches IMPLEMENT 3.7a.
  ...
  - **3c Structured MP4 / YouTube editor (replaces the raw-JSON `#p-media` textarea, `index.html:159`).** `buildProductPayload` (`admin.js:449-455`) currently `JSON.parse`s it and throws on any typo. Replace with a repeatable row builder (sibling of the image list): `#p-media-list` + "Add video" button; each row = `<select>` (video/youtube) + URL input + (video-only) `loop`, `autoplay` & `controls` checkboxes + a poster picker + Remove. New `collectMedia()` (sibling of `collectImages`, `347-356`) serializes the **identical** array shape; in `buildProductPayload` swap the parse block for `payload.media = collectMedia() || null;`; in `openEditor` (`298`) build rows from `eff.media` instead of `JSON.stringify`. Eliminates the JSON.parse throw path entirely; **backend untouched**. *(This is where admin↔GPT video parity lands — the structured editor exposes the same flags the GPT sets conversationally: `loop`, `autoplay`, **`controls`** (the GIF-like "no buttons" vs click-to-play state), and poster. The `controls` toggle closes the one media flag /admin couldn't reach — owner-settable here now, so TESTING item 21 needs no manual Supabase seed. `collectMedia()`/`openEditor` carry `controls` in the same array shape; backend already stores it.)* **Concrete-default CSS:** the clip head row's three-column grid ships as a real class **`.media-row__head`** — its base rule is added to the inline `<style>` in the **P3d concrete-CSS block below** (with the upload-zone rules), **not** an inline `style=` grid, because an inline grid can't be overridden by the P5 media query (it would overflow at ~360px). It stacks to one `1fr` column at ≤640px in P5. The row inputs carry `--c-border-strong` via the `var(--c-border-strong,#hex)` fallback form, and `.m-url` gets `min-width:0` so its `1fr` track can shrink below the URL's intrinsic width.
  - **3d Role-sectioned upload control.** Replace the single file-input + one big role `<select>` (`admin/index.html:195`, `admin.js:onUploadImage`) with **labeled upload zones per role group**: **Hero** · **Gallery** (auto-numbers `gallery-01..NN`) · **Detail** · **Thumbnail** · **SEO thumbnail** · **Checkout image** · **Video** (`video-0N`, auto-skips the Cloudinary crop). The role is chosen by **which zone you drop into**, not a dropdown — so every upload carries the correct role (load-bearing: the frontend reads role from the filename), the role-select footgun is gone, and video gets a first-class home. Each zone lists its uploads inline, feeding the 3a previews + 3b coverage hint. Vanilla DOM; same per-file `/api/upload` multipart call with the zone's role. Concrete default + render-tune (executable design).
    - **Functional rules — lock these (they're behavior, not visual):**
...
```
**NEW (a short **Upload** section heading above seven zones; each zone's label replaces the instructional `<p>`, each zone's `.zone-msg` replaces the global `#upload-status`, and the Video zone's automatic `skip_transform` in the JS below replaces the manual checkbox):**
```html
...
```
*(The file input is **wrapped inside** its `<label>` so the zone name is clickable + programmatically associated, not a bare sibling [a sibling `<label>` does nothing on click and SR users hear only "Choose File"]. `zoneEl.querySelector('.zone-file')` in the wirer is unaffected by the nesting. The structured media-row inputs get `aria-label`s in IMPLEMENT 3.7b; the video-opts checkboxes are already `.checkbox-row`-wrapped.)*
...
```
The zones are role-routers into the same `#p-images`/`#p-media-list` (3a/3b/3c read them) — no separate store, payload shape unchanged. Sean render-tunes the visual (drop targets, grid, hover); the routing is fixed. **The `seo_thumbnail` + `checkout_image` zones also set their column-backed fields.** Those two roles are NOT images-derived: `buildProductPayload` reads them from the dedicated `#p-seo-thumbnail` / `#p-checkout-image` fields (`admin.js:429`/`:432`) and the consumers read the **columns** (Stripe checkout image `api/products.ts:658` → `stripeSync.ts:67`; the OG/social `seo_thumbnail` via `main.js:60/74` + `product.js:204`) — no code derives either from a role prefix (only `hero-` is derived, gallery is `gallery-`-filtered). So a zone upload that only landed in `images[]` would leave the attribute **unset** (a silent miss on a money + SEO path; the GPT sets these as explicit fields, so /admin's zone must match). The success branch mirrors the `thumbnail` special-case for both column-backed single-roles in **both** branches (new-row + existing-row refresh); the file still also lands in `images[]` exactly as `thumbnail` does, so the replace-in-place guard + visual confirmation keep working — populating the field is the load-bearing part.
...
- **P4 · Loading / error / empty states.** Orders loading is bare text (`admin.js:651`); products has no loading state (`218-232`). Add **skeleton** cards (`.skeleton{background:linear-gradient(90deg,var(--c-surface-2) 25%,#e9ecef 37%,var(--c-surface-2) 63%);background-size:400% 100%;animation:shimmer 1.4s infinite;border-radius:var(--r-sm)}` + `@keyframes shimmer{to{background-position:-200% 0}}` + `@media (prefers-reduced-motion:reduce){.skeleton{animation:none}}`). Empty states: `.empty{color:var(--c-text-muted);padding:var(--s-7);text-align:center;border:1px dashed var(--c-border);border-radius:var(--r-md)}` (`--c-text-muted` ≈ 5.8:1, meets AA for body copy; `--c-text-faint` is for true placeholders only) + a CTA where one exists. Errors keep `.status-msg.error` + token colors + a `border-left:3px solid var(--c-danger)` accent (success/info likewise) so they're distinguishable beyond tint.
...
**A11y / SEO / reduced-motion DOM + CSS (exact shape).** CURRENT `index.html:170` is exactly `<h1>Step into Elsewhere</h1>` (copy confirmed — the swap only adds the wrapper + Lottie div, never rewrites the copy). Replace the `.hero__content` `<h1>` with:
```html
