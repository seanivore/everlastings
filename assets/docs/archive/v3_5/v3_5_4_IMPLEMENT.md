# v3.5.4 Implementation Plan — the Content Creator Portal redesign + the v3.5 store/checkout backlog
...
**Revision driven by**: round-1 + round-2 A/B/C/D folds, then **round-3 6-subagent fold** (A/B/C/D + 2 breadth) — v3.5.0 → v3.5.1 → v3.5.2 → v3.5.3 → **v3.5.4**. Round-3 verdicts: C = READY; A/B/D + both breadth = NARROW. Load-bearing round-3 folds: (1) **B-1** — the round-2 `?preview=` guard referenced `previewToken` out of scope in `populateStickyCard` (a ReferenceError on every PDP load) → §6.5b now re-derives it locally; (2) **A-D4-CSS** — the `.badge-unique` hue rule was speced in DESIGN §D.4 but no phase wrote it → new **§9.2a** writes it to `styles.css` (solid tokens). Plus: the badge gated `!sold && !p.featured` to avoid corner-overlap (D3-1); `main.js` §4.3.b + §4.7.0 shared-`DOMContentLoaded` coordination entry; the §2.4 Schedule gate keys off field-completeness not preview (J2); a legacy-sold-row backfill preflight in TESTING (C); §9.1 marked no-code-edit + the §4.6 `code`-source note. *(Round-2 folds, still current: the WS4-struck ↔ WS6-sold card-render collision → WS6→WS4→WS9 merged block, and the `?code=` site-wide `main.js` capture.)* (Living docs renamed to `v3_5_4_*` at this bump, per DEV_RULES current-only; all prior `GAP_REVIEW_*` are kept standing as the rigor trail. Next: a scoped A/B/D round-4 on the round-3 fixes, then minor-bump to `v3_6/` for the formal Agent-SDK A-Type gate.) Rationale is kept inline here on purpose; it is relocated to a sibling `RATIONALE.md` only at the v4.0.0 execution cut (DEV_RULES → Build Guide Final Cuts).

**Required reading first**: `assets/docs/EVERLASTINGS_STORE.md` · `README.md` · THIS doc + its two addenda **in this same `assets/docs/archive/v3_5/` directory** (`v3_5_4_ADDENDUM_DESIGN.md`, `v3_5_4_ADDENDUM_TESTING.md`) · the design handoff in its own read order — `design-handoff/out/INTEGRATION.md` → `out/PRODUCT_LIFECYCLE.md` → `design-handoff/brief.md` → `design-handoff/data-flow.md` → the `out/` files (`products.html`+`products-app.js`, `orders*`, `sales*`, `account*`, `portal.css`, `portal.js`, `data.js`) → `design-handoff/controls.html`+`tokens.css` → `design-handoff/feedback/FEEDBACK_v1.md` + `design-handoff/reference/` → `out/README.md` · `assets/docs/archive/v3_5/v3_5_0_ROADMAP.md` · `.agent/DEV_RULES.md`.
...
*1.3b — add the config + Supabase auth bootstrap (preserves `admin.js:90-221`).* The delivered `portal.js` has **no** config fetch and **no** Supabase — the prototype fakes sign-in. Add a shared `PORTAL.boot()` that reproduces the retired `admin.js` bootstrap: load `/api/config`, create one Supabase client (same options as `admin.js:109-111`), resolve the session, optionally gate the page, and expose `PORTAL.authHeader()` for the data-wiring workstreams. **CURRENT (`portal.js:26-28`):**
```js
  /* the storefront URL for the CURRENT environment (test → this deployment's root; live → prod) */
  P.siteUrl = function () { return P.env().isTest ? location.origin : "https://everlastingsbyemaline.com"; };
...
<!-- NEEDS-VERIFY: every real call must carry the admin Bearer (`...authHeader()` in /admin, `assets/js/admin.js`). The design shell has no auth helper — confirm where the portal stores the session token and expose an `authHeader()` equivalent before wiring, or all calls 401. -->
...
Record three answers: (1) does `applyPromotionCode` succeed when called at init (before the shopper touches a field), (2) does the `change` listener then see the discounted `session.total.total.amount`, (3) does a `removePromotionCode` (or equivalent) exist so a shopper can strip the sale code and enter a personal one.
...
- **No `removePromotionCode` on the bundle →** the keyword field stays the removal path: typing a personal code and pressing Apply *replaces* the sale code (Stripe swaps the promotion), so "delete the sale, use mine" still works without a dedicated remove call. <!-- NEEDS-VERIFY: confirm applyPromotionCode with a second code REPLACES rather than stacks on this bundle -->
...
```ts
  ...
  const autoApply = body.auto_apply === true && body.type === 'percent';
...
```
<!-- NEEDS-VERIFY: confirm [data-cart-subtotal] and [data-cart-estimate] both exist in cart.html and that the estimate line is where the "you'll pay" figure belongs; if there's only one total hook, apply the struck render there. -->
...
- **Start store-wide sale (`%`)** — `#startStoreWide` onclick (sales-app.js:49–54) → **`POST /api/products?_action=coupon`** with `{type:'percent', value, auto_apply:true, code:'<known code>'}`. The known promo code is required (auto-apply + share link need it); server supersedes any prior auto-sale (Phase 4.1.c).
...
- **End store-wide sale** — `#endStoreWide` → `confirmDialog` (sales-app.js:37) → **`POST /api/products?_action=coupon_deactivate`** with `{promotion_code_id}` (or `{code}`) of the active auto-sale.
...
**c. Role REASSIGNMENT of an already-uploaded image requires a re-upload under the new role filename (via the by-link path — no new endpoint).** Because the storefront reads role from the filename, unchecking `gallery` and checking `hero` on an existing `gallery-03-slug.webp` does **not** make it the hero on-page — the file is still named `gallery-03`. To truly re-role it, the modal must `POST /api/upload` **JSON** `{url: <existing CDN url>, slug, role: 'hero'}` — the server re-fetches the public R2 URL (passes `isPublicHttpUrl`), re-crops to the new role's aspect (`upload.ts:178-182`), and writes `hero-slug.webp`; then the old-role entry is dropped/kept per the new checkbox set. **This is the seam mechanic that makes the checkbox model honest against the filename invariant** — spell it out in the modal's Apply diff logic. <!-- NEEDS-VERIFY: confirm the modal computes an add/re-upload/remove diff between opened roles and applied roles, rather than a naive images[] rewrite — a naive rewrite leaves a promoted image matching BOTH /hero-/ (via imgs[0] fallback) and /gallery-/, duplicating it on the page. -->
...
```
*(The `<div>` wrapper is safe: `.filter-group > summary` (`styles.css:1029-1040`) targets the summary by direct child only, and `.checkbox-label` (`:544`) styles independently of its parent. **Taxonomy reconcile-at-build (SETTLED):** the series taxonomy is reconciled against the **live catalog** at build — the filter options are derived from live products' `series`, AND the header/footer `?series=` deep-link slugs (`portals-to-peace|book-nooks|story-lofts|seasonal|limited-edition`, nav in every page template + footer) are realigned to the live series' slugs so a deep-link actually pre-checks its box. Watch the compound-name case: "Book Nooks & Story Lofts" → `book-nooks-story-lofts` would NOT match the nav's `book-nooks`; at build, name the live series so their `seriesSlug()` matches the nav (or realign the nav slugs to the live names). A mismatch is a soft failure — shows all, no crash — but the reconcile removes it.)*
...
```
*(Merged edits, each with its own home section: the `sold` computation + Sold badge / dim / `text-muted` are WS6 §6.5a; the `${sold ? formatPrice(p.price) : priceHTML(p.price, window._activeSale)}` struck price is WS4 §4.5.b gated on `!sold`; the `badge badge-unique` "One of a kind" is WS9 §9.2, spec in DESIGN §D.4, its CSS written by **§9.2a**. `priceHTML` renders plain when there's no active percent sale, so the `!sold` branch is safe with or without a sale. The unique badge is gated **`!sold && !p.featured`** so a tile shows at most ONE of {Sold, Featured, One of a kind} — `.card__media .badge` pins every badge to the same corner (`styles.css:593`), so gating this way avoids overlap with zero new CSS; DESIGN §D.4 carries the optional badge-stack rule if you later want "One of a kind" on featured tiles too.)*
...
<!-- NEEDS-VERIFY: SUPABASE_SECRET_KEY is a project-wide Vercel env (api/webhook.ts + adminAuth already use it), so it is available to the product-feed function — confirm it isn't function/environment-scoped away from this route before shipping. -->
<!-- NEEDS-VERIFY: CRON_SECRET must be added to Vercel (Production scope) for reconciliation to fire; Vercel then auto-injects `Authorization: Bearer <CRON_SECRET>` on the cron hit. Until it is set, reconcileOrders() is a safe no-op (isCronRequest returns false) and the public feed is unaffected. This is the one ops step for 7.3. -->
...
export async function resolveActor(request: Request): Promise<string> {
  ...
  if (token === env('PRODUCT_API_KEY')) return 'gpt';
...
## Verification (end-to-end, dev preview — full plan in `v3_5_4_ADDENDUM_TESTING.md`)
