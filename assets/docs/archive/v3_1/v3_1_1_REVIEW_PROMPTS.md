# v3.1.1 — Gap-review prompts (cold A/B/C/D; fresh instances)

**The build under review** = `assets/docs/archive/v3_1/v3_1_1_IMPLEMENT.md` (functional **WS1-3 + WS6 inventory** byte-anchored; **WS4-5** executable design) **plus its same-version addenda** `v3_1_1_ADDENDUM_DESIGN.md` (presentation: WS4 admin redesign + WS5 homepage) + `v3_1_1_ADDENDUM_TESTING.md` (verification). **The three are one build; every addendum is always in scope.** Effort: maximum. **A new instance per pass** (no context contamination). Reviewers change **nothing** — output is findings only, written to the named `GAP_REVIEW` file.

**Why 4 this round:** v3.1.1 carries a substantial design surface (a full /admin re-skin — token system + P0–P7 — and a homepage Lottie title + old-film hero), so **D (design-correctness) gets its own lens** (per DEV_RULES, D is conditional on substantial design/UX). Collapse to A/B/C only if you'd rather — but then B+C must not skip the design addendum.
...
The current store — everything described in `EVERLASTINGS_STORE.md` and present in the repo today — is **built, tested, and approved to go live**; it's the fixed, proven substrate. **v3.1.1 is a delta layered on top** (refunds, coupons-in-admin, chat-attach upload, admin polish, homepage). Reviewers judge the **v3.1.1 changes** and whether they **fit** that base — they do **not** redesign or re-flag settled/shipped behavior, and a finding must be about what *this build* adds/changes (or a real conflict it creates with the base). This is inlined into every block so reviewers don't manufacture blockers out of proven code.
...
16. **The go-live (`main`) version is untouched** by v3.1.1 — it's a separate `dev` cycle that ships on its own later. Don't entangle them.
...
```
You are a senior engineer doing a pre-build gap review. Effort: maximum. Do NOT change anything — your only output is findings (write them to v3_1_1_GAP_REVIEW_A.md, or print the full file contents if you have no filesystem).
...
READ IN FULL + CO-DESIGN + FLAG-DON'T-ASSERT: read ALL provided docs end-to-end — don't ration tokens (you're at max effort, context is managed for you; grep/jump reading caused real mis-diagnoses last round). Within the v3.1.1 delta a gap is a gap whether it's a flaw in what we wrote OR something the delta SHOULD address but omitted — don't assume the build is mostly complete and you're only auditing what's there. When a finding depends on runtime/code behavior you can't directly see, FLAG it as needs-verification — do NOT assert it as broken (last round three "broken" findings were contradicted by the actual code).

SETTLED BASE — don't re-litigate what's shipped: the current store (all of EVERLASTINGS_STORE.md + the repo as it stands today) is built, tested, and approved to go live — the fixed, proven substrate. This build (v3.1.1) is a DELTA on top. Review the v3.1.1 changes for gaps + whether they FIT the base; do NOT redesign or flag settled/shipped behavior — a finding must be about what THIS build adds/changes, or a real conflict it creates with the base.

CONTEXT
- You are given FOUR documents and NO repository: v3_1_1_IMPLEMENT.md (the plan — WS1-3 byte-anchored with CURRENT/NEW; WS4-5 spec'd as executable design), v3_1_1_ADDENDUM_DESIGN.md (WS4 admin redesign + WS5 homepage), v3_1_1_ADDENDUM_TESTING.md (the E2E plan). A FRESH agent executes ALL of it against the everlastings repo, then tests on the dev preview. "Exclusively executable" = it embeds the exact current code + exact replacement for every edit, so the builder LOCATES and APPLIES, never DISCOVERS or DECIDES. EVERLASTINGS_STORE.md — the store's architecture/state doc, so you can judge whether the plan covers everything the store needs.
- LANDMINES (validate the plan against these, not training data):
  ...
  16. The go-live (main) version is untouched by v3.1.1 (a separate dev cycle).
...
```
You are a senior engineer doing a pre-build gap review. Effort: maximum. Do NOT change code or docs — output findings only (write them to v3_1_1_GAP_REVIEW_B.md).
...
```
You are a senior engineer doing a pre-build gap review. Effort: maximum. Do NOT change code or docs — output findings only (write them to v3_1_1_GAP_REVIEW_C.md).
...
```
You are a senior engineer + design reviewer doing a pre-build gap review. Effort: maximum. Do NOT change code or docs — output findings only (write them to v3_1_1_GAP_REVIEW_D.md).
