...
**MUST VIEW IMAGES**: 
  - `assets/docs/archive/images/checkout-issues-bug-round-three-1.jpg`
    #1 — if the user is not supposed to add address here then why is it there? It seems like you're supposed to because after this page you cannot uncheck or apply the "use same address" option. However the fields do expand in the next section. I think that perhaps the reason they are NOT showing up here is because they are trying to just show up ONE TIME through the process. We could remove them from this page, but then it makes this page, which is necessary to have so that we can add the step to make sure the product didn't sell while they were sitting with it in their cart, look strange in general. I would say we could add the name to this page instead of the address, but I imagine we might have a similar issue — if we do that we can handle it the same as how we handle the email and phone number — these are added to the newsletter list via this first page, and then they are copied into the official Stripe elements when we move to the next page; at least that is how it was explained to me. 
  - `assets/docs/archive/images/checkout-issues-bug-round-three-2.jpg`
    #2 — now there is one field to add a shipping address and full name but
    #3 — you can no longer uncheck (or check) the "Same as shipping" option on this second page. It seems like the logic of this flow is sort of convoluted, where it is confirming things in earlier stages and prevent them in later stages and vice versa which suggests a need to thoroughly dig through this flow to correct it. 
    #4 — This 'loading payment' button NEVER CHANGES. Even after you enter the address which you'll see in the next section. I suspect it might be connected to some of the other issues coming up. 
  - `assets/docs/archive/images/checkout-issues-bug-round-three-3.jpg`
    #5 — Now you can see that as I started typing the address, it used maps to help me, which is great. However you cannot access phone number anymore either. This is logical, but only if it is for both the email and phone number, and it should only be blocked out if it was filled in on the first page. If it was left empty on the first page then there is no reason that the user shouldn't be able to add their phone number to the shipment record at this phase, when they realize it might make more sense to since they are getting something delivered. You also still cannot change the check mark for "same as shipping" and the "loading payment" barely shown in the image here at the bottom, has STILL not finished loading the payment. You might want to use the Stripe CLI tool to look closely at the logs from this session for specifics. Though the next section might reveal why it is unable to prepare the payment amount. 
  - `assets/docs/archive/images/checkout-issues-bug-round-three-4.jpg`
    #6 — Here I simply was hoping that by clicking debit card and adding a number it might release the payment preparation or, as you'll see in the next image, activate the make payment button. It does not, though this was all filled out properly. 
  - `assets/docs/archive/images/checkout-issues-bug-round-three-5.jpg`
    #7 — Scrolling down you can see that the shipping is listed as CALCULATED BY STRIPE. I'm not sure if we want to do that or not. Regardless, I suspect that this is what is holding up the ability for the checkout session to finish loading properly. Again, in the CLI Strip logs it might say what stopped it from loading all the way, but it doesn't seem to have finished loading the checkout session, even though it allowed us to start entering information as if it had. Generally if it is calculating the shipping it is by knowing where she is shipping FROM via zip, and then using the zip for the shipping address just added. I am not confident that it knows where she is shipping from, however the business information was added so it does have the zip. Perhaps this setting should be switched to a rate set on the entry JSON for the product for now if it cannot be figured out dynamically. OH, dynamically, it would probably also need the weight and size of the package to calculate shipping. I'm not sure if this placeholder product has that information. 
