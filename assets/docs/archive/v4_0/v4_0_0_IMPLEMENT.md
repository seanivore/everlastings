# v4.0.0 Implementation — Content Creator Portal redesign + v3.5 store/checkout backlog

> **Read me first.** This is the **exclusively executable** build guide — every decision here is already settled. Build exactly what's written; don't re-litigate scope or approach, and trust that the details were worked out. The design reasoning lives in the sibling **`_RATIONALE.md`** — open it **only if a step is genuinely unclear**, never as required reading. **Read this doc start-to-end once before building:** it went through many revision rounds, so if two places ever disagree, the later / more-specific phase wins.

> **Source files.**
> - **Runtime design set** (the polished portal — port these into `admin/`): `assets/docs/archive/v4_0/design-source/out/` — `portal.css`, `portal.js`, `data.js`, the four shells (`products.html`/`orders.html`/`sales.html`/`account.html`), the four apps (`products-app.js`/`orders-app.js`/`sales-app.js`/`account-app.js`), plus `README.md` / `INTEGRATION.md` / `PRODUCT_LIFECYCLE.md` (read-only, do not ship).
> - **Design context** (unchanged reference): `assets/docs/archive/v3_5/design-handoff/` — `brief.md`, `data-flow.md`, `controls.html`, `tokens.css`, `feedback/FEEDBACK_v1.md`, `reference/`.
> - **Design + test detail** (the `DESIGN §A` surgical edits, `§C.1`, the `§D.1–D.4` NEW storefront-component specs cited from WS4/WS5/WS6/WS9, and the full verification plan): `assets/docs/archive/v3_7/v3_7_4_ADDENDUM_DESIGN.md` + `v3_7_4_ADDENDUM_TESTING.md` (frozen at v3.7.4 — they carry forward unchanged as the v4.0.0 design + test addenda; "DESIGN §…" and "the DESIGN addendum" throughout this doc mean `v3_7_4_ADDENDUM_DESIGN.md`).
> - **Backend truth:** `assets/docs/EVERLASTINGS_STORE.md` (living — if you find missing context, confirm with Sean and update it, don't paper over it). **GPT base files + ROADMAP:** `assets/docs/archive/v3_5/`.
> - **Read order:** `out/INTEGRATION.md` → `out/PRODUCT_LIFECYCLE.md` → `design-handoff/brief.md` → `design-handoff/data-flow.md` → the `out/` runtime files → `design-handoff/controls.html` + `tokens.css` → `design-handoff/feedback/FEEDBACK_v1.md` + `reference/` → `out/README.md`.

> **Design-file anchors — re-verify against the polished set.** The runtime design files (`out/products-app.js`, `out/*.html`, `out/portal.css`) are Claude Design's **polished** set — richer than when the surface-edit anchors below were written (e.g. `products-app.js` gained create-draft, a preview-first publish gate, column sorting + pagination). So **every anchor into a design file** (the `*-app.js` seam line refs, surgical markup edits like §2.1f's `.sched-chip` or §1.1c's shell `<head>`) is re-verified against `v4_0/design-source/out/` at build time: **the quoted CURRENT text is the anchor, not the line number — STOP and reconcile on drift; if a quoted CURRENT no longer exists (CD already fixed it), the edit is moot — skip it and note why.** Function-name anchors (`autosave`, `openPreview`, `commitPrice`, …) are stable; only line numbers moved. The backend anchors (`api/*.ts`) are unaffected — CD changed zero API calls.

**Scope.** One dev cycle (built/tested on `dev`, shipped to `main` on Sean's sign-off): (1) integrate the finished **Content Creator Portal** front-end as the new `/admin`, preserving the backend-complete management contract; (2) build the **store-wide sale** display/auto-apply layer + storefront struck-pricing + top utility bar + once-only popup on the existing coupon foundation; (3) rebuild the **media upload** UX to the designed modal + persist image order; (4) reconcile the **sold-policy / product-state** model; (5) fix the discrete **storefront bugs** + **webhook/money** items (even-split, cart-hold removal, reconciliation); (6) add the **activity log** + **seen/unseen** + **scheduled publish**; (7) research **buy-on-tile** and build the on-thesis version; (8) restore **GPT/admin parity** for the sale + refund guidance.

**Already built — preserve, don't rebuild.** The backend is complete (products CRUD + draft→preview→publish + archive/unarchive + live-apply price/quantity/available + coupons create/list/deactivate + orders list/mark-shipped + the amount-based sibling-aware Stripe-merged refund + upload). The redesign **wires to it**; it must **preserve**: the **two-speed field behavior** (live-apply price/quantity/available vs the staged `draft` overlay), the **client-side `effective` merge** (`admin.js:310` — the public `GET ?id=` returns the raw row, no server enrichment; keep merging client-side), the **sibling-aware refund panel** (`GET ?payment_intent=`), the **coupon scope picker**, and the **strict upload role enum**.

---

## Orchestration — the execution blueprint (evaluate at execution time)

This is a complete plan. At execution it carves into parallel **TRACK BUILDs** by execution boundary; the arrangement below is the starting suggestion, not a contract.
...
- **Last, sequenced:** WS10 GPT + docs parity (needs WS4's sale surface defined), then the **as-built `EVERLASTINGS_STORE.md` doc-sync** — a **fresh agent**, line-by-line, NEVER the tail of the build session.
- **Orchestrator-owned (do not delegate):** gate/promotion calls, branch state + commit cadence, escalation when a decision-shaped question surfaces (stop → surface to Sean, never decide), the end-to-end verification reads, and any file that ships verbatim (the GPT instruction `.txt` — re-run `wc -c` < 8000).

### Shared-file edit coordination (single-edit regions)

Three backend files are edited by more than one workstream. Apply their edits in the fixed order below and **re-anchor each stacked edit against the tree as the prior edit left it** (line hints drift; the quoted CURRENT text is the anchor).

- **`api/products.ts` — order: WS2 → WS4 → WS8.**
  - *GET dispatch* (around the `?_action=coupon` branch, ~:70-71): **WS4** inserts the public `?_action=active_sale` branch **just BEFORE** the coupon branch (§4.2.a — public/no-auth read ahead of the auth'd list) **and WS8** inserts the admin `?_action=activity` branch **just AFTER** the coupon branch (§8.1d). The two BRACKET the coupon branch (active_sale · coupon · activity); each anchors on the coupon line independently.
  - *new HANDLER FUNCTIONS append at the SAME spot:* **WS4**'s `handleActiveSale` (§4.2.b) **and WS8**'s `handleActivityLog` (§8.1d.b) BOTH append in the gap after `handleCouponList`'s closing brace (before the `// ?_action=coupon_deactivate` comment). Apply WS4 first; WS8 then re-anchors on the still-present `}\n}\n\n// ?_action=coupon_deactivate` text (now the tail of `handleActiveSale`) and appends after it. Both land between `handleCouponList` and `handleCouponDeactivate`; neither replaces the other.
  - *PUT handler:* **WS2** is the sole editor (available-off→draft, `scheduled_publish_at` set/clear, the new response flags).
  - *`handleCoupon`* (~:735 metadata, ~:745 return): **WS4** stamps `metadata.auto_apply` + the supersede-prior sweep, **WS8** wraps the return with a `sale.create` log — WS4's edits land first, then WS8's log. WS4 §4.1.c inserts its supersede-prior block between the CURRENT `:745-746` lines, so by the time §8.1c(g) applies they're no longer contiguous — WS8 re-anchors on the unique `return jsonResponse(...)` success return of `handleCoupon` and wraps THAT.
...
- **`assets/js/shop.js` + `assets/js/homepage.js` — order: WS6 → WS4 → WS9 (ONE merged NEW per card-render block).** WS6, WS4, and WS9 all edit the SAME card-render line(s): **WS6** (§6.5a shop card / §6.3d homepage tile) rewrites the whole render block for the quantity-based `sold` state; **WS4** (§4.5.b / §4.5.f) makes the `card__price` struck; **WS9** (§9.2) adds the "One of a kind" badge in `card__media`. Applied independently the last edit silently reverts the others. **Apply WS6 first (its block rewrite is the base), then fold WS4's struck value + WS9's badge INTO that block as ONE merged NEW:** §6.5a and §6.3d carry the fully-merged block; **§4.5.b, §4.5.f, and §9.2 are POINTERS into it, not standalone edits.** The struck price is gated on **`!sold`** (never a struck sale price on an unbuyable piece). *(`product.js` sticky card is NOT a line collision — §4.5.d [`:369`] and §6.5b [`:382`] land on different lines — but §4.5.d's struck gate is still reconciled to the same quantity-based `!sold` via a block-local `sold`.)*
- **`assets/js/main.js` — WS4 §4.3.b + §4.7.0 both append to the SAME `DOMContentLoaded` handler** (`main.js:269`, right after `initConfig()`): §4.3.b mounts the sale chrome (`getActiveSale().then(mountSaleChrome)`), §4.7.0 captures `?code=` → `sessionStorage`. They compose in **any** order; a builder must apply **BOTH** appends — do NOT treat `main.js` as single-owner. (`getCart`/`getCartTotal`/`priceHTML` are separate top-level `main.js` globals, untouched.)

Everywhere else each file has one owner or genuinely non-overlapping regions. The two storefront exceptions — `shop.js`/`homepage.js` card renders (WS6+WS4+WS9) and `main.js`'s shared `DOMContentLoaded` (WS4 §4.3.b + §4.7.0) — are the two bullets above.

---

## Goal (the thesis this build serves)

**Minimize the maker's friction to run her store, mostly from her phone.** The portal's innovation is **simple, entropy-lowering details** — a per-row state LED, field-border color rings, hard toggles, preview-anytime — wrapped in **familiar layouts** (a spreadsheet of rows, plain forms). Organize around **intent, not the data model** ("put this up," "take it down," "run a sale," "send a refund," "mark it shipped"). **Nothing hides without explaining** (a disabled control shows and says why). **Mobile is the primary context.**

**The parity rule.** Every management capability is equally doable in **the portal** AND the **Custom GPT** (the GPT could be down; the portal is easier for some moods). The parity test is whether each surface can *accomplish the essential task* — not that both expose identical controls; the dashboard may carry extra niceties the GPT lacks. Design for the reusable-template **"User,"** not for Emy specifically.
...
- **No new Vercel serverless function.** At **11/12** on Hobby — one slot free, reserved as headroom. The store-wide-sale set/end + the public active-sale read + the activity-log read all fold into `api/products.ts`; nothing adds an `api/*.ts` file.
- **No new cron.** At 1/1 usable cron (`/api/product-feed` daily). Scheduled-publish's flip and the reconciliation both fold into that existing daily function.
- **Sold-policy consistency (cross-cutting).** The state model is honored identically by **four** enforcers: the portal's `computeState()` (WS2), the storefront buy-gate `published && quantity>0` (WS6), the webhook decrement `record_sale` (WS7), and the deliberately-stricter **server checkout/reserve gate** `is_published===true && available===true && quantity>=1` (also rejects archived; `checkout.ts:79`/`:205`) — see WS6 §6.5.
- **`is_test` isolation holds.** Every order/product/sale lookup is scoped by `isTest` (`api/_lib/env.ts`); a test-env action can never touch a live row.
- **Auth unchanged.** `requireAdmin` (orders) / `authorize` (products/upload) accept `PRODUCT_API_KEY` **or** an admin Supabase JWT — no new auth.
- **No hard delete anywhere.** "Delete" = archive; everything is revivable (archive↔resurface, sold↔relist, draft↔publish). Mirrors Stripe (`product.active=false`; a price change is a new Price, never in-place).
- **Single-admin operation.** One operator at a time; reads-then-writes without cross-operator locking. Multi-seat is out of scope.
- **Money in integer cents everywhere.** Render with `PORTAL_DATA.money()` / storefront `formatPrice()`; never store or compare dollars.
- **Brand separation.** The portal is **cool indigo-slate** (a reusable template), deliberately distinct from the storefront's **warm-plum** brand. The storefront brand is untouched except the WS6 fixes + the WS4 struck-price/top-bar/popup additions (which use the storefront's own tokens).
- **GPT instruction char budget — hard cap 8000.** Any WS10 instruction add re-runs `wc -c` on the shipped `.txt`; over-cap, the GPT silently truncates its own instructions. If a WS10 add pushes over 8000, trim from the lowest-value clauses (abbreviate line 4's `Optl.` enumeration first — those fields live in PRODUCT-REFERENCE). The static gate re-counts every ship, so an over-cap `.txt` can never actually ship.
- **Reduced-motion preserved.** Portal motion honors `prefers-reduced-motion`; the storefront hero's reduced-motion fallback stays.
- **The go-live cutover is untouched.** ROADMAP section A (Em-gated) is a separate runbook, not this build.

---

## Locked decisions (already made — the builder chooses nothing)

**Product state / sold policy.** Three orthogonal things: (a) the **record** (never deleted), (b) **publish state** — Live (published) / Draft (never-published or unpublished) / Archived (retired = Stripe `active=false`), (c) **inventory** — the `quantity` number; `quantity=0` = **Sold**. Rules:
...
- **Sold** = `quantity` hits 0 from a **real sale** (automatic). Everlastings is one-of-a-kind, so **Sold is a persistent category kept until the maker Archives it** (a browsable record). Sold/archived pieces also have Available off, but that's a *consequence*, not the user action.
- **Wording is "Sold"** (a completed, archived-eligible sale), NOT "Sold out / out of stock."
- **`computeState()` is the single source of truth** for the dot/word (precedence: `archived > draft > staged-edits > sold(qty0) > live`); the only per-policy variable is which tab `sold` falls under (Everlastings = its own **Sold** tab). The row LED colors **ALL FIVE states** (`products-app.js` `ledFor()` → `.led--{state}`; `portal.css`): green=live, orange (`--waiting`)=staged-edits, yellow=draft, blue (`#297fb4`)=sold, purple-gray (`#83718a`)=archived. Sold + archived **ALSO** appear as tabs — **both the LED color AND a tab**. (live/draft/edits blink; sold/archived are steady.)
- **The Orders "unseen" blink is removed from the Sold tab** — Orders carries its own signal; Sold means only "pieces that have sold."
...
- **SOLD-tagging is a dependency of future product-type work.** The Sold **tab** only coheres while every product is quantity-1 (one row = one lifecycle bucket). When a multi-quantity product type is ever added (new schema — out of scope here), "Sold" must become a **flag** (`archive_reason = 'sold'`), not a tab, and the storefront "Sold" grid filters on that flag. File this with any future product-type schema work; do not pre-build it.

**Store-wide sale (the foundation exists — this is the display/auto-apply layer).**
- **Represented as ONE owner coupon** (percentage sales only for on-site struck pricing; `$`-off stays a checkout code) tagged `metadata.source='owner_sale'` **+ `metadata.auto_apply='true'`** + a known promo code. NO new `discounts` server param (keep `allow_promotion_codes:true`).
- **Auto-apply at checkout:** the storefront auto-applies that promo code on Custom Checkout load → the shopper types nothing; the keyword field **stays visible + removable/replaceable** (delete the sale code, enter a personal code — Stripe allows one discount/order). **GATED on the #219 probe** (does Stripe.js `applyPromotionCode()` fire reliably at `ui_mode:'custom'` session init for the loaded bundle — probe on the dev preview). Documented fallback if unreliable: apply on first interaction / prefill the field + a one-tap apply.
...
- **On-site display:** struck **%** sale prices on shop cards + product page + cart/total; a thin **top utility bar** (free-shipping reminder normally, sale line when active) + a **once-only** upper-right popup (dismissible, `localStorage`-gated). These use the storefront's own tokens.
- **Shareable pre-applying links (`?code=`):** a coupon **"Copy share link"** produces `<siteUrl>/?code=CODE`; `main.js` captures `?code=` on any page → `sessionStorage` (§4.7.0), and `checkout.js` applies it at checkout (§4.7), **instead of** the auto-sale (mutually exclusive). One-time codes mint via `handleCoupon` (`max_redemptions:1`). *(Backlog, not this build: extend the "Copy share link" affordance to regular/one-time coupon cards — the storefront already honors any `?code=`.)*

**Refund (PRESERVE, not build).** The amount-based, sibling-aware, Stripe-merged refund with per-piece relist **already works** (`orders.ts:246-341` + `admin.js:1005-1122`). The redesign **ports the existing refund UI into `orders-app.js`** unchanged in behavior: load all pieces by PaymentIntent, pre-check the clicked piece, auto-sum the amount but keep it freely editable, relist as a separate per-piece choice.

**Editor field rules.** Required-field publish gate uses the authoritative set (`title, slug, headline, description, price, quantity, product_type, story_card, features, materials, care_instructions, shipping_details, dimensions(W·D·H), weight, hero image, ≥5 gallery images, alt text on every media`) + **required-but-auto-generated-if-blank** (never block publish): `seo_title, seo_description, checkout_name, checkout_description, checkout_image, seo_thumbnail` — generated server-side from other fields if empty. **Optional:** `series`, `artist_note`. **Only `product_type = miniature` is in scope** (the picker must not offer printable/storybook — the API validates every type as miniature). **Dimensions = 3 inputs (W/D/H)** assembled/parsed to the `dimensions` string; **validate-and-auto-format, never reject** a good value — same for `weight`. **List fields** (`materials, features, care_instructions, shipping_details`) are one-item-per-line textareas ↔ `string[]`, with a live bullet preview. **Auto-save on close** (click the sticky row / click outside on desktop / ✕ on mobile) → `PUT /api/products` (a draft if new, staged edits if published); the explicit **Save** saves and returns to the list. **Publish requires a Preview** for a new, never-published product (the preview page carries the real Publish via `preview_token`).

**Field-lock taxonomy — three tiers, three lock moments.** Locked fields **show locked, not missing** (a lock chip + `disabled`, never hidden):
- **`sku` — born-locked:** generated at draft-create, never editable, no input affordance.
- **`slug` — locks on first persist:** auto-generates from the title the moment the title is entered (mirrors the DB `set_slug` trigger); the client lock cue keys on **`!!p.slug`** (locked once persisted — the first manual Save, or the auto-save on close — if non-empty). The server rejects any `slug` in a PUT **regardless of publish state** (`products.ts:359-361`), so slug is immutable from creation — NOT in the `everPublished` group.
- **`checkout_name, checkout_description, checkout_image` — lock at publish:** the checkout-identity snapshot. The client lock cue keys on **`everPublished = !!p.published_at`, NOT `is_published`** — a PAUSED (ever-published) piece keeps these locked, matching the server `FROZEN_AFTER_PUBLISH` guard (§2.7a). `checkout_image` defaults to the hero if still unset at publish and freezes then.
- In every tier, if a frozen field reaches the server changed, the 400 **SURFACES** as a field-list toast, never a silent no-op.

**Media (WS5 — full rebuild).** Rebuild `api/upload.ts` + the editor media modal to the FEEDBACK_v1 §8 spec: batch upload / drag-drop / URL / YouTube embed; per-image **role checkboxes** (one hero; hero≠gallery; share/checkout/poster combine freely); per-video **Loop / Mute / Hide controls / Autoplay**; required **alt text** on every item; reorder with **persisted image order** (array sequence, hero first then gallery 1..N); a coverage counter ("hero ✓, gallery 5/5"); "hero is reused for share/checkout if those are missing." Its own review pass.

**Scheduled publish (WS2 — no new cron).** `scheduled_publish_at` (ISO) column + a "Scheduled · <when>" chip; the flip folds into the existing daily `product-feed` cron (date-granular). An optional lazy on-read flip (a past-due scheduled draft flips live when next read) adds time-precision without a new cron. The schedule control ships working.
...
**Seen/unseen (WS8).** A `last_viewed` notion clears the Orders-nav blink; the blink is removed from the Sold tab.

**Reconciliation (WS7, IN, folded into the daily feed).** The existing `product-feed` cron gains a second job: compare Stripe's completed checkout sessions vs the `orders` table for the day → email Sean on any paid-session-without-order gap (do NOT email on raw signature-400s — bot noise). The Stripe-native webhook-failure email stays an ops step, not code.

**Storefront fixes (WS6).** Product page renders the missing detail fields (add populate code — they're never read today); series filter slugifies both sides + reconciles the taxonomy; featured carousel becomes independent horizontal-scroll rows (≤5 = one row, >5 = ~3-per-row decoupled scrollers); /complete drops the raw `cs_…` id (or shows a short friendly reference).

**Buy-on-tile (WS9 — decided: NO raw buy button).** The research concluded a grid buy button does not lift conversion for one-of-a-kind emotional art (the PDP is the conversion engine). So do **not** add "Buy Now"/"Add to Cart" on tiles; the effort goes to **information scent** instead — the already-generous full-tile tap target + the "One of a kind" scarcity badge (WS9). Reasoning: `_RATIONALE.md §9`.

**Portal routing.** Keep the `/admin` base — serve the 4 pages under `/admin/*`; bookmarks/GPT/muscle-memory unchanged. **Site title** on every portal page: `Creator Portal | Everlastings by Emaline`.
...
> **Foundation. Adopt the `out/` markup verbatim (the Design context is the markup source); byte-anchor the routing + config wiring.**

Stand the delivered 4-page portal up under the existing `/admin` base, preserving the current auth/config bootstrap. It changes **routing** (`vercel.json`), **shell wiring** (`portal.js`), and the **auth surface** (`account-app.js`); it does not touch product/order/sales *logic*. Invariants held here: **no new serverless function** (auth reuses `/api/config` + the CDN `@supabase/supabase-js`), the **`/admin` base stays**, the portal keeps its **cool indigo-slate** tokens, and there is **no `api/*.ts` edit**.

---

**Phase 1.1 — Land the `out/` package in `admin/`; retire the old SPA; harden each shell's `<head>`.**

*1.1a — copy the eleven runtime files into `admin/` (verbatim).* The delivered runtime set is `portal.css`, `portal.js`, `data.js` (3), the four shells, and the four surface apps — **eleven** files, in `assets/docs/archive/v4_0/design-source/out/`. Copy those eleven into `admin/` unchanged (WS1's own diffs are Phases 1.3–1.5; later workstreams make their own named surgical edits — WS2 miniature-only picker, WS8 Sold-tab alert removal, WS3 Delivered-pill removal, the `--staged` token fix). The three docs (`README.md`, `INTEGRATION.md`, `PRODUCT_LIFECYCLE.md`) stay in the source dir — **do not ship them**. All internal references are relative, so under `admin/` every asset resolves as `/admin/<file>` with zero path rewriting.

*1.1b — retire the single-page admin.* Delete `admin/index.html` (the 431-line 3-tab SPA). Its only outbound reference — the module load `admin/index.html:429` `<script type="module" src="/assets/js/admin.js"></script>` — dies with it, so `assets/js/admin.js` becomes unreferenced. Keep `assets/js/admin.js` **on disk, unshipped, as the canonical logic port-source** for the data-wiring workstreams; delete it in the final cleanup once the four `*-app.js` reach parity. <!-- NEEDS-VERIFY: confirm no build/deploy manifest bundles assets/js/admin.js independently of the index.html load before final deletion -->

*1.1c — add `robots` noindex + confirm the title, in all four shells.* The retired SPA carried `<meta name="robots" content="noindex, nofollow" />` (`admin/index.html:6`); the delivered shells dropped it. Generic `/admin/*` paths are a bot target — restore noindex on every page. The title is already correct verbatim — confirm, don't retype. **CURRENT (`account.html:6-8`, byte-identical in the other three shells):**
```html
<meta name="color-scheme" content="light">
<title>Creator Portal | Everlastings by Emaline</title>
<link rel="stylesheet" href="portal.css">
```
**NEW (insert the robots meta; title unchanged — repeat in all four shells):**
```html
<meta name="color-scheme" content="light">
...
```
**NEW (drop the `/admin/:path*` line):**
```json
    { "source": "/product/:slug", "destination": "/product" },
```

*1.2b — add the bare-`/admin` landing redirect.* Use a **redirect** (not a rewrite). `permanent: false` (307) keeps the landing relocatable. **CURRENT (`vercel.json:1-7`):**
```json
{
  "cleanUrls": true,
  "trailingSlash": false,
...
```json
{
  ...
  "rewrites": [
```
*(`vercel dev` does not always honor `cleanUrls`/redirects — validate this scheme on a real Vercel **preview** URL, never localhost. Why redirect over renaming: `_RATIONALE.md §1.2`.)*

**Doc impact:** none beyond recording the `/admin/{products|orders|sales|account}` scheme + products landing.

---

**Phase 1.3 — `portal.js`: confirm `PORTAL.env()`/`siteUrl()` (verify-only); add the `/api/config` + Supabase auth bootstrap.**

*1.3a — env / siteUrl are already correct — verify the anchor, do not edit.* `PORTAL.env()` derives Test/Live from `location.hostname` (`everlastingsbyemaline.com` → Live; `*.vercel.app`/localhost/`file://` → Test); `siteUrl()` returns the current preview origin on Test and prod on Live. **CURRENT (`portal.js:13-27`) — ships verbatim, NO CHANGE (if it has drifted byte-for-byte, STOP and reconcile):**
```js
  P.env = function () {
...
*1.3b — add the config + Supabase auth bootstrap (preserves `admin.js:90-221`).* The delivered `portal.js` has **no** config fetch and **no** Supabase. Add a shared `PORTAL.boot()` reproducing the retired `admin.js` bootstrap: load `/api/config` (**field names VERIFIED against `api/config.ts:5-11`: `publishableKey` · `supabaseUrl` · `supabasePublishableKey` · `isTest` — port these exact reads, or the whole portal fails to boot**), create one Supabase client, resolve the session, optionally gate the page, and expose `PORTAL.authHeader()`. **CURRENT (`portal.js:26-28`):**
```js
  /* the storefront URL for the CURRENT environment (test → this deployment's root; live → prod) */
  P.siteUrl = function () { return P.env().isTest ? location.origin : "https://everlastingsbyemaline.com"; };
...
*1.3c — load `@supabase/supabase-js` before `portal.js` in every shell.* `P.boot` calls `window.supabase.createClient`, so the UMD build must load ahead of `portal.js`. **CURRENT (`account.html:80-82`, byte-identical script order in `products.html:399-401`, `orders.html:145-147`, `sales.html:117-119`):**
```html
...
*1.4f — reduced-motion guard on the sign-in canvas FX.* `initLoginFx` (`account-app.js:115`) starts an unconditional `requestAnimationFrame` ribbon loop (`:174`); its only `matchMedia` is the `(max-width:560px)` mobile test (`:122`), so it ignores `prefers-reduced-motion`, and CSS (`portal.css:528-532`) can't reach a JS canvas loop. Guard the loop start: at the top of `initLoginFx`, `if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;`. No other behavior change; the identicon + form are untouched.
...
Each of the other three surfaces must not render for a signed-out visitor — wrap **EVERY entry statement** (not just the last one) in `PORTAL.boot({ requireSession: true }).then(...)`, which redirects to `/admin/account` when there's no session. If only the *final* call is wrapped, earlier entry statements still paint chrome/data before the redirect. Move the surface's whole terminal entry sequence inside `.then()`, in order. The bodies (data calls) are wired in later workstreams; WS1 only establishes this shared entry contract. Their entry points differ in COUNT (confirmed by reading each file):
...
- **`products-app.js`** — its whole init tail must run inside `.then()`: the env-chip IIFE (`:788-793`), the rail-collapse IIFE (`:796-805`), and `render();` (`:807`). Wrap all three.
...
```
*(Copy each IIFE's body byte-for-byte into its wrapper; the `PORTAL.refreshOrdersSignal()` call added by WS8 Phase 8.3.b sits at the END of this `.then()` — inside the wrap.)*
...
- **`products.html` static View Site link is hardcoded to prod** (`products.html:334` `href="https://everlastingsbyemaline.com"`), diverging from `mountShell`'s env-aware `P.siteUrl()` (`portal.js:151`). On a Test preview the View Site should point at the preview storefront — reconcile in the products data-wiring workstream (swap the static href for `P.siteUrl()` or inject the rail via `mountShell`).
- **`products.html` env chip already hides on Live** — the env-chip IIFE `products-app.js:788-793` has a Live branch that hides `#envChip`/`#envStrip` exactly like `mountShell`. No action.
- **`data.js` is mock and ships in WS1 so the shell renders;** its `products`/`orders`/`coupons`/`storeWideSale` arrays and its `config` object are the integration seam replaced downstream. WS1's Account card already reads the real key via `PORTAL.config`.

---

## WS2 — Products surface + sold-policy/state reconciliations + scheduled publish

> **Integration-seam for surface wiring; byte-anchored for backend reconciliations.** The redesigned surface (`out/products-app.js` + `out/products.html`) is a mock: `products = D.products` (`:11`), `autosave()` is a no-op (`:557`), every commerce/lifecycle action mutates the in-memory model with a toast. Phase 2.1 maps each mock/no-op onto the real `api/products.ts` + `api/upload.ts` contract (no backend change). Phases 2.2–2.8 are byte-anchored backend edits: line numbers are hints, the quoted **CURRENT** text is the anchor. Run `npx tsc --noEmit` clean after TS edits; the migration applies via the Supabase CLI.
...
| `commitAvail` ON / `promptStock` (`:228`,`:265`)                        | republish: `[PUT {quantity}]` → `PUT {available:true}` → `POST ?_action=publish {id}` — an under-filled piece can **400** here (the strict publish gate); **SURFACE it** as a field-list toast, not a silent no-op (§2.7)                                                         |
...
*(`computeState()` at `:16-22` already encodes the precedence — `archived > draft(!is_published) > edits(draft) > sold(quantity===0) > live` — with NO `!available→sold` branch; sold is qty-0-only. No client change to `computeState`. The editor's `eff = (k) => (p.draft && p.draft[k] != null ? p.draft[k] : p[k])` (`:340`) is the SAME client-side draft merge as `admin.js:310` — keep it; do NOT add `effective` to the hot GET.)*

**Phase 2.1 — surface wiring (integration-seam, not byte-anchored).**

- **a. Port `buildProductPayload` (the behavioral contract, `admin.js:561-617`).** The editor collects fields via `bindField`/`setEff` (`:529-544`) into the in-memory `p`; the real save serializes to the API body exactly as `admin.js` does: `price` → integer cents (`dollarsToCents`), list fields → `string[]` (`linesToArray` over the one-per-line textareas), `dimensions` already assembled by `bindField` (`:533`) as `` `${w}" W x ${d}" D x ${h}" H` `` and `weight` as `` `${n} lbs` `` (`:534`), `homepage_theme` parsed if present, empty scalars → `null`. On save/close: existing row → `PUT ?id=` (strip `slug` — it 400s as immutable, `products.ts:359`); new row → `POST`.
- **b. `autosave()` (`:557`) becomes the real PUT/POST.** Guard the brand-new case: only `POST` when `title` + `price` are present (a truly-blank New that's closed writes no row). Subsequent saves are `PUT`.
- **c. `product_type` picker → miniature-only.** Edit `editorHTML` (`:364`) `options: ["miniature", "printable", "storybook"]` → `options: ["miniature"]`. The server already treats every type as miniature (`PRODUCT_TYPE_RULES` has only `miniature`, `products.ts:281-286`); offering the others would store a mismatched `product_type`. No server change; surface-only.
...
- **f. Fix the undefined `--staged` token on the scheduled chip (surgical markup edit).** `products.html:288`'s `.sched-chip` uses `var(--staged)` in three `color-mix()`s, but `--staged` is defined nowhere in `portal.css`, so the scheduled chip renders with no background. Replace all three `var(--staged)` with `var(--waiting)`. **CURRENT (`out/products.html:288`, applied in `admin/products.html` after the WS1 copy):**
```css
...
**Phase 2.2 — `api/products.ts` PUT: Available-OFF on a published piece → unpublish to Draft (sold policy).** Turning Available off does NOT mean "sold" — sold is qty-0-from-a-sale only (computed). On a published row, a real OFF transition unpublishes (`is_published=false`) so `computeState` lands on `draft`. *(A genuinely-sold row already has `available=false`, so this OFF branch never fires there; the change-detect `!==` guards a no-op re-send.)* **CURRENT (`api/products.ts:456-466`):**
```ts
    if (
...
```
**NEW (add `unpublished` + `scheduled_updated`):**
```ts
    return jsonResponse(request, {
      success: true,
      product: data,
...
```
*(`checkout.ts:79`/`:205` gate on `is_published===true && available===true && quantity>=1`, so an unpublished row is doubly out-of-shop. The surface's `commitAvail` OFF already renders the "Moved to Drafts" toast — `products-app.js:238`.)*
...
**Phase 2.3 — migration: `scheduled_publish_at` column + partial index.** New `supabase/migrations/20260701000001_v3_5_scheduled_publish.sql` (rename the timestamp prefix to stay monotonic at apply time if a later migration exists — Supabase orders by filename).
```sql
...
**Phase 2.4 — `api/products.ts` PUT: accept `scheduled_publish_at` (set/clear) on both branches.** It's an operational directive applied LIVE like availability/quantity — never staged. It's NOT in `DRAFTABLE` (`:324-330`). Published branch — insert directly after the quantity block. **CURRENT (`api/products.ts:467-475`):**
```ts
    if (
...
```
**Gate the Schedule control on publish-READINESS (field-completeness), not merely "a publish is pending":** only offer "Schedule publish…" when the piece passes the required-field gate. Reuse the surface's own `readiness()` (`out/products-app.js:47` keys purely off the required-field set, with no bundled preview step), so a complete-but-un-previewed draft IS offered Schedule, and the cron (§2.6) enforces the same `validatePublishRules` `readiness()` mirrors. Offering Schedule only on a publish-ready piece guarantees a scheduled value always fires at cron — otherwise a missing-field draft that got scheduled would 400 and be silently skipped every day, forever (the "hides without explaining" failure the thesis forbids). This subsumes the clean-published-no-draft case (nothing to auto-publish → Schedule stays hidden). *(The surface's `openSchedule` (`:490-495`) already emits `new Date(dv+"T"+tv).toISOString()` and `unschedule` (`:454`) sets null; the chip renders off `p.scheduled_publish_at`, `:409-410`.)*

**Doc impact:** `EVERLASTINGS_STORE.md` — note scheduled publish is set/cleared via `PUT` and is live (never staged).

**Phase 2.5 — `api/products.ts` handlePublish: clear the schedule + persist the auto-generated checkout/SEO fields.** Publishing by ANY path (manual, capability token, or the scheduled fold) clears `scheduled_publish_at` so it fires once. Edit-publish update. **CURRENT (`:635-640`):**
```ts
...
```ts
    const { data: updated, error: applyError } = await supabase
      ...
      .single();
```
First-publish — the checkout essentials already fall back on the fly (`:658-666`) but the row columns stay null; "show locked, not missing" needs them PERSISTED (they lock via `FROZEN_AFTER_PUBLISH`), and the required-but-auto-generated set (`seo_title, seo_description, checkout_name, checkout_description, checkout_image, seo_thumbnail`) is generated here so publish is NEVER blocked on them. **CURRENT (`:676-681`):**
```ts
...
```ts
  ...
  const { data: published, error: pubError } = await supabase
    ...
    .single();
```
*(`syncProductToStripe` still reads `checkout_name || title` etc., so Stripe is unchanged; this writes the same values back so /admin shows them locked, not blank. **`published_at` is preserved when already set** (`?? new Date()`) — a paused piece relisting through this branch keeps its original publish date, §2.7a.)*
...
**Phase 2.6 — `api/product-feed.ts`: fold the scheduled-publish flip into the existing daily cron (no new cron, no new function).** The cron (`vercel.json:5`, `0 9 * * *`) is date-granular. This phase establishes the **single shared service-role client `feedAdmin`** (WS7 §7.3 reuses it) + imports `isTest`, and defines `publishDueScheduled`; the feed's own query keeps the RLS-scoped publishable key, which can't see unpublished rows. The publish reuses the `?_action=publish` handler over an authenticated self-call (zero publish-logic duplication — Stripe-create + shape re-validation + token/schedule clear for free). **The scan is `is_test`-scoped (`.eq('is_test', isTest)`); it runs ONLY inside the shared `isCronRequest(req)` gate assembled in §7.3c — never on a bare `GET`.** **CURRENT (`api/product-feed.ts:1-7`):**
```ts
...
**PostgREST `.or()` syntax — ship the fallback as a mechanism, not a build-time note.** The `.or('is_published.eq.false,draft.not.is.null')` clause uses `not.is.null` inside a comma-composed OR — version-sensitive on supabase-js/PostgREST. Replace the single `.or()` query with a runtime try/catch — attempt the `.or()` form first; on PostgREST error, re-issue WITHOUT the OR + emit the `console.warn` once per cold start. Also `title` on the select for the skipped-summary log:
```ts
...
**Skipped-publish backstop.** For a required field cleared *after* scheduling, surface the skip via a WS8 activity-log entry. `api/_lib/activityLog.ts` (WS8 8.1b) is a plain CommonJS module with its own per-invocation service-role client, so it imports cleanly here. Inside the fold's `if (!res.ok)` branch (add above the `console.error(...)`):
```ts
...
```
(Add `import { logActivity } from './_lib/activityLog';` alongside `feedAdmin`'s imports.)

**Optional (deferred) — lazy on-read time-precision flip.** The cron is date-granular (worst case ~1 day latency to the 09:00 UTC run). A lazy flip (publish on first read after the scheduled time) would add minute-precision — but it must NOT touch the hot public GET (`products.ts:96-121`, `:123-139` return the raw row). Ship the cron fold; leave the lazy flip as a future admin-only-read enhancement, so it never adds a write side-effect to the public path.
...
**Phase 2.7 — `api/products.ts`: split validation into create-minimum vs the full publish gate.** Today `validateProductRules` (`:287-322`) is shared by create (`POST :183`) AND publish (`:631`, `:654`) and demands the miniature shape, which BLOCKS the auto-save-as-draft flow (a partial New can't persist). So: create keeps only a DB-satisfiable minimum; publish enforces the full authoritative set (also requiring `slug, quantity, features, materials, care_instructions, shipping_details, dimensions (parseable W·D·H), weight`, and `alt` on every image + media; the six auto-generated fields are NEVER blocking). **CURRENT (`:287-322`, the shared validator):**
```ts
...
```
Retarget the three call sites. **CURRENT — create (`:183`):** `  const problems = validateProductRules(product as Record<string, unknown>);` → **NEW:** `  const problems = validateCreateShape(product as Record<string, unknown>);`. **CURRENT — edit-publish (`:631`):** `    const shapeProblems = validateProductRules(merged);` → **NEW:** `    const shapeProblems = validatePublishRules(merged);`. **CURRENT — first-publish (`:654`):** `  const shapeProblems = validateProductRules(row as Record<string, unknown>);` → **NEW:** `  const shapeProblems = validatePublishRules(row as Record<string, unknown>);`. *(`PRODUCT_TYPE_RULES` `:281-286` + `TypeRules` `:280` stay — `validatePublishRules` still reads `minHero`/`minGallery`/`requireThumbnail`; `required` on the rule object is now unused by the publish path but harmless.)* **Also fix the now-stale comment at `:651`** (first-publish block): `validateProductRules now runs on BOTH publish branches` → `validatePublishRules now runs on BOTH publish branches`.

The expanded `validatePublishRules` runs on **edit-publish (`:631`)** and **first-publish (`:654`)**, so it also gates **republish** and the **Available OFF→ON round-trip** — the strict gate applies to every piece (no grandfather/backfill; the catalog is empty). Forward collection is folded so a maker never hits a confusing publish-400: the GPT schema flags the publish-required fields (WS10 §10.1c), the docs carry the set (§10.6), and the `commitAvail` ON seam surfaces the 400 as a field-list toast. *(Create relaxing to title+price is a GPT-contract change — WS10 §10.2b adds the "createProduct is lenient; the field gate is at PUBLISH" beat. Why the strict gate stands: `_RATIONALE.md §2.7`.)*

**Doc impact:** `EVERLASTINGS_STORE.md` + `GPT_SETUP.md` — document the two-tier validation, the authoritative publish required set, and that `seo_*`/`checkout_*` auto-generate at publish (never block).

**Phase 2.7a — checkout identity is frozen once EVER published; republish never re-stamps.** §2.2 makes Available-OFF unpublish a live piece to draft. But the `FROZEN_AFTER_PUBLISH` freeze lives inside `if (current.is_published)` (`products.ts:390`), so a paused-but-previously-published row (`published_at` set, `is_published=false`) falls to the unpublished-PUT branch and re-opens `checkout_name/description/image` — an edit sticks in the DB while `syncProductToStripe` short-circuits on the existing `stripe_product_id` and never pushes it → a silent site↔Stripe desync. Checkout identity is set once and frozen for the LIFE of the piece; storefront presentation (`title`/`story_card`/`images`/`price`) stays editable, only the Stripe checkout snapshot locks. Two surgical edits:

*(a) — the freeze fires on EVER-published, not just currently-published.* Hoist the `FROZEN_AFTER_PUBLISH` rejection above the `if (current.is_published)` block. **CURRENT (`api/products.ts:385-390`):**
```ts
...
```
**NEW (insert the ever-published freeze guard just above the `if (current.is_published)` block):**
```ts
  const previewToken = randomUUID();
...
*(b) — republish preserves the original `published_at`.* Folded into §2.5's first-publish NEW block — the `published_at: (row.published_at as string) ?? new Date().toISOString()` change there, so a relisted piece keeps its true first-publish date.

*(The one real reason to change a locked checkout name/image — Em repaints a slow seller a new color needing a new title — is a deliberate future "Reissue checkout identity" feature, out of scope here: `_RATIONALE.md §2.7a`.)*

**Doc impact:** `EVERLASTINGS_STORE.md` — checkout identity is frozen for the life of the piece (storefront title/story/images/price stay editable; only the Stripe checkout snapshot locks); a relisted piece keeps its original publish date.

**Phase 2.8 — `api/products.ts` publicView: strip `scheduled_publish_at` from the public projection.** A new internal column — per the `:44-48` boundary note, a new secret/internal column must be added here. **CURRENT (`:49-56`):**
```ts
...
- **No new serverless function, no new cron** — the schedule fold rides the existing `product-feed` GET; the migration adds one column + one index; every other change is inside `api/products.ts`. All scoped by `isTest`. Money stays integer cents. `tsc --noEmit` clean, CommonJS output preserved.
...
The redesigned Orders surface (`out/orders-app.js` + `out/orders.html`) is a pure design mock: it reads `window.PORTAL_DATA.orders` (`orders-app.js:10`) and every "mutation" edits that in-memory array and re-renders. No `fetch` exists in it. This workstream wires each mock/no-op to its real endpoint on `api/orders.ts` and lands three backend reconciliations. **No new serverless function** (all four verbs already live in `api/orders.ts`); `isTest` and `supabase` are already in scope. The portal auth helper is `PORTAL.authHeader()` (WS1 §1.3b) — every data call carries `...P.authHeader()`. Run `npx tsc --noEmit` clean after TS edits.
...
| `#envChip` static "Test" (`orders.html:118`)                       | `GET /api/config` → `{isTest}`                                                                             | show/hide the chip from `isTest`                                                                                                        |

*(After a successful PATCH/POST the design's mutate-then-`render()` may stay as an optimistic update, but a `GET /api/orders` reload is authoritative — the backend owns the status/relist flip.)*

**Doc impact:** none (INTEGRATION.md's "unseen/new — no data source yet" note becomes "poll + `created_at`").

**Phase 3.1 — Refund = PRESERVE the UI, port the behavior (`orders-app.js`).** Do **not** rebuild the refund modal — the design's model (a per-piece **+ Add** summing into a **freely editable** amount + a **separate per-piece Relist switch**) already matches the backend's `{amount_cents, relist_product_ids}` split and the "refund and relist are independent decisions" contract. `POST /api/orders/:id/refund` is **unchanged**. The wiring closes four gaps:

- **a. Load siblings by PaymentIntent.** `openRefund(pi)` must fetch the cart's full sibling set (a multi-piece cart can straddle needs/shipped subtabs), as `/admin` does (`admin.js:1016-1022`): `GET /api/orders?payment_intent=<pi>` → filter `o.status !== 'refunded'` → render as the modal's `rpiece` rows. *(The contract the backend added for this — `orders.ts:79-80`.)*
- **b. Pre-fill the single-item case.** When the purchase has **exactly one** non-refunded piece, pre-toggle its **+ Add** so `refundAmount` pre-fills (matching `admin.js:1004`); multi-piece stays owner-selected.
- **c. Replace the local status/quantity mutation with the POST.** `doRefund` currently flips pieces + bumps `quantity` locally (`:222-223`) — the mock guessing at semantics the backend owns. Wired: `POST /api/orders/:id/refund` with `amount_cents = refundCents()` (`:210`) and `relist_product_ids` = the **Relist switches** (`[data-rrelist]:checked`, `:220`) — **not** the +Add toggles (those only build the amount). The backend flips + relists exactly the returned pieces (`orders.ts:293-341`); a goodwill/partial amount with no relist flips nothing. Then reload the list.
- **d. Owner-confirm + error surfacing.** Refund is owner-confirmed, never auto-issued — the design's modal + red destructive **Refund** button (`orders.html:140`, `btn--danger`) satisfies this. Surface `409` ("already refunded" / "no payment") and `502` ("Stripe refund failed — check the amount, then the Stripe dashboard") **inline**, not as a dead screen.
...
**Doc impact:** none (backend refund behavior unchanged).

**Phase 3.2 — `api/orders.ts` PATCH: add a refunded 409 guard (byte-anchored).** The mark-shipped handler goes straight from id-validation to the `orders` UPDATE + tracking email (`orders.ts:155-209`) with **no status check** — it will ship a refunded order and email the buyer. Add the same 409 guard the refund handler carries (`orders.ts:280-282`), scoped by `isTest`. **CURRENT (`api/orders.ts:111-115`):**
```ts
...
```ts
  ...
  let body: PatchBody;
```
*(The pre-check's `.eq('is_test', isTest)` also closes a cross-env hole: the UPDATE at `:155-167` scoped only by `id`, so a valid id from the other env would mark-ship + email; the guard now 404s it first. `supabase` and `isTest` are already in scope — no new import.)*

**Doc impact:** `STORE_ADMINISTRATION.md` mark-shipped note — "a refunded order can't be marked shipped (409)"; the test-script mark-shipped case gains the refunded→409 expectation.

**Phase 3.3 — `shipping_address` read: make it source-resilient (byte-anchored).** The design reads a top-level `order.shipping_address` (`orders-app.js:36`); the API also returns it under the `customers` embed (`orders.ts:66`). Both exist, so this is a defensive fallback honoring the embed as the contract. **CURRENT (`orders-app.js:36`, inside `groups()`):**
```js
...
```
The address-render at `orders-app.js:99-100` needs no change and **no `.name` handling** — the address JSON carries no name in either source; the buyer name is the sibling column `customers.name`, already rendered at `:104`/`:106` via `g.customer.name`.

*The `orders` table has its own top-level `shipping_address jsonb` column (`initial_schema.sql:109`), written per row by the webhook (`webhook.ts:195`); `GET`'s `select('*, …')` (`orders.ts:65`) returns it, and the `customers` embed (`orders.ts:66`) carries a duplicate. Neither carries a `name` — the value is `{line1,line2,city,state,postal_code,country}`; the buyer name is `customers.name` (`webhook.ts:128`).* **Byte-anchor for the seam contract — CURRENT (`orders.ts:66`):**
```ts
...
**Phase 3.4 — Drop `delivered` from the UI enum (verified: no writer).** `delivered` is enumerated in the schema comment with a `delivered_at` column marked *"post-launch, via Shippo webhook"* (`initial_schema.sql:108`, `:116`) — but **no code writes `status='delivered'`** (`webhook.ts` writes only `'completed'`/`'refunded'`; `orders.ts` PATCH writes `'shipped'`, refund writes `'refunded'`; no Shippo integration exists). **Prune the dead `delivered` UI now** — it renders a state the data never reaches. UI-only; the schema's `delivered`/`delivered_at` stay for the post-launch Shippo webhook, and the pill is re-added when that writer lands.
...
The foundation already exists: `handleCoupon` / `handleCouponList` / `handleCouponDeactivate` in `api/products.ts` create, list, and end owner coupons tagged `metadata.source='owner_sale'`, and `checkout.js` owns `checkout.applyPromotionCode` (`wirePromo`, `checkout.js:144-167`). This workstream is the **DISPLAY + AUTO-APPLY** layer: one owner `%` coupon becomes the store-wide sale, the storefront reads it publicly, auto-applies it at checkout, and paints struck pricing everywhere a price renders.

*The locked shape: the store-wide sale is ONE owner coupon — PERCENTAGE only for on-site struck pricing + auto-apply — tagged `metadata.source='owner_sale'` + `metadata.auto_apply='true'` + a known promo code. A `$`-off "store-wide" sale stays a plain owner coupon with a code (no `auto_apply`, no struck pricing). We do NOT touch the server `discounts` param — `allow_promotion_codes:true` (`checkout.ts:105`) stays; auto-apply happens client-side via `applyPromotionCode`.*

### Phase 4.0 — the #219 probe (GATING — do this FIRST on the dev preview)

Before auto-apply is locked into `checkout.js`, **probe on the dev preview** whether Stripe.js `checkout.applyPromotionCode(code)` fires reliably at Custom Checkout session **INIT** (immediately after mount, before any user interaction). The loaded Basil bundle's real surface has repeatedly diverged from the published docs (see `.agent/STRIPE_CUSTOM_CHECKOUT_GUIDE.md` — probe-first, never bridge over mounted elements), so this is a known landmine.

**How to probe (dev preview URL, never localhost):** create a live test `%` sale via the coupon endpoint, open `/checkout` with an item in cart, and inspect the stashed object:
```
...
```
Record FOUR answers: (1) does `applyPromotionCode` succeed at init (before the shopper touches a field), (2) does the `change` listener then see the discounted `session.total.total.amount`, (3) does a `removePromotionCode` (or equivalent) exist so a shopper can strip the sale code and enter a personal one, **(4) does a SECOND `applyPromotionCode(OTHER_TEST_CODE)` after the sale succeeds REPLACE (new code wins), STACK-AND-ERROR (call rejects), or STACK-AND-BOTH (both apply — a Stripe contract violation).** Answer (4) determines the `wirePromo` Apply-handler branch: on REPLACE it stays a single `applyPromotionCode(newCode)`; on STACK-AND-ERROR it must `removePromotionCode()` first (if answer 3 says one exists), then `applyPromotionCode(newCode)`; on STACK-AND-BOTH stop and surface — outside Stripe's one-discount-per-order contract.
...
- **Init is flaky →** apply on the first `checkout.on('change')` tick instead (defer one event loop), still invisible.
- **Apply-at-init rejected entirely →** *prefill* the promo field (`#promo-code`) with the sale code + a visible one-tap "Apply sale" — degrades to one tap, never blocks purchase.
- **No `removePromotionCode` on the bundle →** the keyword field stays the removal path IF answer (4) = REPLACE. If answer (4) = STACK-AND-ERROR, `wirePromo`'s Apply handler MUST attempt a `removePromotionCode()` first — if neither exists, fall back to the prefilled field + a visible "Apply sale" button.

**STACK-AND-ERROR concrete `wirePromo` skeleton** (if answer (4) = STACK-AND-ERROR; locate-and-apply on the existing `.addEventListener('click', ...)`):
```js
...
```
On REPLACE, skip the remove (a single `applyPromotionCode(newCode)` swaps the promotion). On STACK-AND-BOTH, do NOT auto-apply — surface + stop.
...
*4.1.c — after the new promo is created, supersede any PRIOR active auto-sale.* Only ONE active auto-apply store-wide `%` sale exists at a time; "set a new one" ends the prior automatically. Create-new-first, then deactivate-old (a failure never leaves zero sales). **CURRENT (`api/products.ts:745-746`):**
```ts
...
```ts
    ...
    return jsonResponse(request, { success: true, code: promo.code, coupon_id: coupon.id, promotion_code_id: promo.id, expires_display: typeof body.expires_at === 'number' ? formatExpiry(body.expires_at) : null });
```
*(`handleCouponList` already keys isolation on `metadata.source === 'owner_sale'`, so the `auto_apply` coupon lists + deactivates through the existing owner paths. Ending the store-wide sale is just `coupon_deactivate` on that promo.)*

*4.1.d — surface `auto_apply` on the coupon-list output.* `handleCouponList`'s per-entry return (`products.ts:816-826`) already carries `percent_off` + `store_wide`; add one field — `auto_apply: pc.coupon?.metadata?.auto_apply === 'true'` — so either surface can pick the store-wide sale without guessing. **End-sale heuristic (both surfaces):** the store-wide sale is the single active owner coupon that is `store_wide:true` **and** `percent_off != null` **and** `auto_apply:true` — supersede (4.1.c) guarantees exactly one, so "end the sale" = `coupon_deactivate` on THAT promo. No new GPT Action, no GPT-instruction `.txt` bytes (Actions relay the raw JSON, so the GPT already receives `auto_apply` regardless of the response schema); the deactivate op already exists on both surfaces.

**Doc impact:** `EVERLASTINGS_STORE.md` — the store-wide sale is ONE owner `%` coupon tagged `metadata.auto_apply='true'`; creating a new one supersedes the prior; only one active at a time.
...
*4.2.b — the handler, folded into the SAME module.* Append after `handleCouponList`. **CURRENT (`api/products.ts:831-836`, the tail of `handleCouponList`):**
```ts
...
```
*(The `%`-off + `auto_apply='true'` guard means a $-off store-wide coupon never reports here — it stays a plain shareable code.)*

**Doc impact:** `EVERLASTINGS_STORE.md` — `GET /api/products?_action=active_sale` is a public (no-auth) read returning `{active,type:'percent',value,code,amount_display}` or `{active:false}`, edge-cached.

### Phase 4.3 — `assets/js/main.js`: shared active-sale fetch + struck-price helper (loaded on every page).

*`main.js` is the one script present on all 14 storefront pages and already owns the shared price/config helpers — the sale plumbing lives here so no per-page HTML edit is needed.*
...
*4.3.c — the chrome injector (top bar + popup), placed just above the `DOMContentLoaded` handler at line 269:*
```js
...
*4.3.d — struck-price CSS (append to `assets/css/styles.css`, storefront tokens only):*
```css
/* ---------------------------------------------------------------------------
...
.sale-pop {
  position: fixed;
  top: calc(var(--header-offset, var(--header-height)) + var(--space-md));   /* v3.7.3 D#1: the REAL header bottom — the sale-bar pushes the sticky header down at unscrolled load */
  right: var(--space-md);
...
*4.3.e — the mobile-nav drawer must clear the real header too.* The pre-existing `.mobile-nav` drawer hardcodes `inset: var(--header-height) 0 0 0` — pushed-header-agnostic. Because the utility bar is ALWAYS present, the drawer overlaps the header's lower edge on every page. Re-point it at the same `--header-offset` the sale-chrome JS sets (fallback `--header-height` if the JS hasn't run). **CURRENT (`assets/css/styles.css:311`):**
```css
  inset: var(--header-height) 0 0 0;
```
**NEW:**
```css
  inset: var(--header-offset, var(--header-height)) 0 0 0;
```

**Doc impact:** `EVERLASTINGS_STORE.md` — a top utility bar + once-only (`localStorage everlastings.saleSeen`, code-scoped) sale popup render on every storefront page from `main.js`; struck pricing uses `.price-sale`.
...
Percent-only; each site reads `window._activeSale` (populated by `getActiveSale`, awaited before first render). Sold items always render plain.
...
*4.5.b — `assets/js/shop.js`: struck card price. **POINTER — do NOT apply as a standalone edit.*** WS6 §6.5a rewrites this entire shop-card block for the quantity-based `sold` state, which subsumes the price line. Apply the struck price **inside §6.5a's merged NEW block** (order WS6→WS4→WS9): the `card__price` line reads `${sold ? formatPrice(p.price) : priceHTML(p.price, window._activeSale)}` — struck gated on `!sold`. Applying separately against the pristine `shop.js:139` anchor would be reverted by §6.5a or break its anchor.
...
```
*(The struck gate is `!sold` (quantity-based), matching the storefront buy-gate (§6.5b) and DESIGN §D.1. This `const sold` is block-scoped inside `if (priceEl)`, so it does not collide with §6.5b's `const sold` at `product.js:382`. Do NOT touch `injectProductJsonLd` (`product.js:338-363`) — the JSON-LD `offers.price` stays the true unit price; a sale is a promotion, not a price change.)*
...
```
*(`getCart` and `getCartTotal` are **`main.js` globals** — `main.js:104` / `:167` — and `main.js` loads on every storefront page ahead of `cart.js`/`checkout.js`, so both are in scope. `getCart()` re-reads the current cart after the async sale resolves.)*
...
```
*(Verified: `cart.html:170` carries `[data-cart-subtotal]` and `:178` carries `[data-cart-estimate]`.)*
...
Each mock/no-op call in the prototype (`out/sales-app.js`) wires to an existing endpoint — **no new `api/*.ts` function**. The `%` store-wide "Start sale" is the only call that sends `auto_apply:true`.

- **List coupons** — `renderCoupons()` reads `D.coupons` (`sales-app.js:9`) → **`GET /api/products?_action=coupon`** → `{coupons[], truncated?}` (auth'd). Render with the `*_display` fields.
- **Render store-wide banner** — `renderStoreWide()` reads `D.storeWideSale` (`sales-app.js:11,28`) → **`GET /api/products?_action=active_sale`** → `{active,type:'percent',value,code,amount_display}` or `{active:false}`. *(Only `%` is auto/struck — a `$` store-wide is created as a plain coupon below.)*
- **Start store-wide sale (`%`)** — `#startStoreWide` onclick (`sales-app.js:49-54`) → **`POST /api/products?_action=coupon`** with `{type:'percent', value, auto_apply:true, code:'<code>'}`. A promo code is required for auto-apply + the share link but need **not** be human-chosen — `active_sale` / struck / popup all read the code **back** from Stripe, so a Stripe-auto-generated code works. Send whatever `sales-app` collects (typed OR generated); do not invent a code-input requirement. Server supersedes any prior auto-sale (4.1.c).
...
- **End store-wide sale** — `#endStoreWide` → `confirmDialog` (`sales-app.js:37`) → **`POST /api/products?_action=coupon_deactivate`** with `{promotion_code_id}` (or `{code}`) of the active auto-sale — identify it as the single active `store_wide` + `percent_off` + `auto_apply` owner coupon from the coupon list (§4.1.d; the GPT uses the same heuristic).
- **Create coupon** — `createSale()` (`sales-app.js:177-206`) → **`POST /api/products?_action=coupon`** with `{type, value, code?, product_ids?, min_amount?, expires_date?:'YYYY-MM-DD', max_redemptions?}`. *(`product_ids` are **`stripe_product_id`**, not Supabase ids — map published products first.)* `409` = code exists; `400` = bad type/value. **The New-Sale scope picker must filter its piece list additively to `!p.archived_at && p.is_published`** (NOT a replacement for `!archived_at`: an archived piece can keep `is_published:true` since `handleArchive` only sets `archived_at`, `products.ts:906-911`) — a draft/unpublished piece has no `stripe_product_id`, so scoping a coupon to it can't map → a silent create-coupon failure; filtering the picker prevents it.
...
- **Copy share link** — `[data-share]` (`sales-app.js:82-86`) → client-only: `((D.config && D.config.siteUrl) || 'https://everlastingsbyemaline.com') + '/?code=' + encodeURIComponent(code)`, where `code` is the coupon's own `code` field. Already correct; no endpoint. The storefront **honors** that `?code=` param — **Phase 4.7** adds the `checkout.js` reader.
...
**Doc impact:** none (the seam maps the design surface onto endpoints defined in 4.1–4.2 and pre-existing coupon handlers).

### Phase 4.7 — `assets/js/checkout.js`: honor a `?code=` share link (prefill + apply, same path).

The coupon "Copy share link" (§4.6) produces `<siteUrl>/?code=CODE` — the **site root**, so the shopper lands on the homepage, not `/checkout`. So the code must be **captured site-wide and persisted**: `main.js` (loaded on every page) reads `?code=` on load and stashes it in `sessionStorage` (§4.7.0); the checkout reader consumes the stash (with `location.search` as a fallback for a direct `/checkout?code=` hit), prefills `#promo-code`, and applies via the **same** `applyPromotionCode` path `autoApplyStoreWideSale` uses. An explicit `?code=` runs **instead of** the store-wide auto-apply (mutually exclusive — Stripe allows one discount/order). Fails soft.
...
**Files (WS4):** touches `assets/js/main.js` (§4.7.0 capture) + `assets/js/checkout.js` (the reader). **Doc impact:** none (`EVERLASTINGS_STORE.md` may note the storefront honors a `?code=` share link — captured site-wide, applied at checkout).

---

## WS5 — Media upload rebuild

> **Byte-anchored where backend; the modal UI is spec'd in the DESIGN addendum (`out/` is the markup source — do NOT rebuild the modal markup/UX here).** This workstream is the most intricate item and gets its own review pass. Headline: **`api/upload.ts` needs no change and no new `api/*.ts` function** — every capability the FEEDBACK §8 flow asks for (batch, drag-drop, by-link, YouTube, video) is already served. WS5 is a **new client** (the media modal in `out/products-app.js`) fanning out to those endpoints, plus a handful of **data-shape reconciliations** in `applyMedia`/`openMedia` so what it persists matches what the storefront reads. Line numbers are hints; the quoted text is the anchor. Run `npx tsc --noEmit` clean after any TS touch.

**Phase 5.1 — `api/upload.ts`: confirm the new flow is already supported (no change).**

**a. Batch + drag-drop = client-side fan-out of single-file multipart POSTs — no batch endpoint.** The `POST` multipart branch reads exactly one file per request. **CURRENT (`api/upload.ts:375-385`):**
```ts
    ...
    role = roleField.trim();
```
*(The only batch path is `handleAttachedRefs` — the GPT chat-attach JSON path, images-only, ≤10, not a browser multipart batch. The modal must NOT reuse it.)* The modal drops N files → issues **N single-file `POST /api/upload` multipart requests** (mirror `wireUploadZone`, `admin.js:500-554`), showing per-item progress (`out/products-app.js:676` `it._uploading`) per request.

**b. URL paste (direct `.mp4`/`.jpg` or Drive/Dropbox share) = the existing JSON by-link branch — no change.** **CURRENT (`api/upload.ts:329-341`):**
```ts
...
```
Drive share URLs are already rewritten (`normalizeMediaUrl`, `:81`). The modal's URL row fires `POST /api/upload` **JSON** `{url, slug, role, skip_transform?}`. (Dropbox is not rewritten — a `?dl=0` share returns HTML and hits `ALLOWED_MIME` reject; surface that error as-is. <!-- NEEDS-VERIFY: confirm Em's Dropbox links are direct-download (?dl=1) or drop Dropbox from the placeholder copy -->)

**c. Video (dropped, picked, or by-link) is already accepted — images transform, video/gif skip.** **CURRENT (`api/upload.ts:136-139`):**
```ts
  const skipTransform =
...
```
`ALLOWED_MIME` (`:34-41`) already lists `video/mp4` + `video/webm` (50 MB ceiling at `:131`). The modal sends `skip_transform: 'true'` on a **video** multipart POST (parity with `admin.js:526`). Accepted video = **MP4 + WebM** only; the modal's copy should say so.

**d. All role tokens the modal needs already exist in the enum.** **CURRENT (`api/upload.ts:52-53`):**
```ts
const ROLE_PATTERN =
  /^(hero|thumbnail|gallery-(0[1-9]|1[0-5])|video-0[1-5]|detail-0[1-5]|gif-0[1-5]|checkout_image|seo_thumbnail)$/;
```
The modal's role checkboxes map to these (Phase 5.5 table): `hero`→`hero`, `gallery`→`gallery-01..15`, `share`→`seo_thumbnail`, `checkout`→`checkout_image`, video→`video-01..05`. **Gallery caps at 15**, video at 5 — the numberer must respect that (reuse `nextNumberedRole`, `admin.js:490-498`). Poster needs no role token (Phase 5.4d).
...
**a. YouTube never touches `/api/upload`.** A YouTube URL is held in the modal's in-memory `mItems` and written into `media[]` on Apply, then persisted by `PUT`/`POST /api/products`. The modal already classifies it (`detectKind`, `out/products-app.js:601-606`) and validates the host (`addUrl`, `:708`).

**b. The `media[]` shape is fixed by what the storefront reads.** **CURRENT (`assets/js/product.js:263-275` — the YouTube branch, already shipped):**
```js
...
```
A YouTube item stores as `{ type: 'youtube', url, alt }` — the render path builds a youtube-nocookie iframe (no raw embed HTML stored → GPT-safe). **No storefront change is needed** — WS5 only has to make the modal *store* it.

**c. Video-item field names must match — the prototype `applyMedia` has two shape mismatches.** The storefront reads `m.muted` and `m.poster`; the prototype emits neither (and carries `mute`, the wrong key). **CURRENT (`out/products-app.js:761`):**
```js
...
```
**NEW (emit the keys `product.js:236-260` reads: rename `mute`→`muted`, add `poster`; YouTube stays `{type,url,alt}`):**
```js
...
```
*(`populateMedia` forces `muted:true` for autoplay clips at `product.js:252-254`; the explicit `muted` above only bites for click-to-play clips. This matches the LOCKED "Loop / Mute / Hide controls / Autoplay" spec: `controls:false` = "hide controls".)*

**Doc impact:** `EVERLASTINGS_STORE.md` media contract — pin the four video flags + `poster` + `alt`, and that `media[]` order is "MP4s first, YouTube last."

**Phase 5.3 — Persisted image order: `PUT /api/products` writing the reordered `images` array (confirmed, no backend change).**

**a. `images` (and `media`) are already draftable — reorder persists through the existing PUT.** **CURRENT (`api/products.ts:324-330`):**
```ts
const DRAFTABLE = [
...
```
On a **published** product a changed `images` array stages into `draft.images`; on an **unpublished** draft it writes straight to the `images` column. Reorder mechanism: modal reorders `p.images` → `PUT /api/products?id=` `{images: [...]}`. **No `products.ts` change.**

**b. Array order — not the `gallery-NN` suffix — drives storefront gallery order.** **CURRENT (`assets/js/product.js:415-417`):**
```js
...
```
`.filter().map()` preserves array order, so reordering the `images` array reorders the on-page gallery. The numeric suffix is a **role-uniqueness token** (prevents R2 key collisions), **not** a display-order key — reorder does **not** rename files or re-upload. The prototype already reorders in memory (`wireGalleryDrag`, `out/products-app.js:519-523`); integration replaces the no-op `autosave` (`:557`) with the `PUT`.

**c. `openMedia` must derive roles from the FILENAME, not array index.** **CURRENT (`assets/js/product.js:575-576` — `pickHero`):**
```js
  const imgs = Array.isArray(p.images) ? p.images : [];
  return imgs.find((i) => /\/(?:test_)?hero-/.test(i.url || '')) || imgs[0] || null;
```
But the modal seeds roles by array position. **CURRENT (`out/products-app.js:592`):**
```js
    (p.images || []).forEach((im, i) => mItems.push({ kind: "image", url: im.url, alt: im.alt || "", roles: new Set(i === 0 ? ["hero"] : ["gallery"]) }));
```
**NEW (role from the `{role}-{slug}` filename, mirroring `pickHero`/the gallery filter):**
```js
    (p.images || []).forEach((im) => {
      const u = im.url || "";
      const roles = new Set();
...
**a. Alt-required is a client publish-gate (matches the existing bar; no server change).** **CURRENT (`out/products-app.js:748-749`):**
```js
...
```
`alt` is optional in the schema but alt-presence IS a **hard SERVER publish gate**: WS2 Phase 2.7's `validatePublishRules` checks `alt` on every image + every `media[]` entry. WS5's modal alt gate is the friendly client pre-check on top — WS5 needs no `products.ts` edit for alt.

**b. Role logic (one hero; hero≠gallery; share/checkout/poster combine freely) is client logic + a structural server guarantee.** **CURRENT (`out/products-app.js:726-737` — keep as-is at integration):**
```js
...
```
"One hero" is **also** guaranteed server-side by the filename scheme: role `hero` always writes the same key `{hero}-{slug}` (`upload.ts:215-231`), so a second hero overwrites the first. The coverage counter + "hero reused for share/checkout" fallback (`:743`, mirrored by `pickHero`'s `|| imgs[0]`) are client-only display.

**c. Role REASSIGNMENT of an already-uploaded image requires a re-upload under the new role filename (via the by-link path — no new endpoint).** Because the storefront reads role from the filename, unchecking `gallery` and checking `hero` on an existing `gallery-03-slug.webp` does not make it the hero — the file is still named `gallery-03`. To re-role it, the modal `POST /api/upload` **JSON** `{url: <existing CDN url>, slug, role: 'hero'}` — the server re-fetches the public R2 URL, re-crops to the new role's aspect (`upload.ts:178-182`), and writes `hero-slug.webp`. The exact Apply-diff algorithm is §5.4c.i.

**c.i — Apply-time re-role diff algorithm.** The naive one-pass `mItems → p.images` rewrite fails: a promoted-to-hero image ends up matching BOTH `/hero-/` (via `pickHero`'s `imgs[0]` fallback) AND `/gallery-/` (its unchanged filename), duplicating on the PDP. The diff avoids it:
- **On open** (inside `openMedia`, right after the role-derivation loop from 5.3c): stash the opened role set per item: `mItem.openedRoles = new Set(mItem.roles);`.
- **On Apply** (inside `applyMedia`, before writing `p.images`/`p.media`/`p.thumbnail`/`p.seo_thumbnail`/`p.checkout_image`): compute per-item `added = [...mItem.roles].filter(r => !mItem.openedRoles.has(r))` and `removed = [...mItem.openedRoles].filter(r => !mItem.roles.has(r))`. Only items where `added.length || removed.length` need any write.
- **For each `added` role that renames the R2 key** (`hero`, `gallery`, `seo_thumbnail`, `checkout_image`): `POST /api/upload` JSON `{url: mItem.url, slug, role: <resolvedRole>}`. `gallery` resolves via a ported `nextNumberedRole` scanning the CURRENT `p.images` filenames (copy the helper from `admin.js:490-498`). **Resolve added-gallery roles SEQUENTIALLY, splicing the response URL into `p.images` after EACH POST** so the next `nextNumberedRole()` sees the freshly-taken NN (a parallel resolve would hand out the same NN twice and overwrite an R2 key). Write the response's `url` into `p.images` (or the top-level column for `seo_thumbnail`/`checkout_image`; `checkout_image` obeys 5.4e's ever-published freeze — skip the write on any ever-published piece). `alt` propagates from `mItem.alt` on every write.
- **For each `removed` role**: drop the matching entry from `p.images` (by filename prefix + the original `NN`) OR null the top-level column. Do **not** delete the R2 object (out of scope; a mistaken delete would be data loss).
- **Poster** (per 5.4d, one-per-product): after the loop, set `poster: mItem.url` on every `p.media[]` video that lacks its own poster, from the single `poster`-checked `mItem`. If none checked, leave posters as-is.
- **Zero-role items**: drop the `mItem` from local state; write nothing back.
- **Partial-failure recovery:** the sequential fan-out means a Nth POST can fail after N-1 succeed. On any POST throw or non-2xx: (i) stop the fan-out **but STILL fire the terminal `PUT {images, media, thumbnail, seo_thumbnail, checkout_image}` with the partial (succeeded-so-far) `p.images` before returning** — do NOT abort `applyMedia` outright. Aborting would leave the succeeded R2 uploads in the bucket + local `p.images` but never persisted, so a reload silently ORPHANS them. Persisting the partial state is what makes "no orphan" true. (ii) mark the failed `mItem.errored = true` (render as `.mitem--errored`; **author this in `products.html`'s embedded `<style>` beside `.mitem`'s base at `out/products.html:248`: `.mitem--errored{ border-color:var(--danger); box-shadow:0 0 0 2px var(--danger-bd); }` — both `--danger`/`--danger-bd` are portal.css tokens (`:33`/`:46`), available via the linked `portal.css`**). (iii) toast the failure with the role that failed (`"Couldn't set <hero> — try Apply again."`). (iv) DO NOT clear `mItem.openedRoles` on the failed item so the next Apply retries only the remaining diff (succeeded items now have `openedRoles` = their new state → empty diff on retry → idempotent). Never leaves the maker with a "stuck" modal that requires a reload.

**d. Poster = a URL copied into `media[].poster`, no upload role.** **CURRENT (`assets/js/product.js:241`):**
```js
      if (m.poster) v.poster = m.poster;
```
A "video still poster" is any already-uploaded image URL written into the matching video's `media[].poster` — no new role token, no re-upload. **Poster = one-per-product for v3.5:** the single poster-checked image is written to `media[].poster` on **every MP4 that lacks its own poster**. A per-video poster picker is out of scope for v3.5.

**e. `checkout_image` is FROZEN after first publish — the modal must lock the "checkout" role post-publish.** **CURRENT (`api/products.ts:337-340`):**
```ts
...
```
A published-product `PUT` that changes `checkout_image` returns `400 "Frozen after publish…"`. So on a **published** (ever-published, `published_at != null`) product the modal must (i) show the "checkout" checkbox as locked (reuse the lock-chip pattern, `out/products-app.js:274`) and (ii) **not** include `checkout_image` in the Apply `PUT` unless unchanged. `seo_thumbnail` (share) and `media`/`images`/`thumbnail` are draftable — freely stageable after publish.
...
Everything routes to the two **existing** endpoints; nothing new is added.
...
| Apply (`applyMedia :745`)                         | `PUT /api/products?id=` (or `POST` if new) | `{images, media, seo_thumbnail, thumbnail, thumbnail_alt}`; add `checkout_image` **only if NEVER-published** (`p.published_at == null`; Phase 5.4e) |

**Prerequisites the modal must satisfy (mirrors `admin.js`):** a `slug` must exist before any upload — new products `POST /api/products` first (or derive via `deriveSlug`, `admin.js:511`). **New-product guard:** since uploads need a saved slug, on a brand-new product the modal must prompt/require **title (+ price)** and save first — never silently 400 a media-first attempt. Every call carries the Bearer header (`authHeader`); on a **published** product, `images`/`media`/`seo_thumbnail` **stage** into `draft` and the Apply response returns `preview_url`+`preview_token`, while an **unpublished** product writes through live. Replace the prototype's no-op `autosave` (`:557`) and in-memory `applyMedia` mutations with these calls.

**Doc impact:** `data-flow.md` "Media upload" section already matches this seam; add the re-role-via-by-link mechanic and the poster resolution.

---

## WS6 — Storefront bugs

> **Byte-anchored. Discrete, independent fixes.** Each phase quotes a **CURRENT** block (the locator) + a **NEW** block. Line numbers are hints; the quoted CURRENT text is the anchor. Pure storefront (HTML/CSS/JS): **no backend/API change, no `tsc`.** Storefront brand untouched except these fixes; the `prefers-reduced-motion` layers stay intact.

**Phase 6.1 — product page: render the real spec fields (`product.html` + `product.js`).** The columns `dimensions, weight, materials, power_supply, care_instructions, shipping_details, artist_note` are **fetched** (`main.js` `getProductBySlug`, `:60`) but Track B shipped **no render code** — the static Details `<ul>` shows six hardcoded placeholder bullets regardless of the DB. Add a separate `populateDetails` (leave `populateFeatures` untouched — it only ever touches `p.features`).

*6.1a — replace the placeholder Details list with data-bound containers.* Replace the entire placeholder list — from the `<!-- Features list… -->` comment through its closing `</ul>`. **CURRENT (`product.html:279-280`, the opening boundary):**
```html
...
```
*(The six SVG icons carried no data and were hidden by CSS anyway — `styles.css:930` `.feature-list svg { display: none; }`. `.feature-list` (`styles.css:918`) is a disc-bulleted list; both `<ul>`s reuse it. `.hidden` = `display:none !important`. `data-product-features` keeps its hook so `populateFeatures` is unchanged.)*
...
```
*(`escapeHTML` already exists `:583`. Field types confirmed against the /admin write path: `dimensions`/`weight`/`power_supply`/`artist_note` are strings, `materials`/`care_instructions`/`shipping_details` are arrays; `weight` is in both public selects.)*

**Doc impact:** `EVERLASTINGS_STORE.md` — the product page **now renders** the seven structured fields (Details list + `artist_note` note), only when set.

**Phase 6.2 — `shop.js` / `shop.html`: series filter never matches.** `applyFilters` compares hardcoded kebab checkbox values (`shop.html:186-190`) against the free-text `series` field via `includes(p.series)` → never true. Normalize both sides to one slug **and** derive the options from the live catalog.
...
```
**NEW (populate options first, so `initFromURL` can pre-check a URL-deep-linked box and `wireFilters` binds the fresh inputs):**
```js
...
```
*(`getActiveFilters`/`syncURL`/`initFromURL` all key off `[data-shop-filter="series"]` `value` — now slugs — so they stay consistent end-to-end. `product_type` isn't touched — its one value `miniature` already matches.)*
...
```
**Taxonomy reconcile-at-build:** the filter options derive from live products' `series` (handled by `populateSeriesFilter`), AND the header/footer `?series=` deep-link slugs are **realigned in the nav/footer templates to whatever `seriesSlug()` yields on the live catalog**. Build step: open each template carrying a series deep-link (header nav in every `*.html` + footer nav; `index.html` "browse by series"), query the live catalog's distinct `series` values (or the `bySlug` map), rewrite each `?series=<slug>` link to the matching live slug. A mismatch is a soft failure (shows all, no crash), but the reconcile removes it. *(Why realign nav to catalog, not the reverse: `_RATIONALE.md §6.2`.)*

**Doc impact:** `EVERLASTINGS_STORE.md` — the shop **Series filter is now data-derived**; header/footer `?series=` deep-links pre-check a box only when the target slug matches a live series' slug.

**Phase 6.3 — featured carousel wraps on mobile (`index.html` + `homepage.js`).** `.featured-carousel` is `grid-template-columns: repeat(3, …)` + `overflow-x: auto`, so a 4th tile wraps to a hidden second row instead of scrolling. Fix: independent horizontal-scroll rows — ≤5 items = one row; >5 = rows of ~3, each its own `overflow-x` scroller.
...
```
*(`grid-auto-flow: column` + `grid-auto-columns: minmax(260px, 1fr)` lays each row's tiles in one track: stretch to fill on desktop, clamp to 260px → scroll on mobile. No animation, so `prefers-reduced-motion` is unaffected.)*
...
*6.3c — close the `.featured-row` (closing).* **CURRENT (`index.html:249-253`):**
```html
            </a>
          </article>
...
```
**NEW (identical tile markup, now emitted per-row, with the quantity-based Sold badge + struck price + "One of a kind"):**
```js
function populateFeatured(items) {
  const carousel = document.querySelector('[data-featured-carousel]');
  if (!carousel) return;
...
```
*(`pickThumb`/`escapeAttr`/`formatPrice` unchanged. 6 items → rows of [3,3]; 7 → [3,3,1]; ≤5 → one row. The Sold badge / struck price / "One of a kind" fold WS4 §4.5.f + WS9 §9.2 into this block, same as §6.5a.)*

**Doc impact:** none (homepage visual behavior only).

**Phase 6.4 — `/complete`: stop leaking the raw Stripe session id (`complete.js` + `complete.html`).** `complete.js:36` prints the raw `cs_…` session id as "Order ID", not test-gated, so a `cs_live_…` id is exposed in production. Drop it — the Stripe receipt email carries the customer's reference.
...
```
*(Removing the `[data-complete-order-id]` element makes the dropped `setText` moot; both edits keep them in sync. No other code references `data-complete-order-id`.)*

**Doc impact:** none (the Stripe receipt email carries the order reference).

**Phase 6.5 — storefront sold-state + buy-gate become quantity-based (`shop.js` + `product.js`).** The storefront currently derives "Sold" + un-buyable purely from `p.available`, but the sold policy makes the **storefront buy-gate `published && quantity > 0`**. A sold piece is `quantity ≤ 0`; `available` is only a consequence. To make the enforcers agree (`computeState()` ↔ this storefront gate ↔ `record_sale`) and guarantee a sold piece is **never purchasable regardless of the `available` flag**, treat `p.quantity != null ? p.quantity <= 0 : !p.available` as sold/unbuyable — a known quantity decides, else fall back to `available`. The server checkout/reserve gate (`checkout.ts:79`/`:205`) is deliberately STRICTER (`is_published===true && available===true && quantity>=1`, also rejecting archived), so any display-vs-flag mismatch fails SAFE at the buy step.

**Go-live dependency:** a *pre-v3.3* SOLD row can carry `available:false` with a **non-null stale `quantity ≥ 1`** (old sales didn't decrement) — the quantity-primary rule would render it buyable + struck instead of "Sold/plain" (the null-fallback doesn't catch a non-null stale quantity). It still 410s at the server checkout gate (no *buy* leak) but *displays* wrong (in the storefront AND the portal's `computeState()`). The fix exists — the `20260616000001` cutover backfill zeroes `quantity` on `available:false` rows — but it ships **commented and must be RUN before relying on this gate / enabling any % sale** (sequenced in the TESTING deploy preflight; a go-live checklist item, not a code change here).
...
```
**NEW — the ONE merged shop-card block (compute `sold` once → drives the Sold badge, the image dim, the struck-price gate, and the "One of a kind" badge). Folds WS6 sold-state + WS4 §4.5.b struck price + WS9 §9.2 badge; §4.5.b and §9.2 are pointers into it (apply order WS6→WS4→WS9):**
```js
  grid.innerHTML = visible.map((p) => {
    const heroImg = pickHeroThumb(p);
    const altText = escapeAttr(p.thumbnail_alt || p.title || '');
...
```
*(Merged edits, each with its own home: the `sold` computation + Sold badge / dim / `text-muted` are WS6 §6.5a; the struck price is WS4 §4.5.b gated on `!sold`; the `badge-unique` "One of a kind" is WS9 §9.2 (CSS written by §9.2a). `priceHTML` renders plain when there's no active percent sale. The three badges are mutually exclusive by gate — Sold when `sold`; Featured when `!sold && p.featured`; "One of a kind" when `!sold && !p.featured` — so exactly one renders. Gating Featured on `!sold` also retires the pre-existing Sold+Featured co-occurrence.)*
...
```
*(Availability only — the JSON-LD `offers.price` stays the true undiscounted unit price; do NOT add sale math to `injectProductJsonLd`. `getProductBySlug` already selects `quantity`, so no fetch change.)*
...
```
*(Cosmetic — the related pool is fetched `getProducts({ available: true })`, so a click-through still hits the 6.5b buy-gate; this closes the lag-window on the card badge.)*
...
```
*(Load-bearing: `stripeSync.ts:64-73` creates each Stripe Product with `metadata.supabase_id = product.id`, and `session.metadata.items` carries that same `id`. After the expand, `(li.price.product as Stripe.Product).metadata?.supabase_id` equals the cart item's `id`, and `perItemAmounts[supabaseId] = li.amount_total` keys line up. `tsc` clean — only the runtime value changes.)*
...
```
*(With 7.1a in place, `explicit` is a populated integer-cents value per item, so `amount` = Stripe's authoritative `li.amount_total`. The `fallbackEach`/`fallbackRemainder` split survives only as the degraded path when the `listLineItems` call itself throws.)*

**Doc impact:** correct the `EVERLASTINGS_STORE.md` money-flow note — `orders.amount` is the **real per-line** `amount_total` from Stripe; the even split is now the fetch-failure fallback only.

**Phase 7.2 — Remove the 15-min cart-hold reservation entirely (#224).**

Return to the original no-reservation design. The hold self-locked an abandoning/retrying shopper and mis-fired the apology coupon; without holds, sold-recovery fires only on a *real* sold-out (the availability re-check, which stays). Every `cart_holds` read/write is removed across `api/checkout.ts` + `api/webhook.ts`; the availability check, subscriber upsert, and customer prefill all stay. The `?_action=reserve` route + its `vercel.json` rewrite **stay** — the endpoint keeps serving as the availability/subscriber-capture gate, with no reservation side effect.
...
```ts
    ...
    const { data: products, error: productsError } = await supabase
```
*(`nowIso` was used only by the deleted holds query. The availability re-check at `:77-92` is untouched and still returns `410 { error: 'hold_expired' }` when a piece has since gone unavailable, so `checkout.js:35` still bounces to `/cart`. The `hold_expired` body string is now a slight misnomer meaning "unavailable," but harmless — the frontend branches on the **410 status**, not the string.)*
...
```ts
    const unavailable = items
      ...
      .map((item) => ({ product_id: item.product_id, slug: item.slug }));
```
*(The `409 { error: 'unavailable', unavailable, related }` response + the series/fallback related-product lookup are untouched — a genuinely sold-out piece still triggers the sold-recovery overlay.)*
...
*7.2e — `handleReserve`: drop `expires_at` from the success response.* **CURRENT (`api/checkout.ts:306-313`):**
```ts
    return Response.json(
      {
        ok: true,
...
```
*(`cart.js` never reads `expires_at` — its 200-path branches on `res.ok` and reads only `data.unavailable`/`data.related` on the 409. `isTest` stays imported — still used by the subscriber upsert, prefill, and related queries.)*
...
```ts
    ...
    if (process.env.META_PIXEL_ID && process.env.META_ACCESS_TOKEN) {
```
*(`session.metadata.session_id` is still written by `handleSession` and stored harmlessly — nothing reads it after this removal.)*

*7.2h — `assets/js/cart.js`: KEEP the reserve POST as-is (decision).* **No code change.** Post-#224 the endpoint is a pure **availability check + subscriber capture + prefill**, and its client-facing contract (200 `{ ok }` / 409 `{ unavailable, related }`) is unchanged. Removing the POST would delete the sold-recovery trigger + the checkout-started subscriber capture, so it must stay.

*7.2i — migration: retire the `cart_holds` table (commented per convention).* Create `supabase/migrations/20260701000002_v3_5_drop_cart_holds.sql` (prefix `…000002`: WS2 §2.3's `scheduled_publish` took `20260701000001`, so this sorts after it). Following the project's destructive-op convention, the `DROP` ships **commented** so `supabase db push` records the migration but never auto-drops — Sean runs it by hand after confirming the code is deployed. **NEW file (verbatim):**
```sql
...
*7.2j — reword the customer-facing cart-hold promise.* The storefront still promises the 15-min hold in four places that go **false** once §7.2 ships — remove or reword them to the no-reservation reality (pieces are one of a kind and NOT held; checkout is first-come — complete promptly).
- **`policies.html:167-172`** — **DELETE the entire `<section id="cart-hold">` block.** A truthful `<section id="availability">` already sits directly above it (`:160-165` — heading "Availability & Cart" + the not-reserved / verify-availability-at-checkout / no-artificial-urgency copy), and the FAQ `/policies#availability` deep-link already resolves to *it* — so a rename would create a **duplicate id** and leave the false hold copy live. Just remove the false section; no new copy needed.
- **`policies.html:30`** — meta description: drop "cart holds" (e.g. *"How availability, returns, and care work at Everlastings…"*).
- **`faq.html:194`** — drop the "place a 15-minute soft hold" clause; keep + lead with the already-true *"Adding a piece to your cart does not reserve it — another collector may complete their purchase first. We verify availability when you click Checkout."*; the `/policies#availability` link now resolves.
- **`shipping.html:197`** — drop "and cart-hold" so the label matches reality: e.g. *"Full availability details: see our Policies."*

**Doc impact:** remove `cart_holds` from the `EVERLASTINGS_STORE.md` schema + the checkout-flow description (no reservation; sold-out is caught at the availability re-check); note the `reserve` endpoint is now "availability check + subscriber capture," not a hold. Mark the reservation **SUPERSEDED** everywhere it's still documented as live + add the go-live note: flip **KAD #18 "Availability check BEFORE any PII" / #19 "Soft cart holds"** and the higher-precedence **Architecture-Reference duplicates AR #28/#29** (`:310-319` — AR OUTRANKS KAD on conflict, so leaving them keeps the authoritative copy asserting the hold as live) to SUPERSEDED; drop `cart_holds` from the **API map**; correct the table counts at each literal spot — `Tables (8)` (System Diagram, `:140`) → 7, "all 8 tables" (`:62`) → 7, "all six core tables" (`:1142`, which lists `cart_holds`) → **five** (drop `cart_holds` from that list), "8 tables (was 7) — added `cart_holds`" (`:1150`) → revert to 7; and name the System-Diagram `cart_holds` row (`:148`) + the availability-formula that defines "available for purchase" via `NOT EXISTS (cart_hold …)` (`:1140`). **Go-live note:** the store returns to the **no-reservation** design (#224) — a two-shopper race on a one-of-a-kind piece is an accepted, low-volume trade-off (`record_sale` keeps `quantity ≥ 0`, and a rare true double-pay is recovered by the existing amount-based refund with an empty `relist_product_ids`). **Test suite:** retire `tests/integration/08_hold_expiry.sh` (it POSTs a `cart_holds` row and asserts `410 hold_expired` — a false-fail once §7.2a removes that check) and drop its entry from `tests/integration/run-all.sh`; spot-check `06_checkout.sh` and `10_full_purchase_flow.sh` (both mention a hold) at test-run.

**Phase 7.3 — Fold order↔Stripe reconciliation into the existing daily `product-feed` cron (#227).**

No new function, no new cron: the daily cron already hits `/api/product-feed` at `0 9 * * *` and its Supabase read doubles as the DB keep-alive. Add a second job to the same GET handler that compares **Stripe's completed-and-paid checkout sessions** for the day against the `orders` table and **emails on any paid-session-without-order gap** (a webhook that never landed). It reads Stripe's authoritative completed-session ledger, so it inherently never alerts on raw signature-400s. Gated to the **authenticated cron hit** via `CRON_SECRET`.

*Constraint:* `api/product-feed.ts` uses the **publishable** key, but `orders` is `authenticated`-read only. So reconciliation needs the shared **`feedAdmin`** service-role client WS2 §2.6 already declared. Reuse it; do NOT declare a second client.

*7.3a — add the stripe/email imports (reuse `feedAdmin`). Re-anchor against the post-§2.6 tree.* **CURRENT (the import block as §2.6 left it):**
```ts
import { createClient } from '@supabase/supabase-js';
import { corsHeaders, preflight } from './_lib/cors';
import { isTest } from './_lib/env';
```
**NEW (append stripe + email imports; `feedAdmin` from §2.6 is reused as-is):**
```ts
import { createClient } from '@supabase/supabase-js';
import { corsHeaders, preflight } from './_lib/cors';
import { isTest } from './_lib/env';
...
*7.3c — the ONE shared cron gate at the top of `GET` (both jobs: WS2's scheduled-publish + this reconciliation).* This is the single `GET` edit for the merged file — WS2 §2.6 deliberately did NOT edit `GET`. Both jobs are gated to the authenticated cron hit; publish runs FIRST so freshly-published rows appear in the same feed run; each job is independently wrapped so one failing never blocks the other or the feed. **CURRENT (`api/product-feed.ts:18-19`, as §2.6 left it):**
```ts
...
export async function GET(req: Request) {
  ...
  const { data: products, error } = await supabase
```
*(No `vercel.json` change — the cron already targets `/api/product-feed`. `stripe.checkout.sessions.list` params are stable Basil params; `limit: 100` covers this store's daily volume — a `has_more` day would under-report, acceptable for v1 and grep-able in logs. `tsc` clean. Reconciliation alerts go to the existing `ORDER_NOTIFY_EMAIL`; `RECONCILE_ALERT_EMAIL` is an optional override.)*

**Doc impact:** add the reconciliation job to `EVERLASTINGS_STORE.md` (daily cron now does feed **+** order↔Stripe reconciliation, with the Stripe-resend replay path); document the new `CRON_SECRET` (required to arm it) and optional `RECONCILE_ALERT_EMAIL` env vars. **Go-live handoff — REQUIRED build output:** `CRON_SECRET` is a **net-new production dependency this build introduces** (`isCronRequest` exists nowhere in `api/` today) and it gates **BOTH** the §2.6 scheduled-publish flip **and** this §7.3 reconciliation. It must be set in the **Production** Vercel scope at cutover, or both jobs are silently inert in prod — a maker schedules a piece for Friday and it never publishes, with no error, no log, no activity row. Add **"set `CRON_SECRET` in the Production scope"** to the go-live checklist in `EVERLASTINGS_STORE.md` and hand it to the ROADMAP-A runbook.

**WS7 net surface:** `api/webhook.ts` (7.1a expand + 7.2g cleanup removal), `api/checkout.ts` (7.2a–7.2f), `assets/js/cart.js` (7.2h — no change, decision recorded), the storefront copy edits (7.2j — `policies.html` delete + meta, `faq.html`, `shipping.html`), one new commented migration (7.2i), `api/product-feed.ts` (7.3a–7.3c). No new `api/*.ts` function, no new cron, no `vercel.json` change. New env: `CRON_SECRET` (required to arm 7.3), `RECONCILE_ALERT_EMAIL` (optional).

---

## WS8 — Activity log + seen/unseen order tracking

> **Byte-anchored (migration + write-helper + call-site inserts + read) + integration-seam.** Each phase quotes a **CURRENT** block + a **NEW** block. Line numbers are hints; the quoted CURRENT text is the anchor. Run `npx tsc --noEmit` clean after the TS edits (all `api/*.ts` stay CommonJS-compilable — no new function). Scope everything by `isTest`; no hard delete; single-admin.

**Design premise.** The Account surface renders an **Activity log** card from `window.PORTAL_DATA.activityLog` (`out/account-app.js:53,82-86`) — a UI stub with no backend. Two cross-surface **blink** signals (`data-alert`) also have no data source: the Orders-nav item and the Sold product tab. 8.1 gives the log a real audit trail; 8.2 gives the Orders blink a `last_viewed` data source and **removes** the Sold-tab blink (Sold means only "pieces that have sold"; Orders carries its own signal).

**Phase 8.1a — migration: the `activity_log` table.** New `supabase/migrations/20260702000001_v3_5_activity_log.sql` (apply via the Supabase CLI). Renumber the timestamp prefix to stay monotonic if a later migration exists. *(`entity_id` is `text`, not `uuid`: it holds a product/order uuid AND a Stripe promo id like `promo_1S…`. RLS is service-role-only, mirroring `webhook_events`; `is_test` isolates env.)*
```sql
...
**Doc impact:** none (internal helper; actor = the admin email via JWT, `'gpt'` via `PRODUCT_API_KEY`).

**Phase 8.1c — `api/products.ts`: instrument the mutating call sites.** Import the helper, then insert one awaited `logActivity(...)` immediately before each **success** return (skip the no-op returns). Actions use the `product.*` / `sale.*` prefixes the dot color keys off.
...
**(g) create sale — `handleCoupon`.** **⚠️ Re-anchor:** apply order is WS2→WS4→WS8 — WS4 §4.1.c inserts its supersede-prior block between the `const promo = …create(promoParams)` line and this `return`, so the pre-WS4 `:745-746` pair is no longer contiguous once §4.1.c lands. Anchor ONLY on the unique `return jsonResponse(...)` line and insert the log immediately before it. **CURRENT (the `handleCoupon` success return — anchor on this line alone):**
```ts
    return jsonResponse(request, { success: true, code: promo.code, coupon_id: coupon.id, promotion_code_id: promo.id, expires_display: typeof body.expires_at === 'number' ? formatExpiry(body.expires_at) : null });
```
**NEW:** *(`body.value` is a percent for `type:'percent'` and cents for `type:'amount'`.)*
```ts
    await logActivity({
      actor: await resolveActor(request),
      action: 'sale.create',
...
**(i) archive / unarchive — `handleArchive`.** *(The update `.select().single()` returns the full row, so `data.title` is present.)* **CURRENT (`api/products.ts:915-916`):**
```ts
  }
...
**Phase 8.1d — `api/products.ts` GET: fold in the activity READ (no new function).** A `?_action=activity` branch in the existing GET dispatch, admin/GPT-only, mirroring `handleCouponList`. Returns the exact `{ at, actor, action, summary }` shape `window.PORTAL_DATA.activityLog` expects.
...
**Phase 8.1e — `api/orders.ts`: instrument mark-shipped + refund.** `requireAdmin` already returns the user, so the actor is computed once per handler.
...
```
Then log once after the status flip. **CURRENT (`api/orders.ts:174-176`):**
```ts
...
**(c) refund (`POST`) — log before the final return.** *(The `actor` const for refund is added in Phase 8.2a's POST refactor — do that block first, or add `const actor = 'user' in auth ? (auth.user.email ?? auth.user.id) : 'gpt';` after `:256` if building 8.1e alone.)* **CURRENT (`api/orders.ts:338-341`):**
```ts
...
**Phase 8.2a — `api/orders.ts`: `last_viewed` seen/unseen (no migration — `site_config` already exists).** `site_config` (`(key text unique, value jsonb)`) is the simplest single-admin mechanism: one per-env row holds the last time Orders was viewed. It has **no `is_test` column**, so env lives in the **key**. GET returns `unseen_count` + `last_viewed`; a new `POST ?_action=seen` stamps `now()`.
...
```
**NEW:** *(`supabase` here is the service-role client from `requireAdmin`. The unseen count is filter-independent — a global nudge, not the current subtab's slice.)*
```ts
  const { data, error } = await query;
  if (error) {
    console.error('Orders GET failed:', error.message);
...
```
**NEW:** *(One dispatch, same shape as `products.ts`'s `_action` fork. `site_config.key` is UNIQUE, so `upsert(onConflict:'key')` is the whole write. The `actor` const also serves the 8.1e refund log.)*
```ts
  const { supabase } = auth;
  const actor = 'user' in auth ? (auth.user.email ?? auth.user.id) : 'gpt';
...
**Doc impact:** `EVERLASTINGS_STORE.md` — add the `site_config.orders_last_viewed_{env}` key + the unseen-order signal.

**Phase 8.2b — `out/products-app.js`: remove the Sold-tab blink.** The Sold tab means only "pieces that have sold"; Orders carries its own signal (8.2a). *(Anchored against `out/products-app.js`; if integration has already relocated this JS into `/admin`, apply the same byte edits there. `unseenOrders` is referenced ONLY at `:13` and `:98`, so removing both leaves no dangling reference.)*
...
**Doc impact:** note in `STORE_ADMINISTRATION.md` if it references a Sold-tab alert (Sold no longer blinks).

**Integration seam** — how the redesigned surfaces get real data (client wiring, not byte-anchored here):
- **Account activity card** (`out/account-app.js:53,82-86`, shape `{ at, actor, action, summary }`) → **`GET /api/products?_action=activity`** → `{ activityLog: [...] }` (newest 25, `isTest`-scoped). *(`actor` is stored but not rendered — single-admin; the card shows `summary` + relative `at` + the dot.)*
- **Activity dot color** (`.logitem__dot--{prefix}`, `out/account-app.js:83`) → derived client-side from `action.split(".")[0]` (`product`→green, `sale`→blue, `order`→orange). All WS8 actions use those three prefixes.
- **Orders-nav blink ← `unseen_count`; numeric badge ← unfulfilled count; `mountShell` fetches `unseen_count` centrally on EVERY page.** The blink is a cross-page nav element (`out/portal.js:147,154`), but in the prototype only the Orders surface knows `unseen_count` — so the blink would never light on the **landing** page (products). Fix: `portal.js`/`mountShell` does its own authed **`GET /api/orders`** on every page (reading `unseen_count`) and drives the blink from it (`data-alert` when `unseen_count > 0`), independent of the caller's `ordersBadge`. The visible numeric **badge stays the unfulfilled count**; the two are decoupled. **Products' static-rail path needs the same wiring** — its rail isn't built by `mountShell`. Clearing is the `?_action=seen` stamp. → implemented byte-anchored in Phase 8.3.
- **Clear the blink (Orders viewed)** (`out/orders-app.js:12,132-133` in-memory `seen` Set) → **`POST /api/orders?_action=seen`** on Orders mount/view → stamps `site_config.orders_last_viewed_{env}` → next `GET` returns `unseen_count: 0` until a newer order arrives.
- **Sold product-tab blink** (`out/products-app.js:13,98`) → **removed** (Phase 8.2b).

**Phase 8.3 — `portal.js`: the central new-order signal (byte-anchored).** A shared `PORTAL.refreshOrdersSignal()` does the authed `GET /api/orders?status=needs_shipping` (a small payload) and drives the nav `data-alert` (from `unseen_count`, filter-independent) + the numeric badge (from the real needs-shipping count) on the Orders nav items — decoupled, on every page (`mountShell` pages **and** the Products static rail). Runs after `PORTAL.boot()`, so `authHeader()` is set.
...
> **Storefront-only. Small. Shares the card-render edit region with WS6 (single-edit region — coordinate).** Research concluded the grid-buy-button conversion evidence is commodity e-comm and does **not** transfer to one-of-a-kind, emotional, $200–500 art — the product page is the conversion engine, and scarcity is realized on the PDP. **Decision: do NOT add a raw "Buy Now" / "Add to Cart" on tiles.** Instead spend the small effort on *information scent* that pulls buyers into the story. (Reasoning + the sourced research summary: `_RATIONALE.md §9`.)

**Phase 9.1 — shop card + featured tile: full-tile tap target (verify, no code edit).** The whole product tile is **already** a generous tap target — §6.5a (shop) + §6.3d (homepage) wrap the entire card in a single `<a href="/product/…">`, with no competing in-tile button — so WS9's "full-tile tap" is delivered by those blocks; there's **no separate `shop.js`/`homepage.js` edit**. Any affordance polish (a hover/press cue) is a CSS-only render-tune on `.product-tile`. **Doc impact:** none.

**Phase 9.2 — "One of a kind" scarcity badge on tiles.** A small `badge badge-unique` with the copy **"One of a kind"** renders in the tile's `card__media` on **purchasable (`!sold`) tiles**. **Render markup is already folded into the merged card blocks** — §6.5a (shop grid) and §6.3d (homepage carousel), gated on `!sold` (do NOT add a separate card edit). **Visual spec (class + copy + CSS + placement) is DESIGN §D.4** — a NEW storefront-brand component (not in `out/`). **Doc impact:** none.

*9.2a — `assets/css/styles.css`: write the `.badge-unique` hue rule (without this the badge renders UNSTYLED).* **CURRENT (`assets/css/styles.css:586-593`):**
```css
.badge-featured {
  background: var(--bg-primary);
  border: 1px solid var(--color-gold);
...
.card__media .badge {
```
**NEW (insert `.badge-unique` after `.badge-featured`; solid storefront tokens — no `color-mix()`):**
```css
.badge-featured {
  background: var(--bg-primary);
  border: 1px solid var(--color-gold);
...
.card__media .badge {
```
*(Storefront tokens only — `--bg-primary` + `--accent-primary` (warm-plum), mirroring `.badge-featured` with a plum border instead of gold. `.badge-unique` inherits `.card__media .badge`'s absolute placement; the `!sold && !p.featured` render gate guarantees it never co-occupies the corner with Sold or Featured.)*

**Phase 9.3 — clear grid Sold state.** Sold pieces (`published && quantity===0`) show a clear **"Sold"** state on the grid (not "Sold out"), never a purchasable affordance — ties to the WS6 buy-gate (`published && quantity > 0`). Delivered by §6.5a. **Doc impact:** none.

---

## WS10 — GPT + docs parity (#222)

The Custom GPT ("The Sunkeeper") must run the new **automatic store-wide sale** and handle a **shared-payment refund** exactly as `/admin` does. This workstream is the GPT half only: it adds the `auto_apply` **schema param** on the coupon Action (the backend marker is WS4's), folds the store-wide-sale set/end beat + the initiative nudge + the shared-payment refund guidance into the instruction text, and restores the Coupon/Promotion Code/Discount vocabulary. **Sequenced after WS4.** The instruction file **ships verbatim** from the CURRENT/NEW blocks below — do not paraphrase; re-run `wc -c` < 8000 on the shipped `.txt` as the last step (static gate).

*Base files (latest shipped): `assets/docs/archive/v3_3/v3_3_0_GPT_INSTRUCTIONS_TRIMMED.txt` (`wc -c` = **7787 / 8000**) + `v3_3_0_GPT_SCHEMA.txt` (no total cap; per-`summary`/`description` soft cap 300). Copy both to `assets/docs/archive/v3_5/` as `v3_5_0_GPT_INSTRUCTIONS_TRIMMED.txt` + `v3_5_0_GPT_SCHEMA.txt`, bump the schema `version: 3.3.0` → `version: 3.5.0` (`:5`), apply the deltas below, then paste both into Em's Custom GPT. No Stripe API docs go into the GPT KNOWLEDGE — a Custom GPT can only call its declared Action endpoints; widen capability by adding the param + instruction beat, never by documenting Stripe.*

*Label-consistency delta: the base file's `PUBLISHING:` beat reads "Missing required fields: story_card", but the server's plain-label 400 now says "Story" (`FIELD_LABELS`), so the shipped `.txt` must read `Story` on both surfaces. **Phase 10.2b's NEW block already applies this swap** (it re-quotes the beat with `Story`); if you copy the base beat anywhere else, apply the same swap. Byte-negative (~10B).*

*(Param name `auto_apply` matches across WS4 and WS10 — one name, verified consistent.)*
...
**Phase 10.1 — GPT schema (`v3_5_0_GPT_SCHEMA.txt`): add the `auto_apply` param on `createCoupon`.** The GPT sets the automatic store-wide sale by passing `auto_apply:true` on `createCoupon` and ends it with the existing `deactivateCoupon` — no new op.
...
```
*(summary → 225 chars, `auto_apply` description → 206 chars, both under the 300 soft cap. The schema file has no total byte cap; only the instruction `.txt` does.)*
**Doc impact:** `EVERLASTINGS_STORE.md` KAD #10 (the GPT action-set) gains "`createCoupon` `auto_apply` = the automatic store-wide sale (v3.5)."

**Phase 10.1b — GPT schema: `editProduct` parity for take-down + scheduled-publish (schema-only, no instruction-`.txt` budget).** Both new parities go on `editProduct` param annotations (no instruction-text cost).

*10.1b.a — annotate `available` + add `scheduled_publish_at`.* **CURRENT (`editProduct` properties, `v3_3_0_GPT_SCHEMA.txt:127` — anchor by CONTEXT):** the token `available: { type: boolean }` is non-unique — it appears at `:36`, `:91`, `:127`, `:403`; **`:127` is the `editProduct` target** (the `available` between `product_type`/`quantity` and `featured`). Annotate ONLY that occurrence. (Scoping the take-down→draft annotation to `editProduct` and NOT `createProduct` is correct: a created piece is a draft regardless of `available`.)
```
                available: { type: boolean }
```
**NEW (annotate `available`; add `scheduled_publish_at`):**
```
...
```
*(Both parities land in the schema so no instruction-`.txt` byte is spent — the `.txt` stays 7988/8000. `PUT /api/products` already accepts both, WS2 §2.2/§2.4.)*
...
**Phase 10.1c — GPT schema: flag the publish-required detail fields so the GPT collects them like the dashboard (schema-only, no instruction-`.txt` budget).** WS2 §2.7's strict publish gate stands for every piece, so a maker driving via the GPT must gather the same full set the dashboard's form does, or `publishProduct` 400s. The schema already exposes all these fields but they carry no description, and `createProduct`'s `required` array + instruction line 4 frame them as optional. Annotate them. **Create stays lenient** (a partial draft still persists — §2.7 / §10.2b); this only guides the normal create→publish flow so it doesn't dead-end at a publish-400.

*10.1c.a — annotate the seven publish-required detail fields on BOTH `createProduct` and `editProduct`.* Set/extend each field's `description` with **"Required to publish."**. **Pattern (e.g. `createProduct.materials`, currently `:30`):**
```
                materials: { type: array, items: { type: string }, description: "Required to publish." }
```
Repeat on `dimensions`, `weight`, `features`, `care_instructions`, `shipping_details`, `quantity` (and their `editProduct` twins). **`features` is included deliberately:** it is publish-required (`validatePublishRules` REQUIRED_LIST, §2.7) but NOT in `createProduct.required` (schema `:19` = `[title, headline, story_card, description, price, product_type, slug, images, thumbnail]`), so it needs the flag. The genuinely already-required-on-create fields are `title/headline/story_card/description/price/product_type/slug/thumbnail`+`images`. `alt` is publish-required but stays **schema-optional** (it can't be `required`-on-create without breaking lenient partial-draft create) — it rides instruction line 10 + the per-image `images[].alt` publish gate. Schema descriptions have no total cap → **zero instruction-`.txt` cost** (stays 7988/8000).

**Doc impact:** STORE_ADMINISTRATION.md "Add a new piece" + the `PRODUCT-REFERENCE` knowledge file document the **required-to-publish** set (both surfaces) — those detail fields move out of "optional" framing.

*Residual — the instruction `.txt` line 4 still says `Optl.` for the six detail fields (`dimensions, weight, materials, care_instructions, shipping_details, quantity`; line 4's `Reqd:` already includes `features`), left unchanged (the 12-byte headroom can't absorb a line-4 rewrite). §10.1c bets the schema's per-field `Required to publish.` overrides line 4's `Optl.` for the GPT's collection behavior. TESTING 6b's GPT half verifies this — it asserts the GPT proactively ASKS for the six during a fresh create. If the test shows the GPT skips them, rework line 4 to move the six out of `Optl.`, self-funded via §10.2b's freed clauses (~40+ bytes) so the `.txt` stays under 8000.*

**Phase 10.2 — GPT instructions (`v3_5_0_GPT_INSTRUCTIONS_TRIMMED.txt`): store-wide-sale parity + restored vocabulary (the COUPONS beat, `:36`).** Fold in (a) the precise Stripe vocabulary — a sale = a **Coupon** + a **Promotion Code** = the **Discount**; (b) the automatic store-wide sale set/end path; (c) the one-discount-per-order reality stated plainly. Trim the illustrative date example + the "never a Unix timestamp" tail to hold the budget.
...
```
*(line delta +263 B → running `wc -c` = 7787 + 263 = 8050; the 10.3/10.4 trims bring it back under — all instruction edits ship as one set, recount once at 10.5.)*
...
**Phase 10.2b — GPT instructions: "create is lenient; the gate is PUBLISH" (the PUBLISHING beat `:34`, funded by a redundant `:13` trim).** WS2 §2.7 relaxes `createProduct` to a title+price minimum, but the GPT is never told, so a lenient 200 on a partial create could read as "it accepted everything." Add a short beat to the PUBLISHING line (its natural home). Self-funded: drop `:34`'s now-niche "Never make a no-op edit to 'regenerate' a link" clause and `:13`'s redundant "no need to read fields back." tail.
...
```
*(deltas: `:34` +29 [insert +87, drop the no-op clause −48, `story_card`→`Story` ×2 −10], `:13` −29 → phase net +0 B. The `story_card`→`Story` label swap rides in this NEW block.)*
**Doc impact:** the GPT create/publish walkthrough notes create is lenient (title+price min) and the required-field gate is at publish.

**Phase 10.3 — GPT instructions: the initiative nudge (`:1`), funded by two now-redundant trims (`:15`, `:30`).** Add the standing posture — do the reversible things without asking, confirm only the irreversible — to the intro, replacing the vague "Action description show mechanics to rely on." tail. Fund it by tightening `:15` and `:30`.
...
```
*(deltas: `:1` +23, `:15` −12, `:30` −39 → phase net −28 B.)*
**Doc impact:** none in STORE; the initiative posture is a GPT-behavior beat (test-script parity item).

**Phase 10.4 — GPT instructions: shared-payment refund guidance, #228 GPT half (`:40`).** The refund code is sound (WS3 preserves it) — this is GPT guidance only. On a multi-item shared payment, when Em names ONE piece: identify that named line, default the amount to that line, relist ONLY the named piece, confirm clearly, never silently swap a sibling's piece or amount. Condense the relist mechanics to hold budget (the field paths stay verbatim — load-bearing).
...
- 10.2 (`:36`) **+263** · 10.2b (`:34 +29`, `:13 −29` = **+0**) · 10.3 (`:1 +23`, `:15 −12`, `:30 −39` = **−28**) · 10.4 (`:40`) **−44**
- **net +191 → projected `wc -c` = 7978 / 8000 (22 headroom).**

*(Over-cap = the GPT silently truncates its own instructions and the gate fails; the shipped `.txt` must still be re-counted — orchestrator-owned, never delegated. Then the one human touchpoint: re-paste schema + instructions into Em's ChatGPT and spot-check the sale set/end + a shared-payment refund.)*

**Phase 10.6 — docs (as-built, after the build — a fresh agent, per the As-built section; not mid-build).** Apply the `Doc impact:` annotations above into `EVERLASTINGS_STORE.md` (KAD #10 action-set gains `auto_apply`; the coupon glossary restores Coupon/Promotion Code/Discount; the Stripe-sync note gains the shared-payment named-piece default), then refresh `STORE_ADMINISTRATION.md` + `GPT_SETUP.md` (the shipped v3.5 instruction/schema text) + the GPT behavior test script. Code is the tiebreaker where doc and behavior disagree (cite `file:line`).
...
## Verification (end-to-end, dev preview)

Per-surface + backend E2E on seeded **test** products (production-grade placeholders — real content is never a build/test gate): store-wide sale live/struck/at-checkout + removable keyword; refund partial + relist (preserved behavior); media modal roles/reorder/persist; product-page fields render; series filter matches; carousel rows decoupled; webhook even-split correct per-item amounts; cart-hold gone; sold-policy transitions (available-off→draft, qty0→sold, sold persists until archive); seen/unseen clears + not-on-Sold-tab; publish + preview gates; scheduled publish flips; activity-log rows written; WS9 information-scent (the "One of a kind" badge + the full-tile tap render on shop/homepage — no raw buy button). Plus the **static gate** (tsc/CJS clean · function count unchanged · no new cron · `node --check` · valid `vercel.json` · GPT `.txt` `wc -c` < 8000) and the **GPT-parity spot-check** (the one human touchpoint, in Em's ChatGPT). *(Carry forward the v3.7.4 testing addendum as the full verification plan — it is current.)*

## As-built doc-sync — LINE-BY-LINE (the final phase; a fresh agent; NEVER mid-build)

When execution completes, a **fresh agent** (not the tired tail of the build session) brings `EVERLASTINGS_STORE.md` current: (1) read STORE **end-to-end, linearly, like a human** first so it coheres and contradictions surface; (2) walk the build-adjusted IMPLEMENT **line by line** as the change-source (with the `BUILD_REPORT` for build-time deltas) — a memory summary is what let details fall through the cracks last cycle; (3) where the IMPLEMENT/report and the doc disagree on a **behavior, the actual code is the tiebreaker** (cite `file:line`). Each phase carries a one-line **`Doc impact:`** annotation so the as-built is *apply the annotations* + the coherence read. Then refresh `STORE_ADMINISTRATION.md` + `README.md`. Two reconciler landmines: the STORE Status/Version header drifts a release behind (verify vs the newest `*_BUILD_REPORT.md` first); stale `file:line` anchors survive code growth (re-open before trusting).

## Cross-references

Architecture/glossary/schema → `EVERLASTINGS_STORE.md` · design intent + contract → `design-handoff/brief.md` + `data-flow.md` + `out/INTEGRATION.md` + `out/PRODUCT_LIFECYCLE.md` · the finished design markup → `v4_0/design-source/out/` (CD's polished set) · backlog → `v3_5_0_ROADMAP.md` · the design *why* → `_RATIONALE.md` · branching/versioning/gap-gate → `.agent/DEV_RULES.md`.
