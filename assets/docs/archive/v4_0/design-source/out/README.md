...
- **Publish-from-preview is the primary publish path.** The storefront preview shows the
  **review bar** — the canonical spec is the live site's `assets/js/product.js`
  `mountPreviewBanner` (POSTs `/api/products/publish`). `out/` points at it rather than
  reinventing it; the editor's Publish gate is what leads into it.
- **Gap fixes are logged in [`CHANGELOG_GAPS.md`](./CHANGELOG_GAPS.md)** — what changed, which
  files, and each file tagged PORTABLE (safe drop-in) vs. SEAM (`data.js`, logic ports/data doesn't).
