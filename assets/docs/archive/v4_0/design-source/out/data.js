...
(function () {
  ...
  /* ---------------------------------------------------------------- created_at
     Schema field (real Postgres has products.created_at). Seeded here so the
     list has an explicit, stable DEFAULT ORDER (newest-first) instead of
     depending on API return order — see GAPS.md #7 / PRODUCT_LIFECYCLE.md.
     A product created in the UI gets `new Date()`, so new pieces sort to the top. */
  const CREATED_AT = {
    "a2bf5bf3-0000-4000-8000-000000000001": "2026-06-14T15:20:00Z", // Lantern Keeper's Cottage
    "a40677f8-0000-4000-8000-000000000002": "2026-06-15T19:02:00Z", // The Reading Hour
    "427c35db-0000-4000-8000-000000000003": "2026-06-15T20:41:00Z", // First Snow, Stillwood
    "aa867f06-0000-4000-8000-000000000004": "2026-06-15T21:10:00Z", // The Tide Library
    "32769d4e-0000-4000-8000-000000000005": "2026-06-15T23:58:00Z", // The Clockmaker's Window
    "efc9c3a9-0000-4000-8000-000000000006": "2026-06-16T00:05:00Z", // The Night Train (staged edits)
    "a846cc13-0000-4000-8000-000000000007": "2026-06-26T14:03:00Z", // The letter room
    "6ed8a47d-0000-4000-8000-000000000008": "2026-06-26T17:44:00Z", // The Tide-Glass Lantern
    "d1000000-0000-4000-8000-0000000000d1": "2026-06-29T11:15:00Z", // Greenhouse at Dusk (draft, WIP → newest)
    "50100000-0000-4000-8000-000000000501": "2026-05-18T10:30:00Z", // Cartographer's Desk (sold)
    "a5c10000-0000-4000-8000-0000000000a5": "2025-11-28T13:00:00Z", // Harbor Lights (archived)
  };
  products.forEach((p) => { if (!p.created_at) p.created_at = CREATED_AT[p.id] || p.published_at || "2026-01-01T00:00:00Z"; });

  /* ---------------------------------------------------------- unfulfilledCount
     Single source of truth for the "unshipped orders" nav badge (GAPS.md #6).
     Groups order rows by stripe_payment_intent (one card per purchase) and
     counts the purchases that still have a paid-but-unshipped line. Any surface
     that shows the Orders badge reads THIS, so the number can never drift or go
     stale — it drops the moment a piece ships and clears to 0 (no badge, no blink). */
  function unfulfilledCount() {
    const seen = new Set();
    let n = 0;
    orders.forEach((o) => {
      if (seen.has(o.stripe_payment_intent)) return;
      const lines = orders.filter((x) => x.stripe_payment_intent === o.stripe_payment_intent);
      if (lines.some((l) => l.status === "completed" && !l.shipped_at)) n++;
      seen.add(o.stripe_payment_intent);
    });
    return n;
  }

  window.PORTAL_DATA = { products, orders, coupons, storeWideSale, config, activityLog, computeState, money, unfulfilledCount };
})();
