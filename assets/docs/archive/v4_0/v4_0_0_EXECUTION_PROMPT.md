...
- **For Sean/Em — one item, needed only at the GPT-parity spot-check (end of testing), NOT at kickoff:** repointing the **Custom GPT** at the dev preview (its Action server URL → the Preview URL, auth → the **Preview** `PRODUCT_API_KEY`) lives in Em's ChatGPT account and can't be automated. Build everything and run every non-GPT test first; when you reach the GPT-parity items, STOP and request the repoint, then run them.
