...
PRE-FLIGHT: Runtime preconditions, already triaged. Every item is the orchestrator's to **confirm-then-act** — **none blocks kickoff and none needs Sean.** Sean's env is warm from prior builds, so several are already satisfied; when one is, note it and move on — **never hard-stop on a done precondition.**

- **You handle — all confirm-then-act (details in the TESTING addendum's "Before you start"):** seed test products in the dev preview (incl. a multi-piece cart of ≥2 pieces with UNEQUAL prices for the WS7 even-split test, and ≥1 qty-1 + ≥1 qty≥2 piece); confirm/set `CRON_SECRET` on the preview (arms both cron-gated jobs — check `vercel env ls`, set only if missing, don't overwrite an existing one); **confirm `charge.refunded` is subscribed** on the preview webhook (already added on preview **and** prod — a check, not an add); run the legacy sold-row backfill **before enabling any % sale**; verify the cron gate with the two curl checks; confirm the preview is `VERCEL_ENV !== 'production'`. Confirm a Stripe test event and a GPT call both reach the preview before trusting any webhook/GPT item (Deployment Protection / preview SSO is off for your use).
