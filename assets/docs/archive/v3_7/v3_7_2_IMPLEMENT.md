...
- **GPT instruction char budget — hard cap 8000.** Any WS10 instruction add re-runs `wc -c` on the shipped `.txt`; over-cap, the GPT silently truncates its own instructions. **Named trim target if a future WS10 add pushes over 8000 (C#2, v3.7.2):** first cut is §10.2b's freed clauses (the same reserve §10.1c's residual note names for its test-fail case); if that's short, abbreviate line 4's `Optl.` enumeration (those fields already live in PRODUCT-REFERENCE). Fallback only — the static gate re-counts every ship, so an over-cap `.txt` can never actually ship; worst case is one trim cycle.
...
## Verification (end-to-end, dev preview — full plan in `v3_7_2_ADDENDUM_TESTING.md`)
