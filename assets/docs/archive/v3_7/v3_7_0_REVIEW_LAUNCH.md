# v3.7.0 — B/C/D reviewer launch kit (human-facilitated Claude Code)

The setup is done; this is the fire-the-reviewers card. Five fresh reviewers this round: **B · C · D** (the three lanes the A-round-1 folds re-opened) + **2 breadth** (owner-journey + integration, the cross-lane backstop). Run each in its **own fresh Claude Code window** so there's no context contamination.
...
4. When it finishes it writes `assets/docs/archive/v3_7/v3_7_0_GAP_REVIEW_<angle>.md` **and** prints its findings — paste that reply back here so the orchestrator validates + folds.

The build under review (all three, read end-to-end): `assets/docs/archive/v3_7/v3_7_0_IMPLEMENT.md` + `v3_7_0_ADDENDUM_DESIGN.md` + `v3_7_0_ADDENDUM_TESTING.md`. Source material resolves in the sibling `assets/docs/archive/v3_5/` per the docs' directory banner.

## B · C · D — use the self-contained blocks in `v3_7_0_REVIEW_PROMPTS.md`

Each is already paste-ready (complete lens + the full ledger 1–72 + false-alarm classes inlined). Open `v3_7_0_REVIEW_PROMPTS.md` and copy the whole fenced block under:
...
DOCS (read all three, in `assets/docs/archive/v3_7/`): v3_7_0_IMPLEMENT.md (10 workstreams) + v3_7_0_ADDENDUM_DESIGN.md + v3_7_0_ADDENDUM_TESTING.md. Source material (design-handoff, GPT files, ROADMAP) is in the sibling assets/docs/archive/v3_5/. The settled "do not re-raise" ledger (entries 1–72) is inside v3_7_0_REVIEW_PROMPTS.md — read it and do NOT re-raise those.
...
DOCS (read all three, in assets/docs/archive/v3_7/): v3_7_0_IMPLEMENT.md + v3_7_0_ADDENDUM_DESIGN.md + v3_7_0_ADDENDUM_TESTING.md. Source material is in the sibling assets/docs/archive/v3_5/. The settled "do not re-raise" ledger (1–72) is inside v3_7_0_REVIEW_PROMPTS.md — read it; do NOT re-raise those.
