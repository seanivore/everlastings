...
*7.2j — reword the customer-facing cart-hold promise (breadth-integration #1, v3.7.3).* The storefront still promises the 15-min hold in three places that go **false** once §7.2 ships — reword them to the no-reservation reality (pieces are one of a kind and NOT held; checkout is first-come — complete promptly). **Proposed copy (storefront voice — Sean/Em confirm tone):**
- **`policies.html:167-172`** — the `<section id="cart-hold">`: rename the id to `availability` (the FAQ already deep-links `/policies#availability`) and replace the "soft 15-minute hold … no one else can claim them" + "the hold quietly releases" paragraphs with, e.g. — heading **"Availability"**; body: *"Because every piece is one of a kind, we don't reserve items — a piece is yours once your order completes. We check availability the moment you head to checkout, so if two collectors want the same piece, it goes to whoever finishes first. If you love it, check out promptly."*
