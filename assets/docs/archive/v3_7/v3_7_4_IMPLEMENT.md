# v3.7.4 Implementation Plan — the Content Creator Portal redesign + the v3.5 store/checkout backlog

> **📁 Directory note (read first).** This `v3_7/` package is the **B/C/D gap-review gate copy** (human-facilitated Claude Code), copied from the converged **v3.6.7** A-Type gate docs. The three living docs live here in `assets/docs/archive/v3_7/` (`v3_7_4_IMPLEMENT.md` + the two addenda). The frozen **v3.6.7 A-gate copy + its full A-Type `GAP_REVIEW_*` trail** sit in the sibling `assets/docs/archive/v3_6/` (the completed prior phase). **All source material — the design handoff (`design-handoff/…`, `out/…`), `v3_5_0_ROADMAP.md`, the GPT base files, `v3_5_0_RATIONALE.md` — lives in the SIBLING `assets/docs/archive/v3_5/` directory.** Every `design-handoff/…`, `out/…`, and `v3_5_0_…` path in these docs is relative to that `v3_5/` dir. The frozen in-session `v3_5_5_*` copies + the full `GAP_REVIEW_*` rigor trail (rounds 1–4) also remain in `v3_5/`.
...
**Revision**: **v3.7.4 — convergence-tail fold + design-currency reconciliation (gate CONVERGED).** The B/C/D + 2-breadth re-run on v3.7.3 came back all-NARROW; every finding was a loose end of a prior fold, none load-bearing. Folded the **cart-hold ripple cluster**: `policies.html` now **DELETEs** the `#cart-hold` section (the truthful `#availability` section already exists above it — a rename would duplicate the id and leave false copy live); the §7.2 doc-impact retires `tests/integration/08_hold_expiry.sh` (+ its `run-all.sh` line, spot-check 06/10) and names STORE.md `:148`/`:1140`; `shipping.html:197` drops "and cart-hold"; the count summaries read **four**. Homepage §6.3d renders the **Sold** badge on a sold-but-featured tile (mirrors shop §6.5a). §4.3 recomputes `--header-offset` on scroll (rAF-throttled) so a nav opened after scroll doesn't inherit a stale offset. WS10 swaps the GPT `.txt` publish-400 example `story_card`→`Story` (FIELD_LABELS parity, ledger 92). **Field-lock taxonomy settled (3 tiers, §3.3–3.8):** `sku` born-locked; `slug` auto-generates from the title on entry + locks on first persist (`!!p.slug`; server rejects any slug PUT regardless of publish state, `products.ts:359-361`) — pulled OUT of the `everPublished` bundle; checkout identity (`checkout_name/description/image`) locks at publish. The `:651` comment follows the `validatePublishRules` rename (#11). **Design-currency RESOLVED off the review loop:** Claude Design delivered the polished dashboard; a line-by-line pass verified it **BACKEND-CLEAN** (products-app.js zero API changes; `data.js` the lone SEAM — `created_at` is already a column + indexed, `unfulfilledCount` uses existing order fields) → it drops in at the **v4.0.0** execution cut. Dropped as non-issues (do not re-raise): the media-modal checkout-role lock (#3 — checkout image locks once set / defaults-to-hero-at-publish) and the dashed `.field--locked` visual (#4 — the lock chip + `disabled` already reads clearly). **Gate CONVERGED — no more review rounds; next is the v4.0.0 execution cut.** Ledger → 100. **v3.7.3 — scoped-re-run fold** (C + D + 2 breadth, all NARROW; validated against the code). Closed 4 load-bearing: **§8.1c(g)** `handleCoupon` block re-quoted to the post-§4.1.c anchor (completes B#2/ledger 80, which had added only the coordination note); **cart-hold customer-facing copy** reworded to the no-reservation reality (WS7 §7.2 — `policies.html`/`faq.html`/meta; my C#1 hardened STORE.md but forgot the storefront copy); **§2.7a client-side half** (the checkout-field lock cues + the media modal's checkout_image now key on `everPublished = !!published_at`, matching the server freeze — a paused piece shows them LOCKED, not editable-then-silently-400); **sale-popup + mobile-nav** measure the live header bottom (`.sale-bar` is normal-flow and pushes the sticky header down, so the hardcoded `--header-height` top overlapped the cart icon at unscrolled load). Tightened my v3.7.2 folds: C#1 also marks AR #28/#29 + names the real table-count refs; D#6 filter is additive (`!p.archived_at && p.is_published`); C#2's trim-target drops the phantom §10.2b reserve; the "three enforcers" summary spots now say **four** (→ §6.5). **Decision (Sean, option A): snake_case-400 RESOLVED** — `validatePublishRules` maps field keys to plain labels on both surfaces (the thrice-flagged item, closed). Ledger → 92. **v3.7.2 — B/C/D gate fold** (3 Sonnet-5·MAX peers: B fidelity READY, C integration READY, D design NARROW; all validated against the code). Folded: **B#1** ledger 61/70a reword (`.mitem--errored` is a WS5 build output, not yet in the tree — corrects a false "already shipped" claim); **C#1** the Phase 7.2 doc-impact now marks `cart_holds` SUPERSEDED in `STORE.md` (KAD #18/#19 + schema + API map + 8→7 tables) with a go-live note on the accepted no-reservation oversell trade-off + refund recovery; **D#2** a reduced-motion guard on the sign-in canvas (§1.4f); **B#2** the `handleCoupon` re-anchor note; + polish **D#6** (scope picker → published-only) and **C#2** (named `.txt` trim target). **D#1 resolved** — the publish-button must-preview gate is already in the build (§2.1/§2.7); its button-state cosmetics are stale-`/out/` → parked on the post-build **design-currency backlog** (reconciled via Claude Design, NOT re-synced now — a re-sync would force a full re-gap-test). Dismissed as already-settled: D#3 (char-counter = ledger 54), D#4 (`data.js` is prototype mock, not ported). Ledger → 84. **v3.7.1 — folded the pre-B/C/D breadth round** (2 Sonnet-5 peers, both NARROW, validated against the code): §2.7a **freeze checkout identity on *ever*-published** + preserve `published_at` on relist (journey #1 — closes the pause→edit→relist Stripe-desync); the `handleActiveSale`/`handleActivityLog` **append-stack** coordination note (integration #1); §5.4c.i **media partial-failure terminal-PUT persist** (journey #2 — no R2 orphan); the GPT `Optl.`↔schema `Required to publish.` **residual + a proactive-collection test** (§10.1c / TESTING 6b, integration #2); + 2 polish coherence fixes (stale-qty both-surfaces, §4.1.d no-dangling-WS10). **3 polish items DEFERRED (not built — keep the build lean, Sean's call):** Related-Havens struck pricing, GPT scheduled-publish push-notif, Orders-nav load-blink. **v3.7.0 — opens the B/C/D gap-review gate (human-facilitated Claude Code).** Copied verbatim from the converged v3.6.7 A-Type gate copy — no content change at the minor bump; the version bump is the phase crossing. The A-Type gate ran round-1 cold-A (NARROW → nine folds + the owner decision surfaced-then-RESOLVED) and exited clean. B (fidelity) / C (integration + architecture) / D (design-correctness) now get their **full pass** on this v3.7.0 triplet: they have not formally reviewed the v3.6.6/.7 folds (§10.1c GPT publish-set collection, §2.7 strict-gate RESOLVED, §5.4c.i `.mitem--errored`). — Prior history: consolidated at v3.6.4; QA pre-flight at v3.6.5; round-1 cold-A + breadth folded at v3.6.6; **v3.6.7 — owner decision RESOLVED + its forward fold.** Sean's call on the round-1 surfaced decision: the **strict publish gate STANDS** for every piece (no grandfather, no backfill) — there is **no live catalog yet** (empty), so nothing predates the strict rules, and products were always intended to carry the full field set. The one forward requirement Sean flagged is folded: **both surfaces + the admin docs must COLLECT the full publish-required set** so a maker never hits a confusing publish-400. §2.7's decision block is now RESOLVED; a new **WS10 §10.1c** flags the seven publish-required detail fields (`dimensions, weight, features, materials, care_instructions, shipping_details, quantity`) as "required to publish" on the GPT `createProduct`+`editProduct` schema (schema-only, **no instruction-`.txt` byte** — the `.txt` stays 7988/8000), so the GPT gathers them up front like the dashboard does; STORE_ADMINISTRATION.md + the PRODUCT-REFERENCE knowledge file gain the required-to-publish set (§10.6 doc-impact); TESTING item 6b is reframed to test the strict gate on BOTH surfaces. Folds to date: round-1 A + round-1 breadth + owner-decisions + this decision (see REVIEW_PROMPTS ledger 39-92). Rationale stays inline through the pre-execution loop; relocates to sibling `RATIONALE.md` only at the v4.0.0 execution cut (DEV_RULES → Build Guide Final Cuts).

**Required reading first**: `assets/docs/EVERLASTINGS_STORE.md` · `README.md` · THIS doc + its two addenda **in this same `assets/docs/archive/v3_7/` directory** (`v3_7_4_ADDENDUM_DESIGN.md`, `v3_7_4_ADDENDUM_TESTING.md`) · the design handoff (in the sibling `v3_5/` dir per the directory note above) in its own read order — `design-handoff/out/INTEGRATION.md` → `out/PRODUCT_LIFECYCLE.md` → `design-handoff/brief.md` → `design-handoff/data-flow.md` → the `out/` files (`products.html`+`products-app.js`, `orders*`, `sales*`, `account*`, `portal.css`, `portal.js`, `data.js`) → `design-handoff/controls.html`+`tokens.css` → `design-handoff/feedback/FEEDBACK_v1.md` + `design-handoff/reference/` → `out/README.md` · `assets/docs/archive/v3_5/v3_5_0_ROADMAP.md` · `.agent/DEV_RULES.md`.
...
> **Status / depth.** This is the **converged gap-review copy** — the B/C/D + breadth gate closed at v3.7.4 (all lanes NARROW, every finding a loose end of a prior fold). The clean execution copy is the pending **v4.0.0** cut, NOT this file. Backend reconciliations + new backend (WS2 state semantics, WS4 sale extension + public read, WS5 upload, WS6 storefront fixes, WS7 webhook/money, WS8 activity log, WS9 buy-on-tile) are **byte-anchored** where the code is settled (exact CURRENT/NEW — line numbers are hints, the quoted CURRENT text is the anchor; STOP + reconcile if it drifts). The **presentation-integration** workstreams (WS1 shell, WS2/WS3/WS4 the per-surface data→API seams) carry **integration-seam tables** (each mock/no-op `<surface>-app.js` call → its real endpoint) rather than full byte-blocks, because the delivered `out/` files are the canonical markup source — see `v3_7_4_ADDENDUM_DESIGN.md`. Design ships as concrete-default + render-tune per DEV_RULES; the verification plan is `v3_7_4_ADDENDUM_TESTING.md`.
...
- **`api/products.ts` — order: WS2 → WS4 → WS8.**
  ...
  - *new HANDLER FUNCTIONS append at the SAME spot* (breadth integration #1, v3.7.1): **WS4**'s `handleActiveSale` (§4.2.b) **and WS8**'s `handleActivityLog` (§8.1d.b) BOTH append their function body in the gap after `handleCouponList`'s closing brace (before the `// ?_action=coupon_deactivate` comment, live tree `:836-838`). This is a **stack, not a collision**: apply WS4 first (`handleActiveSale` lands in the gap), then WS8's `handleActivityLog` re-anchors on the still-present `}\n}\n\n// ?_action=coupon_deactivate` text — now the tail of `handleActiveSale` — and appends after it. Both functions end up between `handleCouponList` and `handleCouponDeactivate`; neither replaces the other. (The line hints `~:676`/`~:786` drift after the first append — the `coupon_deactivate` comment is the stable anchor.)
  - *PUT handler*: **WS2** is the sole editor (available-off→draft, `scheduled_publish_at` set/clear, the new response flags).
  - *`handleCoupon`* (~:735 metadata, ~:745 return): **WS4** stamps `metadata.auto_apply` + the supersede-prior sweep, **WS8** wraps the return with a `sale.create` log — WS4's edits land first, then WS8's log. **(B-fidelity #2, v3.7.2 — re-anchor):** WS4 §4.1.c and WS8 §8.1c's (g) create-sale log quote the SAME 2-line CURRENT anchor `products.ts:745-746`; §4.1.c inserts its supersede-prior block BETWEEN those two lines, so by the time §8.1c(g) applies, `:745-746` is no longer contiguous — WS8 re-anchors on the unique `return jsonResponse(...)` success return of `handleCoupon` and wraps THAT (same re-anchor discipline as the `handleCouponList`-tail stack, ledger 74).
...
- **Sold-policy consistency (cross-cutting — the reviewers' highest-value check).** The state model is honored identically by **four** enforcers: the portal's `computeState()` (WS2), the storefront buy-gate `published && quantity>0` (WS6), the webhook decrement `record_sale` (WS7), and the deliberately-stricter **server checkout/reserve gate** `available===true && quantity>=1` (`checkout.ts:79`/`:205`) — see WS6 §6.5 for the 4th enforcer + its fail-safe edges. *(v3.7.3, C re-run: this summary line + the TESTING cross-cutting item said "three"; corrected — the code always had four.)* See *Locked decisions → Product state / sold policy*.
...
- **GPT instruction char budget — hard cap 8000.** Any WS10 instruction add re-runs `wc -c` on the shipped `.txt`; over-cap, the GPT silently truncates its own instructions. **Named trim target if a future WS10 add pushes over 8000 (C#2, v3.7.2):** trim from the lowest-value instruction clauses — abbreviate line 4's `Optl.` enumeration first (those fields already live in PRODUCT-REFERENCE), then the least-load-bearing beat. (v3.7.3, C re-run: there is NO separate unspent reserve to draw from — §10.2b's own trim was already spent funding its +10B add, already counted in the 7988 tally.) Fallback only — the static gate re-counts every ship, so an over-cap `.txt` can never actually ship; worst case is one trim cycle.
...
**Editor field rules (§3.3–3.8).** Required-field publish gate uses Sean's authoritative set (`title, slug, headline, description, price, quantity, product_type, story_card, features, materials, care_instructions, shipping_details, dimensions(W·D·H), weight, hero image, ≥5 gallery images, alt text on every media`) + **required-but-auto-generated-if-blank** (never block publish): `seo_title, seo_description, checkout_name, checkout_description, checkout_image, seo_thumbnail` — generated server-side from other fields if empty. **Optional:** `series`, `artist_note`. **Only `product_type = miniature` is in scope** (the picker must not offer printable/storybook — the API validates every type as miniature; a new type is separate development). **Dimensions = 3 inputs (W/D/H)** assembled/parsed to the `dimensions` string; **validate-and-auto-format, never reject** a good value (coerce; show the expected format in the label tooltip) — same for `weight`. **List fields** (`materials, features, care_instructions, shipping_details`) are one-item-per-line textareas ↔ `string[]`, with a live bullet preview. **Auto-save on close** (click the sticky row / click outside on desktop / ✕ on mobile) → `PUT /api/products` (a draft if new, staged edits if published); the explicit **Save** saves and returns to the list. **Publish requires a Preview** for a new, never-published product (the preview page carries the real Publish via `preview_token`). **Field-lock taxonomy — three tiers, three lock moments (v3.7.4; supersedes the single "lock-after-publish" bundle).** Locked fields **show locked, not missing** (a lock chip + `disabled`, never hidden): **(1) `sku` — born-locked:** generated at draft-create, never editable, no affordance to change it (today `sku` is a create-time value; it is never offered as an input). **(2) `slug` — locks on first persist:** it **auto-generates from the title the moment the title is entered** (mirrors the DB `set_slug` trigger), and the client lock cue keys on **`!!p.slug`** (locked once the row has been persisted — the first *manual* Save, or the auto-save when the draft closes — as long as it's non-empty). The server rejects any `slug` in a PUT **regardless of publish state** (`products.ts:359-361`), so slug is **immutable from creation**, not from publish — which is why it is **NOT** in the `everPublished` group. **(3) `checkout_name, checkout_description, checkout_image` — lock at publish** (the checkout-identity snapshot): the client lock cue keys on **`everPublished = !!p.published_at`, NOT `is_published` (v3.7.3, journey #1)** — a PAUSED (ever-published) piece keeps these locked, matching the server `FROZEN_AFTER_PUBLISH` guard which fires on `published_at` (§2.7a) — else the client shows them editable, the maker edits one, and the server silently 400s. `checkout_image` **defaults to the hero if still unset at publish** and freezes then (in the CD uploader it can also lock earlier, once explicitly set). **General rule:** auto-generated-locking fields **populate on title-entry and lock on first persist**. In every tier, if a frozen field ever reaches the server changed, the 400 **SURFACES** as a field-list toast, never a silent no-op (the `commitAvail`-ON pattern).
...
*(The prototype's prefilled `value="${esc(account.email)}"` on the email input now starts blank; that's the correct sign-in behavior. The identicon is untouched.)*

*1.4f — reduced-motion guard on the sign-in canvas FX (D#2, v3.7.2).* `initLoginFx` (`account-app.js:115`) starts an unconditional `requestAnimationFrame` ribbon loop (`account-app.js:174`); its only `matchMedia` is the `(max-width:560px)` mobile test (`:122`), so it ignores `prefers-reduced-motion` — and CSS (`portal.css:528-532`) can't reach a JS canvas loop. Guard the loop start: at the top of `initLoginFx`, `if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;` (paint one static frame / nothing, skip the `raf` schedule) so the sign-in honors reduced-motion (ledger 11). No other behavior change; the identicon + form are untouched.
...
```ts
  ...
  const { data: published, error: pubError } = await supabase
    ...
    .single();
```
*(`syncProductToStripe` still reads `checkout_name || title` etc., so Stripe is unchanged; this just writes the same values back so /admin shows them locked, not blank. **`published_at` is preserved when already set** (`?? new Date()`) — v3.7.1 §2.7a: a paused piece relisting through this first-publish branch keeps its original publish date instead of re-stamping to now.)*
...
**Phase 2.7a — checkout identity is frozen once EVER published, and republish never re-stamps (breadth journey #1, v3.7.1; Sean's call).** §2.2 makes Available-OFF unpublish a live piece to draft (`is_published=false`). But the checkout-identity freeze lives **inside** `if (current.is_published)` (`products.ts:390`), so a **paused-but-previously-published** row (`published_at` set, `is_published=false`) falls to the unpublished-PUT branch and re-opens `checkout_name/description/image` for edit. Because first-publish's `autoGen` is **fill-if-blank** (`row.checkout_name || title`, §2.2 first-publish NEW), that edit *sticks* in the DB on relist — but `syncProductToStripe` short-circuits on the existing `stripe_product_id` (`stripeSync.ts:39-47`) and never pushes it, so the site shows a new checkout name/image while **Stripe still charges under the old** → a silent site↔Stripe desync. **Decision (Sean, v3.7.1): checkout identity is set once and frozen for the LIFE of the piece — a paused piece keeps it locked.** Storefront presentation (`title`/`story_card`/`images`/`price`) stays fully editable post-publish; only the Stripe **checkout snapshot** locks. Changing that snapshot is a deliberate, confirmed *future* feature (below), never this path — "don't touch the Stripe catalog willy-nilly." Two surgical edits:

*(a) — the freeze fires on EVER-published, not just currently-published.* Hoist the `FROZEN_AFTER_PUBLISH` rejection above the `if (current.is_published)` block so it also guards a paused row (the inner check stays — redundant-but-harmless for a live row, which always has `published_at`). **CURRENT (`api/products.ts:385-390`):**
```ts
...
*(b) — republish preserves the original `published_at` (no re-stamp on relist).* A paused piece relisting routes through first-publish (`:648`, `row.is_published` false), which re-stamps `published_at`. Preserve it so a relisted piece keeps its true first-publish date (and doesn't jump to the top of a recency sort). Folded into §2.2's first-publish NEW block — the `published_at: (row.published_at as string) ?? new Date().toISOString()` change there.

**DEFERRED FEATURE (NOT built here) — "Reissue checkout identity."** The one real reason to change a locked checkout name/image is the "Em repaints a slow seller a new color + it needs a new title" case. When it's worth building: a deliberate, **confirmed** action (its own control + a "this changes the Stripe catalog" confirm — never a casual field edit) that (1) grows the `stripeSync` short-circuit (`:39-47`) into an **update** path for the existing Stripe product's name/description/images and (2) writes the DB in lockstep so the two never diverge. Logged so the intent isn't lost; **out of scope for v3.x** (keeps this build lean — Stripe-catalog writes stay rare + intentional).
...
*4.3.e — the mobile-nav drawer must clear the real header too (D#1, v3.7.3).* The pre-existing `.mobile-nav` drawer hardcodes `inset: var(--header-height) 0 0 0` — pushed-header-agnostic. Because the utility bar is ALWAYS present (free-shipping default, not just during a sale), the drawer overlaps the header's lower edge on every page. Re-point it at the same `--header-offset` the sale-chrome JS now sets (fallback `--header-height` if the JS hasn't run yet). **CURRENT (`assets/css/styles.css:311`):**
```css
...
- **Create coupon** — `createSale()` (sales-app.js:177–206) → **`POST /api/products?_action=coupon`** with `{type, value, code?, product_ids?, min_amount?, expires_date?:'YYYY-MM-DD', max_redemptions?}`. *(`product_ids` are **`stripe_product_id`**, not Supabase ids — map published products first, per the v3.3 coupon decision.)* `409` = code exists; `400` = bad type/value. **(D#6, v3.7.2 — the scope picker lists PUBLISHED pieces only):** the New-Sale scope picker must filter its piece list additively to **`!p.archived_at && p.is_published`** (v3.7.3, D re-run #2 — NOT a replacement for the existing `!archived_at`: an archived piece can keep `is_published:true` since `handleArchive` only sets `archived_at`, `products.ts:906-911`, so a bare "published" filter could resurface it) — a draft/unpublished piece has no `stripe_product_id`, so scoping a coupon to it can't map (the `product_ids`→`stripe_product_id` step above drops it) → a silent create-coupon failure when a maker taps a draft; filtering the picker prevents it.
...
| Apply (`applyMedia :745`)                         | `PUT /api/products?id=` (or `POST` if new) | `{images, media, seo_thumbnail, thumbnail, thumbnail_alt}`; add `checkout_image` **only if NEVER-published** (`p.published_at == null`, §2.7a — not merely `is_published:false`; Phase 5.4e) |
...
- **`checkout_image` is frozen once EVER published** (v3.7.3, journey #1) — the modal locks the checkout role on any `p.published_at != null` piece (including a PAUSED one, not merely `is_published`) and **excludes `checkout_image` from the terminal `PUT`** there, so a paused-piece media Apply never sends a frozen `checkout_image` the server rejects — which would 400 the WHOLE `PUT` and drop the legit gallery changes made in the same Apply. Matches the server freeze (§2.7a). (§5.4e)
...
*7.2j — reword the customer-facing cart-hold promise (breadth-integration #1, v3.7.3).* The storefront still promises the 15-min hold in four places that go **false** once §7.2 ships — remove or reword them to the no-reservation reality (pieces are one of a kind and NOT held; checkout is first-come — complete promptly). **Proposed copy (storefront voice — Sean/Em confirm tone):**
- **`policies.html:167-172`** — **DELETE the entire `<section id="cart-hold">` block.** A truthful `<section id="availability">` already sits directly above it (`:160-165` — heading "Availability & Cart" + the not-reserved / verify-availability-at-checkout / no-artificial-urgency copy), and the FAQ `/policies#availability` deep-link already resolves to *it* — so a rename would create a **duplicate id** and leave the false hold copy live. Just remove the false section; no new copy needed. **(B#1 + journey, v3.7.4 — supersedes the v3.7.3 rename.)
- **`policies.html:30`** — meta description: drop "cart holds" (e.g. *"How availability, returns, and care work at Everlastings…"*).
- **`faq.html:194`** — drop the "place a 15-minute soft hold" clause; keep + lead with the already-true *"Adding a piece to your cart does not reserve it — another collector may complete their purchase first. We verify availability when you click Checkout."*; the `/policies#availability` link now resolves (the section id above).
- **`shipping.html:197`** — drop "and cart-hold" so the label matches reality: e.g. *"Full availability details: see our Policies."* (the `/policies#availability` link already resolves; C, v3.7.4).

*(This is the customer-facing footprint of the accepted no-reservation trade-off, ledger 78 — the honest copy is what keeps the site from promising a protection it no longer has. `about.html` was grep-cleared; these four are the whole footprint.)*

**Doc impact:** remove `cart_holds` from the `EVERLASTINGS_STORE.md` schema + the checkout-flow description (no reservation; sold-out is caught at the availability re-check, which drives sold-recovery); note the `reserve` endpoint is now "availability check + subscriber capture," not a hold. **(C#1, v3.7.2 — mark the reservation SUPERSEDED everywhere it's still documented as live + add the go-live note):** also flip **KAD #18 "Availability check BEFORE any PII" / #19 "Soft cart holds, not hard reservations"** to SUPERSEDED, drop `cart_holds` from the **API map** (`POST /api/checkout/reserve` is now availability+subscriber, no hold), **also flip the higher-precedence Architecture-Reference duplicates AR #28/#29** (`:310–319` — the `[CHECKOUT]`-fires-a-15-min-hold flow note + the "Soft cart holds, not hard reservations" entry; STORE.md's own rule says **AR OUTRANKS KAD** on conflict, so leaving them keeps the authoritative copy asserting the hold as live), **and correct the table counts at each literal spot**: `Tables (8)` (System Diagram, `:140`) → 7, "all 8 tables" (`:62`) → 7, "all six core tables" (is_test paragraph, `:1142`, which lists `cart_holds`) → **five** (drop `cart_holds` from that list), and "8 tables (was 7) — added `cart_holds`" (`:1150`) → revert to 7. **Go-live note:** the store returns to the **no-reservation** design (ROADMAP #224) — a two-shopper race on a one-of-a-kind piece is an **accepted, low-volume trade-off**: `record_sale` keeps `quantity ≥ 0` (the counter never oversells) and a rare true double-pay is recovered by the existing amount-based refund (refund with an empty `relist_product_ids`), the same way `CRON_SECRET` was promoted to the go-live checklist (ledger 59). **Also name at their literal spots (C#3, v3.7.4):** the System-Diagram `cart_holds` row itself (`:148`) and the availability-formula that defines "available for purchase" via `NOT EXISTS (cart_hold …)` (`:1140`) — reword both so the as-built sync catches them. **Test suite (C#1, v3.7.4):** retire `tests/integration/08_hold_expiry.sh` (it POSTs a `cart_holds` row and asserts `410 hold_expired` — a false-fail once §7.2a removes that check) and drop its entry from `tests/integration/run-all.sh`; spot-check `06_checkout.sh` and `10_full_purchase_flow.sh` (both mention a hold) at test-run. *(As-built `STORE.md` edit lands post-build with the other doc-syncs — this names the required change so the sync + go-live checklist catch it.)*
...
**(g) create sale — `handleCoupon`.** **⚠️ Re-anchor (v3.7.3, C + journey #2):** apply order is WS2→WS4→WS8 — WS4 §4.1.c inserts its supersede-prior block BETWEEN the `const promo = …create(promoParams)` line and this `return`, so the pre-WS4 `:745-746` pair is **no longer contiguous** once §4.1.c lands. Anchor ONLY on the unique `return jsonResponse(...)` line and insert the log immediately before it (the same re-anchor discipline §8.1d.b already uses, ledger 74). **CURRENT (the `handleCoupon` success return — anchor on this line alone):**
```ts
    return jsonResponse(request, { success: true, code: promo.code, coupon_id: coupon.id, promotion_code_id: promo.id, expires_display: typeof body.expires_at === 'number' ? formatExpiry(body.expires_at) : null });
```
...
*Label-consistency delta (C, v3.7.4): the inherited `PUBLISHING:` beat's publish-400 example still reads "Missing required fields: story_card" — swap `story_card` → `Story` in the copied `v3_5_0_GPT_INSTRUCTIONS_TRIMMED.txt` so it matches the server's new plain-label 400 (`FIELD_LABELS`, ledger 92) and the maker sees the same wording on BOTH surfaces. Byte-negative (~5B), no `.txt`-cap impact; the beat is inherited from the base file, not re-quoted in the blocks below.*
...
**Residual (breadth integration #2, v3.7.1) — the instruction `.txt` line 4 still says `Optl.`, and that's a bet we now VERIFY.** Line 4's `Reqd:` list already includes `features`, but the other six publish-required detail fields (`dimensions, weight, materials, care_instructions, shipping_details, quantity`) still ride the trailing `Optl. fields in PRODUCT-REFERENCE.` framing — left unchanged here (the 12-byte headroom, `7988/8000`, can't absorb a line-4 rewrite, and §10.2b already reworks the create-is-lenient framing). §10.1c **bets** the schema's per-field `Required to publish.` description overrides line 4's `Optl.` for the GPT's *collection* behavior. That bet is now **verified, not assumed** — TESTING 6b's GPT half asserts the GPT **proactively asks for** the six during a fresh create (not merely that it 400s if they're missing). **If the test shows the GPT skips them**, the schema-only approach failed → rework line 4 to move the six out of `Optl.`, self-funded via §10.2b's freed clauses (~40+ bytes) so the `.txt` stays under 8000.
...
## Verification (end-to-end, dev preview — full plan in `v3_7_4_ADDENDUM_TESTING.md`)
