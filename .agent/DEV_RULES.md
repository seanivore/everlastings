...
### Model & effort selection — split the roles by benchmark; run the loop IN Claude Code

Which model runs which seat is a real lever — pick it by the seat's actual workload, not one default for everything. Calibration data lives here so it compounds; add a dated row after each cycle. (Figures below are from the 2026-07 Anthropic model cards; keep them as a *starting point*, not gospel — re-check when a new model lands.)

- **The harness is the load-bearing variable — not the model tier.** Everlastings' v3.6 A-gate ran the *entire* loop (reviewers **and** orchestrator) on **Opus 4.8 MAX** via the **Agent SDK** and still produced the classic failures: obvious-fold approvals, fold-then-ask ordering, forgotten version bumps, cookie-cutter inattention. Root cause was **environment, not capability** — SDK instances lack the Claude Code terminal basics (they "float," hallucinate tool use) and, critically, don't inherit the *ambient seriousness* that memory files, `.agent/` docs, to-do lists, plans, and a real chat give a terminal instance. A workflow authored *for* Claude Code instances degrades when run outside one. **Run the gap-review loop inside Claude Code** (terminal subagents the orchestrator spawns, or human-pasted blocks) — the memory/plan/todo scaffolding is not optional polish, it's what makes the reviewers take the work seriously.
- **Reviewers (A/B/C/D + breadth) = knowledge-work reading + reasoning**, not multi-step file mutation. On the matching benchmarks **Sonnet 5 ≈ Opus 4.8** — GDPval-AA v2 (knowledge work) **1618 vs 1615** (Sonnet marginally *ahead*); HLE-with-tools **57.4 vs 57.9** (dead heat). Same **1M context** (fits a 300 KB triplet + repo), same adaptive thinking, at **~2.5× lower cost** ($2/$10 vs $5/$25 per MTok through 2026-08-31). Reviewers are the bulk of the loop's token spend, so this is the **rate-limit unlock** — run the swarm on **Sonnet 5**.
- **Effort is a separate lever from model, and for reviewers it's the one that matters most.** A fresh subagent has *none* of the ambient seriousness above, and even Opus MAX under-reads long docs unless explicitly pushed — so **effort is what forces the full-doc read** the gap-hunt lives on. Run reviewers at **MAX** (thoroughness is the entire product of a review; don't economize the deep read). The Twitter "just use lower effort, results are fine" advice holds for routine work and is *wrong* for gap-hunting.
