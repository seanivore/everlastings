...
3. **Orchestrator folds** real findings into IMPLEMENT. A "gap" that's actually an architecture decision → pause, surface to the human (don't research it away). Watch for *plan drift* — fresh reviewers know only what the plan says, not what it's *for*, so they sometimes reshape the architecture; steer back or escalate.
...
**Re-run the breadth pass on every version-push, and expect non-monotonic convergence.** The cheap in-session breadth pass — **two SEPARATE subagents (one owner-journey, one integration; never a single combined agent), through the project lens** — is worth running after *each* fold (every version-push with changes folded in), not only while first-drafting. A fold can silently introduce the next bug, so the doc that just changed is exactly the doc to re-scan; it's near-free insurance against regressions the fold itself created. It stays **advisory** — never a substitute for the fresh-instance gate. And convergence is **non-monotonic**: a clean-looking pass can still surface something, and a fix can *create* the next finding (in the Everlastings v1.5 gate, an 8th-round data-loss bug was introduced by the 7th round's own fix). That's why the exit condition is *a fresh pass that finds nothing load-bearing* — never *it feels close*.
...
2. **The full landmines list** (the project's hard-won truths — "validate against reality, not training data"). **Living + bounded + current-only:** append each newly-found landmine as the loop runs; a fold that *supersedes* one **replaces** it (never append a contradiction); prune at each condense. (Everlastings' v1.5 A-loop grew it ~6→36; that accretion drove the build near-flawless.) **Inline-per-block only while the reviewer is a web-UI agent that needs a self-contained paste** (the cold A-loop). Once the loop moves to in-repo Claude Code reviewers (B/C/D at 1M context, handed the whole file), **collapse the duplicated per-block lists to ONE shared "Settled — do not re-raise" ledger** every block points to — same content, far less bulk (Everlastings v3.1 ran 37 landmines inlined ×5 blocks; v3.2 collapsed them to one shared ledger for the B/C/D batch).
...
**Closing the gate angle-by-angle (anti-polish-treadmill).** Once an angle returns READY TO BUILD it does **not** get a fresh full re-hunt next round. It re-runs only if a later fold landed a **named change (X) in that angle's lane** — and then it is **scoped to confirm just X**: told explicitly that it passed, that the repo is otherwise unchanged, and that it must NOT re-audit the cleared build. No X in its lane → the angle stays **CLOSED**. So the gate closes **angle-by-angle**, not "all angles must end together." (An angle that returned NEEDS-ANOTHER-PASS still gets a normal full re-review — it's genuinely open.) The reason: a passed angle re-run at maximum effort will *always* find one more "polish" nicety, and reflexively folding those manufactures work plus new seams that need yet another round. The **2-subagent breadth pass is the cross-lane backstop** for a fold whose consequence we didn't predict, so closing on small, contained folds is gate-consistent — not a shortcut. (This is the opposite case from the "don't let the lens shrink the scope" mandate above, which governs *first* passes — there a reviewer wrongly *excluding* part of the build is the failure; a deliberately scoped re-review of an *already-cleared* angle is intentional. Everlastings v3.2 round 3: B+C scoped-confirmed clean / one-LOW-in-lane instead of inventing polish, while D — still open — drew its own closure line.)
...
**Surface it as a soft preamble, not a mandate.** In the execution copy, lead with a short *"Orchestration — suggested, evaluate for yourself"* section: the parallel/sequential groupings, the real sequencing constraints, and what the orchestrator must NOT delegate (gate calls, branch/commit state, escalation, verification reads, any ships-verbatim artifact) — framed explicitly as practice to take in and then run the build its own way. The point is that the executing orchestrator makes its delegation call *from* good practice (so a poorly-run build can't happen merely because we suggested nothing), without being boxed in. (Everlastings `v3_3/v3_3_0_IMPLEMENT.md` opens with one.)

**Surface the human's steps early — and default to agent-does-it.** Whatever the build will need a *human* to do (a dashboard click, a credential, a one-off script, a content decision) goes in a clear human-step list produced **up front during planning**, each step saying *why it's the human's* — not discovered at the end of a build guide. And bias hard toward the agent doing it: anything an agent *could* do (set up env via the Vercel CLI, run an idempotent bootstrap script, scope-and-run a grep, draft interim copy) is **offered to the agent** for the human to approve or push back on, rather than parked as a human chore. Two failure modes this kills: moot steps that survive because no one re-checked them (a "point DNS" step that was always automatic), and agent-doable steps the human ends up doing by hand (re-entering env vars) — both leave the human anxious they're missing something they actually understand. A quick "is this really yours to do?" pass at planning time is cheaper than the end-of-build surprise. (Everlastings v2.0.0 go-live: half the "pre-flight" turned out moot or agent-runnable.)
...
**Long, multi-angle gate loops — demarcate phases with a MINOR bump.** The default above (each pass bumps PATCH) is right for a normal loop. When a gate runs many rounds across multiple angles and the per-version `docs/archive/vX_Y/` dir gets large, you MAY open each new *review phase* with a MINOR bump so each phase keeps its own directory and a self-contained patch-trail: the holistic A-loop lives in `vX.5.*`, the B/C(/D) round opens at `vX.6.0`, **the condense into the execution copy opens its own MINOR phase dir** (where the review scaffolding is finally shed — see *The execution copy*, below), and execution/as-built opens the next MAJOR (`v(X+1).0.0`). (Everlastings v1.5: A ran in `v1_5/` as `v1.5.1…v1.5.9`, B/C opened at `v1_6/` `v1.6.0`; the v3.x cycle ran B/C/D in `v3_2/` and condensed into the execution copy `v3_3/` at `v3.3.0`, the build/as-built opening `v4.0.0`.) The number still tracks changes (no empty bumps); the minor boundaries just keep a long trail navigable.

**The execution copy — condense the gate-cleared docs into a lean builder's guide.** Once the gap-gate clears, the living planning docs still read as a *gate record* — thick with review scaffolding (per-fold `AR`/round/reviewer/verdict tags, rejected-alternative notes, the `REVIEW_PROMPTS` charters) that is the wrong context for the agent who will *execute*. So before promotion/as-built, do a deliberate **condense into an execution copy** — a fresh, full **linear** read (no chunking, per *As-built doc-sync*) that produces the orchestrating executor's clean copy. It opens a new MINOR phase dir entered by **COPY** across the boundary (so the prior dir stays frozen as the diffable pre-condense snapshot — this subsumes the `_2`-diff carve-out for the condense). Three rules: **(1) cut the `REVIEW_PROMPTS` file entirely** — the executor reads IMPLEMENT + addenda only; **(2) keep every CURRENT/NEW byte-anchor, test, phase number, and settled decision** (and any genuinely *operational* "why" — an ordering constraint or a footgun warning that stops a builder re-breaking a fix); **(3) relocate the rest of the why** to a RATIONALE companion (see *The Artifact System*) rather than deleting it. Fenced code blocks (CURRENT/NEW + any ships-verbatim block) stay **byte-identical** — the condense touches prose only. Run the 2-subagent breadth pass on the result (a condense can drop a load-bearing line — non-monotonic). (Everlastings v3.3.0: v3.2.3's B/C/D-gate-cleared trio condensed into `v3_3/`; ~79 `AR#` tags + round/reviewer/verdict framing shed, every anchor/test/phase kept.)
...
**Archiving GAP REVIEW surfaced revisions — current-only living docs; keep the records.** When GAP REVIEWS bump from revisions they surfaced, the **living planning docs** (the IMPLEMENT + its addendums + the review-prompt charters) are **current-only**: rename them to the new version (`git mv`, history preserved) so git + descriptive per-file commits hold the superseded patch and the dir doesn't fill with near-duplicates. **Terminal records are kept standing, never renamed:** the per-pass `vX_Y_Z_GAP_REVIEW_<angle>.md` findings, `BUGS`, and `FEEDBACK` logs — they're cited forward and are the visible rigor trail. **Two carve-outs:** (1) when a living doc is *condensed/whittled* (a big rewrite, not a normal fold), keep a diffable pre-condense copy (`…_2.md`) so the human's inline notes aren't silently lost; (2) *dead-end **decisions** within a doc* stay in context (anti-mixed-truth — a content rule, distinct from keeping old version *files*). This relaxation rides on consistent, descriptive commits; without them, prefer keeping copies. (Early drafts of this doc imagined keep-every-file as the norm before real flow was recorded — this is the reconciliation.) **Crossing into a new phase *directory* (a MINOR-bump batch boundary) is a COPY, not a `git mv`:** save-as the highest prior living docs into the new dir under the new version (e.g. `v3_1/v3_1_7_*` → `v3_2/v3_2_0_*`), leaving the prior dir's highest version **standing as a frozen reference snapshot** — never leave a closed phase dir without its final living-doc state. (Within a dir, rename-on-bump still holds; git carries the patch. The boundary copy is the one deliberate near-duplicate, so a finished phase's dir reads as a complete record.)
...
### The core file types (all in `docs/archive/vX_Y/`)
...
**The RATIONALE companion (optional — pairs with an execution copy).** When the gate-cleared docs are condensed into the execution copy (§ *Versioning → The execution copy*), the operational why + decision rationale stripped from the lean docs is **relocated, not deleted**, into a `vX_Y_Z_RATIONALE.md` — clean and concise, keyed by the same phase/workstream/section labels the build guide uses, so a doubting executor finds the relevant reasoning instantly. It is **not** an `AR#`/round-by-round dump (that forensic trail stays in the standing `GAP_REVIEW` records); it captures the *considered* why ("why amount-based refunds," "why this CSS block must be physically last"). The execution copy references it **softly** — one line saying it exists, that the build doesn't require it, and *don't load it by default*; the executor opens it only if an edit's shape makes them want to question or "fix" it. Close the loop: the **BUILD_REPORT notes candidly whether it was consulted at all** — builds that never need it say "fold the load-bearing bits back inline and drop the file"; builds that lean on it say the lean copy cut too much. (First run: Everlastings `v3_3/v3_3_0_RATIONALE.md`.)
...
**Go-live hygiene (every ship to main):** bump any legal **effective dates** (privacy/terms) to the ship date, and run the **scoped** content-placeholder check — a repo-wide `grep -rn 'PLACEHOLDER:'` cries wolf (it catches docs, archives, and any styleguide that *documents* the convention), so scope it to the served files (e.g. `grep -n 'PLACEHOLDER:' *.html | grep -v <styleguide>`) and have the agent run it, not the human.
...
- **Environment secrets — agent sets them up; readable by default.** On a solo-owner project, the agent provisions env vars via the **Vercel CLI at project start** (`vercel env add` per scope) rather than handing the human a checklist — fewer transcription errors. And **don't flag them Sensitive**: a Sensitive value can't be read back from the dashboard or `vercel env pull`, which forces re-minting keys every time a value is in doubt; left readable, the sole owner just copies the real value to verify. (The owner has exclusive Vercel access, so the exposure tradeoff is theirs to accept — name it, don't decide it for them. A single highest-blast-radius secret can still be Sensitive by explicit choice.)
...
**As you work:** if the plan is wrong, stop + surface + bump the IMPLEMENT (don't silently patch); confirm via `git diff` before commit.

**Before closing:** `BUILD_REPORT_<source>.md` if you ran a BUILD; update `PROJECT_NAME.md` if architecture changed; commit mirroring the chunk.
...
- write a separate "completion/walkthrough" file (orchestrator findings go in the BUILD_REPORT);
